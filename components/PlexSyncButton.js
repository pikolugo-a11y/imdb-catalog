...
export default async function PlexSyncButton(){
  ...
  const [[lastComplete],[activeRun]]=await Promise.all([
    ...
    sql`SELECT run_id,process_code,started_at,requested_at FROM process_runs WHERE process_code IN('PROC-NOV-009','PROC-NOV-008') AND technical_status IN('queued','running') AND finished_at IS NULL ORDER BY requested_at DESC LIMIT 1`
  ]);
  ...
  return <PlexSyncButtonClient defaultReviewFrom={value} initiallyRunning={Boolean(activeRun)} activeProcess={activeRun?.process_code||null}/>;
}
