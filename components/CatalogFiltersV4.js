...
export default function CatalogFiltersV4({genres=[],countries=[],invalidYearRange=false}){
  ...
  return <section className="cv4-filters" aria-label="Filtros del catálogo">
    ...
    <label><span>País</span><select value={country} onChange={e=>apply({country:e.target.value||null})}><option value="">Todos</option>{countries.map(c=><option key={c} value={c}>{c}</option>)}</select></label>
