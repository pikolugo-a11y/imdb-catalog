...
export default function CatalogFiltersV4({genres=[],invalidYearRange=false}){
  ...
  const clear=()=>{const p=new URLSearchParams(sp.toString());['q','plex','genres','genreMode','yearFrom','yearTo','country','page','status','genre'].forEach(k=>p.delete(k));router.replace(`${path}${p.toString()?`?${p}`:''}`,{scroll:false});};
