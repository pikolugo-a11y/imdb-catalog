...
20. **Automatización masiva segura sobre operaciones unitarias.** Lifecycle unitario no prohíbe el procesamiento masivo. La unidad canónica sigue siendo una operación idempotente y trazable sobre un título/serie/archivo/etapa; un Batch Engine puede ejecutar muchas unidades consecutivas. Los lotes nunca deben depender de mantener abierto un request de Vercel ni convertir GitHub Actions en infraestructura de procesamiento continuo. Toda automatización masiva debe usar cola/checkpoint, concurrencia acotada, rate limit por fuente, backoff, circuit breaker, pausa/cancelación, presupuestos y parada obligatoria ante estados de revisión humana. No se aumenta velocidad a costa de riesgo de suspensión de GitHub, Vercel, Neon o fuentes externas. La arquitectura canónica está en `docs/BATCH_AUTOPILOT_ARCHITECTURE.md`.
