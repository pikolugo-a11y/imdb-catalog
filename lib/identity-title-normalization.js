...
export function validateIdentityEvidence(e){
 ...
 const spanishPairs=[titleSimilarity(e.imdb_title,e.tmdb_title_es),titleSimilarity(e.imdb_title,e.fa_title_es),titleSimilarity(e.tmdb_title_es,e.fa_title_es)].filter(Number.isFinite);
 const spanishStrong=Math.max(0,...spanishPairs)>=90;
 ...
 // Dos fuentes muy fuertes + mismo año + títulos localizados coherentes: evita falsos negativos por alfabetos/transliteraciones.
 const crossAlphabetRescue=exactYears&&strongestPair>=82&&spanishStrong;
 if(crossAlphabetRescue){score=Math.max(score,88);suspected=null;}
 const severeOutlier=Boolean(suspected&&sourceAvg[suspected]<48&&pairWithout[suspected]>=88&&!crossAlphabetRescue);
 ...
 return{status,score,suspected,details:{explanation,similarity:{imdb_tmdb:simIT,imdb_fa:simIF,tmdb_fa:simTF},years:{imdb_tmdb:yi,imdb_fa:yf,tmdb_fa:ytf},source_average:sourceAvg,spanish_bonus:spanishBonus,cross_alphabet_rescue:crossAlphabetRescue}};
}
