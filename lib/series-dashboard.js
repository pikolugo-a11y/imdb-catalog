...
const displayBand=b=>({honors:'excellent',outstanding:'excellent',notable:'very_good',good:'correct',sufficient:'improvable',fail:'deficient'}[b]||b);
...
export async function getSeriesDashboard(showRatingKey){
  ...
  try{
    aggregates=await sql`
      ...
      WHERE formula_version=${PIKOQUALITY_ACTIVE_VERSION}
        AND ((entity_type='show' AND entity_key=${key}) OR (entity_type='season' AND parent_key=${key}))
      ORDER BY entity_type,season_index NULLS FIRST`;
  }catch{
    aggregates=[];
  }

  const normalized=aggregates.map(x=>({...x,canonical_band:x.band,band:displayBand(x.band)}));
  const quality=normalized.find(x=>x.entity_type==='show')||null;
  const seasonQuality=normalized.filter(x=>x.entity_type==='season');
