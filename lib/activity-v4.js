...
export const ACTIVITY_PLANNER_HEALTH_MINUTES=75;
export const ACTIVITY_DISPATCH_HEALTH_MINUTES=75;
...
const DOMAIN_RULES=[
  ...
  ['novedades',['PROC-NOV-']],
  ['calidad',['PROC-ID-','PROC-IV-','PROC-DATA-','PROC-MOV-','PROC-SER-','PROC-PQ-','PROC-REL-']],
...
function episodeCode(season,episode){return `T${Number(season)}E${Number(episode)}`}
function manualSeriesSummary(row){const c=row.context||{},after=row.after_compact||{};if(row.process_code!=='PROC-SER-005')return null;if(c.operation==='review_series_double_episode'){if(c.mode==='reopen')return `Deshiciste la decisión manual de ${episodeCode(c.season,c.episode)}.`;return `Marcaste ${episodeCode(c.season,c.episode)} como incluido en ${episodeCode(c.season,c.source_episode)}. La decisión queda guardada hasta que tú la cambies.`}if(c.operation==='review_series_extra'){if(c.decision==='reopen')return `Reabriste la revisión de ${episodeCode(c.season,c.episode)}.`;if(c.decision==='not_needed')return `Marcaste ${episodeCode(c.season,c.episode)} como episodio extra que no necesitas.`;if(c.decision==='special')return `Aceptaste ${episodeCode(c.season,c.episode)} como especial / extra válido.`}if(after?.decision==='manual_present')return 'Decisión manual guardada.';return null}
function functionalSummary(row){
  const explicit=String(row.after_compact?.activity_summary||'').trim();if(explicit)return explicit;
  const manual=manualSeriesSummary(row);if(manual)return manual;
  ...
  if(row.process_code==='PROC-PLAN-002'&&row.functional_result==='no_change')return 'Comprobación automática terminada. No había nada que reorganizar.';
...
function visibleEntityLabel(row){const label=String(row.after_compact?.entity_label||row.entity_label||'').trim();if(!label||/^\d+(?:,\d+)*$/.test(label)||/^PROC-[A-Z]+-\d+$/i.test(label)||label==='automatic')return null;return label}
function humanTitle(row,entityLabel){if(row.process_code==='PROC-PLAN-002')return 'Trabajo automático';if(row.process_code==='PROC-SER-005')return entityLabel?`${entityLabel} · decisión manual`:'Decisión manual de serie';if(row.process_code==='PROC-SER-002')return entityLabel?`${entityLabel} · actualización Plex`:'Actualización Plex de serie';const process=processDisplay(row.process_code);return entityLabel&&row.entity_type?.startsWith('series')?`${entityLabel} · ${process.name}`:process.name}
function mapRun(row){
  const entityLabel=visibleEntityLabel(row);
  return {...row,kind:'run',domain:activityDomain(row.process_code),title:humanTitle(row,entityLabel),entityLabel,statusLabel:statusText[row.technical_status]||row.technical_status,resultLabel:functionalSummary(row),originLabel:activityOrigin(row.trigger_source),attention:['failed','partial'].includes(row.technical_status)||row.functional_result==='blocked',technicalHref:`/admin/runs/${row.run_id}`};
}
function collapsePlannerNoise(items){const seen=new Map(),out=[];for(const item of items){const noise=item.process_code==='PROC-PLAN-002'&&item.technical_status==='succeeded'&&item.functional_result==='no_change'&&item.originLabel==='Automático';if(!noise){out.push(item);continue}const key=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Madrid',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(item.requested_at));const existing=seen.get(key);if(existing){existing.collapsedCount++;existing.resultLabel=`${existing.collapsedCount} comprobaciones automáticas realizadas. No hubo cambios.`;continue}item.collapsedCount=1;item.resultLabel='1 comprobación automática realizada. No hubo cambios.';item.entityLabel=null;seen.set(key,item);out.push(item)}return out}
...
const entityLabelJoin=`
  ...
  LEFT JOIN series_reference asr ON asr.show_rating_key::text=COALESCE(NULLIF(split_part(r.entity_id,':S',1),''),r.context->>'rating_key')`;
const entityLabelSelect=`COALESCE(NULLIF(r.after_compact->>'entity_label',''),am.title_es,am.title,am.original_title,ap.name,ascg.name,asr.title,r.entity_id) AS entity_label`;
...
export async function getActivityTimeline({domain='todo',q='',page=1,limit=ACTIVITY_PAGE_SIZE}={}){
  ...
  return{items:collapsePlannerNoise(rows.slice(0,safeLimit).map(mapRun)),hasMore:rows.length>safeLimit,page:safePage};
}
...
export async function getActivityRunDetail(runId){
  ...
  const safeErrors=errors.map(error=>({...error,functionalMessage:error.entity_id?`No se pudo completar el trabajo sobre una entidad de ${error.entity_type||'PikoFilm'}.`:'No se pudo completar una parte de esta actividad.',nextStep:error.resolved_at?'La incidencia ya está resuelta.':error.retryable?'PikoFilm puede volver a intentarlo automáticamente.':'Puede requerir revisión.'}));
...
export async function getActivityCalendar(){
  ...
  const [rows,capacityRows,statsRows,dailyRows,plannerRows]=await Promise.all([
    ...
    sql.query(`SELECT ((planned_at AT TIME ZONE 'Europe/Madrid')::date)::text AS day_key,count(*)::int plan_count,COALESCE(sum(estimated_load),0)::float8 estimated_load,COALESCE(sum(planned_volume),0)::bigint volume,COALESCE(sum(planned_volume) FILTER(WHERE protected),0)::bigint protected_volume,bool_or(deliberate_peak) deliberate_peak FROM process_plans WHERE ${planScope} AND planned_at IS NOT NULL GROUP BY 1 ORDER BY 1`),
    sql.query(`SELECT run_id,technical_status,functional_result,requested_at,finished_at,context FROM process_runs WHERE process_code='PROC-PLAN-002' AND parent_run_id IS NULL AND trigger_source='activity_planner' ORDER BY requested_at DESC LIMIT 30`)
  ]);
  const plans=rows.map(mapPlan),baseline=Math.max(900,Number(capacityRows[0]?.baseline||0)),stats=statsRows[0]||{};
  const daily=dailyRows.map(row=>({day:String(row.day_key),planCount:Number(row.plan_count||0),estimatedLoad:Number(row.estimated_load||0),volume:Number(row.volume||0),protectedVolume:Number(row.protected_volume||0),deliberatePeak:Boolean(row.deliberate_peak),baseline}));
  ...
  const latestAutomatic=plannerRows[0]||null,latestFull=plannerRows.find(row=>row.context?.mode==='full')||null;
  const automaticAgeMs=latestAutomatic?Date.now()-new Date(latestAutomatic.requested_at).getTime():Infinity,fullAgeMs=latestFull?Date.now()-new Date(latestFull.requested_at).getTime():Infinity;
  const dispatchHealthy=Boolean(latestAutomatic&&['succeeded','running'].includes(latestAutomatic.technical_status)&&automaticAgeMs<=ACTIVITY_DISPATCH_HEALTH_MINUTES*60000);
  const fullHealthy=Boolean(latestFull&&['succeeded','running'].includes(latestFull.technical_status)&&fullAgeMs<=ACTIVITY_PLANNER_HEALTH_MINUTES*60000);
  const cronSecretConfigured=Boolean(process.env.CRON_SECRET),plannerHealthy=cronSecretConfigured&&dispatchHealthy&&fullHealthy;
  const planner={healthy:plannerHealthy,cronSecretConfigured,dispatchHealthy,fullHealthy,lastRunAt:latestAutomatic?.requested_at||null,lastFullRunAt:latestFull?.requested_at||null,status:latestAutomatic?.technical_status||'never',runId:latestAutomatic?.run_id||null};
