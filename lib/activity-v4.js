...
export async function getActivityCalendar(){
  ...
  const totalPlans=Number(stats.total_plans||0),counts={pending:Number(stats.pending_volume||0),pendingPlans:Number(stats.pending_plans||0),attention:Number(stats.delayed_plans||0)+overloads.filter(x=>!x.deliberatePeak).length,next7:Number(stats.next7_volume||0)};
  return{plans,counts,overloads,daily,baselineLoadSeconds:baseline,totalPlans,truncated:totalPlans>plans.length,detailLimit:ACTIVITY_PLAN_DETAIL_LIMIT};
}
