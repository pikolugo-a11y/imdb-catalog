...
export async function getActivityCalendar(){
  const sql=db();
  const [rows,capacityRows,statsRows,dailyRows]=await Promise.all([
    ...
    sql.query(`SELECT count(*)::int total_plans,COALESCE(sum(planned_volume) FILTER(WHERE status='pending_planning'),0)::bigint pending_volume,count(*) FILTER(WHERE status='delayed')::int delayed_plans,COALESCE(sum(planned_volume) FILTER(WHERE planned_at>=now() AND planned_at<now()+interval '7 days'),0)::bigint next7_volume FROM process_plans WHERE ${planScope}`),
    sql.query(`SELECT ((planned_at AT TIME ZONE 'Europe/Madrid')::date)::text day,COALESCE(sum(estimated_load),0)::float8 estimated_load,COALESCE(sum(planned_volume),0)::bigint volume,COALESCE(sum(planned_volume) FILTER(WHERE protected),0)::bigint protected_volume,bool_or(deliberate_peak) deliberate_peak FROM process_plans WHERE ${planScope} AND planned_at IS NOT NULL GROUP BY 1 ORDER BY 1`)
  ]);
  const plans=rows.map(mapPlan),baseline=Math.max(900,Number(capacityRows[0]?.baseline||0)),stats=statsRows[0]||{};
  const overloads=dailyRows.map(day=>({day:String(day.day),estimatedLoad:Number(day.estimated_load||0),volume:Number(day.volume||0),protectedVolume:Number(day.protected_volume||0),deliberatePeak:Boolean(day.deliberate_peak),baseline})).filter(day=>day.estimatedLoad>baseline*1.5).map(day=>({...day,reason:day.deliberatePeak?'Pico deliberado por encima de la carga habitual.':'La carga prevista sigue muy por encima de la carga habitual incluso tras la planificación actual.'}));
  const totalPlans=Number(stats.total_plans||0),counts={pending:Number(stats.pending_volume||0),attention:Number(stats.delayed_plans||0)+overloads.filter(x=>!x.deliberatePeak).length,next7:Number(stats.next7_volume||0)};
  return{plans,counts,overloads,baselineLoadSeconds:baseline,totalPlans,truncated:totalPlans>plans.length,detailLimit:ACTIVITY_PLAN_DETAIL_LIMIT};
}
