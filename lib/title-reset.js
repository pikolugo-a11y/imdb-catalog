...
const RESET_TABLES=new Set([
  ...
  'catalog_read_model','movie_countries','movie_country_names','movie_genre_names','movie_genres_canonical','title_ratings'
]);
...
export async function resetTitleToNews(imdbId,trace=null){
  ...
  await trace?.event?.({eventType:'step_completed',step:'reset_catalog_state',message:'Estado derivado retirado y candidato recreado',data:{tables:present}});
