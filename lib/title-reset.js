...
const PRESERVE_TABLES=new Set(['process_runs','process_run_events','process_run_errors','admin_events','pipeline_runs','person_filmography']);
...
export async function resetTitleToNews(imdbId,trace=null){
  ...
  const ratingKeys=(await sql`SELECT DISTINCT p.rating_key AS rating_key FROM plex_items p JOIN plex_external_ids x ON x.rating_key=p.rating_key AND x.provider='imdb' AND x.external_id=${id}`).map(x=>String(x.rating_key)).filter(Boolean);
  const ops=[];
