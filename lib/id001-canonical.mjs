...
async function fetchTmdb(imdbId,itemType,{apiToken,apiGate,lane='manual',owner='id001'}={}){
  ...
  let permit=null;
  try{
    permit=await gate.acquire('tmdb',{lane,owner});
    ...
    if(response.status===429){const retryAfter=Math.max(1,Number(response.headers.get('retry-after'))||60);const e=Object.assign(new Error(`TMDb limitó la petición (429)`),{status:429,retryAfter,source:'tmdb',processStep:'resolve_tmdb',retryable:true});await gate.failure('tmdb',e,{permit,lane});throw e}
    if(!response.ok){const e=Object.assign(new Error(`TMDb respondió HTTP ${response.status}`),{status:response.status,source:'tmdb',processStep:'resolve_tmdb',retryable:response.status>=500});await gate.failure('tmdb',e,{permit,lane});throw e}
    const f=await response.json();
    await gate.success('tmdb',{permit,lane});
    ...
    throw error;
  }finally{if(permit?.leaseId)await gate.release(permit).catch(()=>{})}
}
