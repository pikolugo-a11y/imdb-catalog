...
export async function addProcessEvent(runId,input={}){
  ...
  if(['batch_queued','batch_items_appended','batch_resumed'].includes(eventType)){
    let pool=cleanText(input.data?.worker_pool,40);
    if(!pool){
      const [control]=await sql`SELECT worker_pool FROM batch_run_control WHERE run_id=${runId}::uuid LIMIT 1`;
      pool=cleanText(control?.worker_pool,40);
    }
    if(pool)await wakeWorkerPool(pool,{reason:eventType}).catch(error=>console.error('[worker-wake]',pool,String(error?.message||error)));
  }
  return event;
}
