...
async function universe(){
  ...
  return sql.unsafe(`
    WITH ratings AS (
      SELECT tr.imdb_id,
        jsonb_agg(jsonb_build_object(
          'source',tr.source,'rating',tr.normalized_rating,'votes',tr.votes,'provider',tr.provider,
          'fetched_at',tr.fetched_at,'expires_at',tr.expires_at,'status',tr.status
        ) ORDER BY tr.source) FILTER (WHERE tr.status='available' AND tr.normalized_rating>0) normalized_ratings
      FROM title_ratings tr GROUP BY tr.imdb_id
    ), credit_flags AS (
      SELECT c.imdb_id,
        bool_or(c.credit_type='crew' AND c.job='Director') director,
        bool_or(c.credit_type='cast') cast
      FROM movie_credits c GROUP BY c.imdb_id
    ), genre_flags AS (
      SELECT DISTINCT g.imdb_id FROM movie_genres g
    ), plex_flags AS (
      SELECT DISTINCT px.external_id imdb_id
      FROM plex_items pi JOIN plex_external_ids px ON px.rating_key=pi.rating_key AND px.provider='imdb'
      WHERE pi.active AND pi.item_type IN ('movie','show')
    )
    SELECT m.imdb_id,m.tmdb_id,m.type,m.title_es,m.original_title,m.year,m.final_rating,m.runtime,m.country,m.poster_path,m.backdrop_path,m.source_status,
      ...
      COALESCE(m.ratings_refreshed_at,m.pikoscore_calculated_at) lifecycle_updated_at,
      COALESCE(m.source_status #>> '{data_quality_external_poster,url}','') external_poster_url,
      COALESCE(m.source_status #>> '{data_quality_external_backdrop,url}','') external_backdrop_url,
      ...
      (pf.imdb_id IS NOT NULL) in_plex,(gf.imdb_id IS NOT NULL) genres,COALESCE(cf.director,false) director,COALESCE(cf.cast,false) cast,
      COALESCE(r.normalized_ratings,'[]'::jsonb) normalized_ratings
    ...
    LEFT JOIN ratings r USING(imdb_id)
    LEFT JOIN credit_flags cf USING(imdb_id)
    LEFT JOIN genre_flags gf USING(imdb_id)
    LEFT JOIN plex_flags pf USING(imdb_id)
...
function overviewFrom(rows){
  const resolved=rows.filter(operationallyResolved);
  return{total:rows.length,incomplete:rows.filter(r=>!r.dataReady).length,ratingsPending:rows.filter(r=>r.dataReady&&(!r.ratingsReady||!r.ratingsFresh)).length,scoreReady:rows.filter(r=>r.dataReady&&r.ratingsReady&&r.ratingsFresh&&!r.pikoScoreCurrent).length,resolved:resolved.length,resolvedComplete:resolved.filter(r=>r.coverage===100).length,resolvedImprovable:resolved.filter(r=>r.coverage<100).length,inPlex:rows.filter(r=>r.in_plex).length,stuck:rows.filter(r=>r.stuck).length,averageCoverage:rows.length?Math.round(rows.reduce((s,r)=>s+r.coverage,0)*10/rows.length)/10:100};
}
function filterRows(rows,filters={}){
  const q=String(filters.q||'').trim().toLowerCase(),type=String(filters.type||'all'),plex=String(filters.plex||'all');
  return rows.filter(r=>(!q||String(r.imdb_id||'').toLowerCase().includes(q)||String(r.title_es||'').toLowerCase().includes(q)||String(r.original_title||'').toLowerCase().includes(q))&&(type==='all'||String(r.type||'').toLowerCase()===type.toLowerCase())&&(plex==='all'||(plex==='in'&&r.in_plex)||(plex==='out'&&!r.in_plex)));
}
function pageFrom(allRows,filters={}){
  ...
  let rows=filterRows(allRows,filters);
...
export async function getDataQualityBundle(filters={}){
  const assessed=(await universe()).map(assessDataQuality);
  return{overview:overviewFrom(assessed),page:pageFrom(assessed,filters)};
}
export async function getDataQualityOverview(){const assessed=(await universe()).map(assessDataQuality);return overviewFrom(assessed)}
export async function getDataQualityPage(filters={}){const assessed=(await universe()).map(assessDataQuality);return pageFrom(assessed,filters)}
export async function getDataQualityTitle(imdbId){const rows=filterRows((await universe()).map(assessDataQuality),{q:imdbId});return rows[0]||null;}
