...
export async function getDataQualityView(filters={}){
  ...
  const [result]=await sql`
    WITH genre_flags AS (
      ...
      SELECT imdb_id,bool_or(credit_type='cast') AS cast,bool_or(credit_type='crew' AND job='Director') AS director
      FROM movie_credits
      ...
      LEFT JOIN genre_flags g ON g.imdb_id=m.imdb_id
      LEFT JOIN credit_flags cf ON cf.imdb_id=m.imdb_id
      WHERE ex.imdb_id IS NULL AND cl.lifecycle_state IN ('DATA_INCOMPLETE','PIKOSCORE_PENDING','MOVIE_FILE_PENDING','MOVIE_FILE_REVIEW','SERIES_SYNC_PENDING','SERIES_REVIEW','TECH_PENDING','COMPLETE')
    ), ratings AS (
      SELECT tr.imdb_id,
      ...
      FROM title_ratings tr JOIN base b ON b.imdb_id=tr.imdb_id GROUP BY tr.imdb_id
      ...
      FROM base b LEFT JOIN ratings r ON r.imdb_id=b.imdb_id
    ), filtered AS (
      ...
      SELECT f.*,
        row_number() OVER(ORDER BY
          ...
          lower(COALESCE(f.title_es,f.original_title,f.imdb_id)) ASC,f.imdb_id ASC) AS _row
      FROM filtered f
      ORDER BY
        CASE WHEN ${p.sort}='title' THEN lower(COALESCE(f.title_es,f.original_title,f.imdb_id)) END ASC,
        CASE WHEN ${p.sort}='coverage' OR (${p.sort}='priority' AND ${p.state}='resolved') THEN f.coverage END ASC,
