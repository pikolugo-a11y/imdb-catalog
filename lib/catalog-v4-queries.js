...
export function parseCatalogV4(raw={}){
  ...
  const fallbackSort=defaultSort({scope,plex}),sort=SORTS.has(first(raw.sort))?first(raw.sort):fallbackSort;
...
export function catalogV4Href(state,patch={}){
  const s={...state,...patch},fallbackSort=defaultSort(s),effectiveSort=s.sort||fallbackSort;
...
const priorityCte=`WITH series_votes AS (
 ...
 SELECT sv.imdb_id,
  35*COALESCE(max(cr.market_pct),sv.global_pct)+20*sv.global_pct+1.5*COALESCE(sv.final_rating,0)
  +CASE WHEN bool_or(mc.country_code='ES') THEN 20 ELSE 0 END
 ...
 FROM series_votes sv LEFT JOIN country_rank cr ON cr.imdb_id=sv.imdb_id LEFT JOIN movie_countries mc ON mc.imdb_id=sv.imdb_id LEFT JOIN movies m ON m.imdb_id=sv.imdb_id LEFT JOIN movie_metadata mm ON mm.imdb_id=sv.imdb_id
 GROUP BY sv.imdb_id,sv.global_pct,sv.final_rating,mm.original_language,m.source_status
)`;
