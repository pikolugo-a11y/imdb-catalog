...
function scopeWhereSql(p,params){
  const w=['c.imdb_id IS NOT NULL'];
  if(p.scope==='movies')w.push(`c.type='Película'`);
  if(p.scope==='series'){
    const i=params.push(SERIES_TYPES);
    w.push('c.type=ANY('+String.fromCharCode(36)+i+'::text[])');
  }
  return w.join(' AND ');
}

function whereSql(p,params,{includePlex=true}={}){
  const w=[scopeWhereSql(p,params)];
...
export async function getCatalogV4(raw={}){
  ...
  if(p.invalidYearRange)return{state:p,rows:[],summary:{total:0,in_plex:0,without_plex:0,profile_due:0},filteredTotal:0,universeTotal:0,pageCount:1,page:1};
  const sql=db();
  const rowParams=[PIKOQUALITY_ACTIVE_VERSION,PIKORELEVANCE_VERSION],summaryParams=[PIKOQUALITY_ACTIVE_VERSION,PIKORELEVANCE_VERSION],universeParams=[PIKOQUALITY_ACTIVE_VERSION,PIKORELEVANCE_VERSION];
  const where=whereSql(p,rowParams),summaryWhere=whereSql(p,summaryParams,{includePlex:false}),universeWhere=scopeWhereSql(p,universeParams),order=orderSql(p);
  const [[summary],[countRow],[universeRow]]=await Promise.all([
    ...
    sql.query(`SELECT count(*)::int total ${baseSql} WHERE ${universeWhere}`,universeParams)
  ]);
  ...
  return{state:{...p,page},rows,summary:summary||{total:0,in_plex:0,without_plex:0,profile_due:0},filteredTotal,universeTotal:Number(universeRow?.total||0),pageCount,page};
}
