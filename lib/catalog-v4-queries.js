...
const SORTS=new Set(['score','spain','year','title']);
...
const defaultSort=({scope,plex})=>scope==='series'&&plex==='without_plex'?'spain':'score';
...
export function parseCatalogV4(raw={}){
  ...
  const fallbackSort=defaultSort({scope,plex}),requestedSort=SORTS.has(first(raw.sort))?first(raw.sort):fallbackSort,sort=requestedSort==='spain'&&scope!=='series'?fallbackSort:requestedSort;
...
export function catalogV4Href(state,patch={}){
  const s={...state,...patch},fallbackSort=defaultSort(s),requestedSort=s.sort||fallbackSort,effectiveSort=requestedSort==='spain'&&s.scope!=='series'?fallbackSort:requestedSort;
...
function whereSql(p,params){
  ...
  if(p.country)add(p.country,i=>`(EXISTS(SELECT 1 FROM movie_countries mc JOIN countries ctry ON ctry.code=mc.country_code WHERE mc.imdb_id=c.imdb_id AND ctry.name_es=$${i}) OR trim(COALESCE(mv.country,''))=$${i} OR $${i}=ANY(regexp_split_to_array(COALESCE(mv.country,''),'\\s*,\\s*')))`);
...
export async function getCatalogV4(raw={}){
  const p=parseCatalogV4(raw);if(p.invalidYearRange)return{state:p,rows:[],summary:{total:0,in_plex:0,without_plex:0,profile_due:0},pageCount:1,page:1};
  const sql=db(),params=[PIKOQUALITY_ACTIVE_VERSION],where=whereSql(p,params),order=orderSql(p);
  const [summary]=await sql.query(`SELECT count(*)::int total,count(*) FILTER(WHERE c.effective_status='in_plex')::int in_plex,count(*) FILTER(WHERE c.effective_status<>'in_plex')::int without_plex,count(*) FILTER(WHERE c.type IN ('Serie','Miniserie') AND mv.tmdb_id IS NOT NULL AND (NULLIF(mv.source_status #>> '{series_profile,seasons}','') IS NULL OR NULLIF(mv.source_status #>> '{series_profile,episodes}','') IS NULL))::int profile_due ${baseSql} WHERE ${where}`,params);
  ...
  const rows=await sql.query(`${prefix} SELECT c.imdb_id,c.display_title,c.original_title,c.year,c.type,c.poster_path,c.final_rating,c.effective_status,COALESCE((SELECT array_agg(ctry.name_es ORDER BY ctry.name_es) FROM movie_countries mc JOIN countries ctry ON ctry.code=mc.country_code WHERE mc.imdb_id=c.imdb_id),CASE WHEN NULLIF(trim(mv.country),'') IS NOT NULL THEN regexp_split_to_array(mv.country,'\\s*,\\s*') ELSE '{}'::text[] END) countries,CASE WHEN c.effective_status<>'in_plex' THEN NULL WHEN c.type='Película' THEN pq.score/10.0 ELSE pqa.score/10.0 END pikoquality,COALESCE(sr.official_seasons,NULLIF(mv.source_status #>> '{series_profile,seasons}','')::int) series_seasons,COALESCE(sr.official_episodes,NULLIF(mv.source_status #>> '{series_profile,episodes}','')::int) series_episodes,COALESCE(mv.source_status #>> '{series_profile,spain,streaming}','false')='true' spain_streaming,COALESCE((SELECT array_agg(g.name_es ORDER BY g.name_es) FROM movie_genres_canonical mgc JOIN genres g ON g.id=mgc.genre_id WHERE mgc.imdb_id=c.imdb_id),c.genres,'{}'::text[]) genres ${baseSql} ${structureJoin} ${priorityJoin} WHERE ${where} ORDER BY ${order} LIMIT ${CATALOG_V4_PAGE_SIZE} OFFSET ${offset}`,params);
  return{state:{...p,page},rows,summary:summary||{total:0,in_plex:0,without_plex:0,profile_due:0},pageCount,page};
}
