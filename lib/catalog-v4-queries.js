...
export function parseCatalogV4(raw={}){
  ...
  return{scope,view,sort,dir,plex,q,genres,genreMode,yearFrom,yearTo,page,invalidYearRange};
}
...
function whereSql(p,params){
  ...
  if(p.genres.length){params.push(p.genres);const i=params.length;if(p.genreMode==='all')w.push(`(SELECT count(DISTINCT g.genre) FROM movie_genres_canonical g WHERE g.imdb_id=c.imdb_id AND g.genre=ANY($${i}::text[]))=${p.genres.length}`);else w.push(`EXISTS(SELECT 1 FROM movie_genres_canonical g WHERE g.imdb_id=c.imdb_id AND g.genre=ANY($${i}::text[]))`);}
  return w.join(' AND ');
}
...
export async function getCatalogV4(raw={}){
  ...
  const rows=await sql.query(`SELECT c.imdb_id,c.display_title,c.original_title,c.year,c.type,c.poster_path,c.final_rating,c.effective_status,CASE WHEN c.effective_status<>'in_plex' THEN NULL WHEN c.type='Película' THEN pq.score/10.0 ELSE pqa.score/10.0 END pikoquality,COALESCE((SELECT array_agg(g.genre ORDER BY g.genre) FROM movie_genres_canonical g WHERE g.imdb_id=c.imdb_id),'{}'::text[]) genres ${baseSql} WHERE ${where} ORDER BY ${order} LIMIT ${CATALOG_V4_PAGE_SIZE} OFFSET ${offset}`,params);
  return{state:{...p,page},rows,summary:summary||{total:0,in_plex:0,without_plex:0},pageCount,page};
}

export async function getCatalogV4Genres(){const sql=db();const rows=await sql`SELECT DISTINCT genre FROM movie_genres_canonical WHERE COALESCE(genre,'')<>'' ORDER BY genre`;return rows.map(r=>r.genre)}
