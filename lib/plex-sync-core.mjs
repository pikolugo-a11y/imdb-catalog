...
export async function discoverPlexUrlCore(token,baseUrl=''){
  const fixed=String(baseUrl||'').replace(/\/$/,'');if(fixed)return fixed;
  const r=await fetch('https://plex.tv/api/resources?includeHttps=1',{headers:{'X-Plex-Token':token,'X-Plex-Client-Identifier':CLIENT},cache:'no-store',signal:AbortSignal.timeout(15000)});
  if(!r.ok){const e=new Error(`No se pudo descubrir Plex (${r.status})`);e.status=r.status;throw e}
  ...
  throw new Error('Plex no publica una conexión remota accesible. Configura PLEX_URL.');
}
...
async function upsertSectionFilenames(sql,items){
...
async function syncSection(sql,base,token,section,{shouldStop=async()=>false}={}){
  ...
  try{
    if(await shouldStop())throw Object.assign(new Error('Plex sync pausado por kill switch'),{code:'PAUSED'});
    const body=await pget(base,token,`/library/sections/${section.key}/all?includeGuids=1&includeMedia=1&X-Plex-Container-Start=0&X-Plex-Container-Size=20000`),items=list(body).filter(x=>x.type===kind);
    ...
    const filenameCount=await upsertSectionFilenames(sql,items);
    const changed=baseRows.filter(x=>x.changed);const enriched=[];
    for(let pos=0;pos<changed.length;pos+=8){
      if(await shouldStop())throw Object.assign(new Error('Plex sync pausado por kill switch'),{code:'PAUSED'});
      const chunk=changed.slice(pos,pos+8);const got=await Promise.all(chunk.map(async b=>{const body=await pget(base,token,`/library/metadata/${b.rating_key}?includeGuids=1`);const d=list(body)[0]||{};return{rating_key:b.rating_key,guids:gids(d),media:medias(d)}}));enriched.push(...got)
    }
    ...
    return{section_key:String(section.key),section:section.title,type:kind,total:items.length,new:newCount,changed:changedCount,unchanged:unchangedCount,missing:missing.length,details:changed.length,filenames:filenameCount};
...
export async function probePlexCore({token,baseUrl=''}){
  if(!token)throw Object.assign(new Error('PLEX_TOKEN no está configurado'),{permanent:true,status:401});
  const base=await discoverPlexUrlCore(token,baseUrl),root=await pget(base,token,'/library/sections'),sections=list(root).filter(s=>s.type==='movie'||s.type==='show').map(s=>({key:String(s.key),title:s.title||null,type:s.type||null}));
  if(!sections.length)throw Object.assign(new Error('No se encontraron bibliotecas de películas/series en Plex'),{permanent:true});
  return{source:'plex',probe:true,sections,section_count:sections.length};
}

export async function syncPlexFastCore({sql,token,baseUrl='',completedSectionKeys=[],onCheckpoint=async()=>{},shouldStop=async()=>false}){
  if(!token)throw Object.assign(new Error('PLEX_TOKEN no está configurado'),{permanent:true,status:401});
  const base=await discoverPlexUrlCore(token,baseUrl),root=await pget(base,token,'/library/sections'),sections=list(root).filter(s=>s.type==='movie'||s.type==='show');
  if(!sections.length)throw new Error('No se encontraron bibliotecas de películas/series en Plex');
  const completed=new Set((completedSectionKeys||[]).map(String)),results=[];
  for(const s of sections){
    if(completed.has(String(s.key)))continue;
    if(await shouldStop())throw Object.assign(new Error('Plex sync pausado por kill switch'),{code:'PAUSED'});
    const result=await syncSection(sql,base,token,s,{shouldStop});results.push(result);completed.add(String(s.key));
    await onCheckpoint({completed_sections:[...completed],last_section:result,partial_results:results});
  }
  if(await shouldStop())throw Object.assign(new Error('Plex sync pausado por kill switch'),{code:'PAUSED'});
  const seriesIdentityChanges=await reconcileSeriesReferencesFromPlexCore(sql);
  await rebuildCatalogStatus(sql);
  return{source:'plex',sync:true,completed_sections:[...completed],results,total:results.reduce((a,x)=>a+x.total,0),new:results.reduce((a,x)=>a+x.new,0),changed:results.reduce((a,x)=>a+x.changed,0),missing:results.reduce((a,x)=>a+x.missing,0),filenames:results.reduce((a,x)=>a+(x.filenames||0),0),seriesIdentityChanges};
}
