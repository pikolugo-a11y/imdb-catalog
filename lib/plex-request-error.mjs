...
export async function fetchPlexJsonWithRetry({
  ...
  for(let attempt=0;attempt<attempts.length;attempt++){
    ...
    try{
      if(typeof trace?.heartbeat==='function')heartbeatTimer=setInterval(()=>{Promise.resolve(trace.heartbeat({step:'plex_request_wait',path,attempt:attempt+1})).catch(()=>{})},defaultHeartbeatIntervalMs);
