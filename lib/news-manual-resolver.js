...
export async function resolveManualNewsCandidate(imdbId,{trace=null}={}){
  let omdb=null,omdbError=null;
  try{
    if(trace?.externalCall)await trace.externalCall(1);
    omdb=await omdbMinimumByImdb(imdbId);
  }catch(e){omdbError=e?.message||String(e)}
