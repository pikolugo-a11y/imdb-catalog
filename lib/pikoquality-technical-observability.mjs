...
export async function addTechnicalCaptureCounters(sql,runId,{claimed=0,ok=0,failed=0,scored=0}={}){
  if(!runId)return;
  await sql`UPDATE process_runs SET context=COALESCE(context,'{}'::jsonb)||jsonb_build_object(
    'capture_claimed',COALESCE((context->>'capture_claimed')::int,0)+${Number(claimed)||0},
    'capture_ok',COALESCE((context->>'capture_ok')::int,0)+${Number(ok)||0},
    'capture_failed',COALESCE((context->>'capture_failed')::int,0)+${Number(failed)||0},
    'pikoquality_scored',COALESCE((context->>'pikoquality_scored')::int,0)+${Number(scored)||0}
  ),updated_at=now() WHERE run_id=${runId}::uuid AND process_code=${PROCESS_CODE}`;
}
