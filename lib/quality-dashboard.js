...
export async function getQualityDashboard(){
  ...
  const [movie,series,identity,plexIdentity,validation,pq,runs]=await Promise.all([
    ...
    sql`SELECT count(*) FILTER(WHERE m.imdb_id~'^tt[0-9]+$' AND m.tmdb_id IS NOT NULL AND m.fa_id IS NOT NULL)::int total_three_ids,
      ...
      WHERE ex.imdb_id IS NULL AND m.imdb_id~'^tt[0-9]+$' AND m.tmdb_id IS NOT NULL AND m.fa_id IS NOT NULL`,
