...
export async function getQualityDashboard(){
  const sql=db();
  const [movie,series,identity,plexIdentity,pq,runs]=await Promise.all([
    ...
    sql`SELECT DISTINCT ON(job_type) job_type,status,started_at,finished_at,processed_count,updated_count,error_count,summary
      FROM pipeline_runs
      WHERE job_type IN('movie_quality_analysis','series_v2_refresh','identity_scan')
  ...
  return {totalAttention,movie:m,series:s,identity:{...i,missing_plex_imdb:pi.total||0,affected:identityAffected},pikoquality:{...q,progress},runs:{movies:latest.movie_quality_analysis||null,series:latest.series_v2_refresh||null,identity:latest.identity_scan||null}};
}
