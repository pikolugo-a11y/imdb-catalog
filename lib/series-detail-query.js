...
export async function getSeriesDetailSummary(ratingKey){
  ...
  if(!s)return null;
  const[seasonCounts,totals,extras,combined,quality,runs,overrides]=await Promise.all([
  ...
  const t=totals[0]||{total:0,present:0,missing:0,unknown:0,noes:0};
  const unresolvedExtras=extras.filter(e=>!['special','not_needed'].includes(String(e.decision||'')));
  ...
  return{...s,seasonCounts,totals:{total:num(t.total),present:num(t.present),missing:num(t.missing),unknown:num(t.unknown),noes:num(t.noes)},extras,unresolvedExtras,resolvedExtras:extras.filter(e=>['special','not_needed'].includes(String(e.decision||''))),combined,quality:quality[0]||null,runs,overrides,classification};
}
