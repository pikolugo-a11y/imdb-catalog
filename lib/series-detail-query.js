...
export async function getSeriesDetailSummary(ratingKey){
  ...
  const[seasonCounts,totals,extrasRaw,combined,quality,runs,overrides]=await Promise.all([
    ...
    sql`SELECT d.season_number,d.episode_number,d.status,d.confidence,d.reason,d.covered_by_rating_key,d.expected_name,d.expected_runtime_minutes,d.actual_duration_minutes,d.diagnosed_at,o.decision override_decision,o.note override_note,o.updated_at override_updated_at FROM series_diagnostics d LEFT JOIN series_episode_overrides o ON o.show_rating_key=d.show_rating_key AND o.season_number=d.season_number AND o.episode_number=d.episode_number AND o.decision='manual_present' WHERE d.show_rating_key=${ratingKey} AND d.status='covered_combined' ORDER BY d.season_number,d.episode_number LIMIT 200`,
...
export async function getSeriesEpisodeScope(ratingKey,{season='all',status='all',coverage='all',page=1,pageSize=SERIES_EPISODE_PAGE_SIZE}={}){
  ...
  const[count]=await sql`SELECT count(*)::int total FROM series_episode_effective_status e WHERE e.show_rating_key=${ratingKey} AND (${sn}=0 OR e.season_number=${sn}) AND (${st}='all' OR e.effective_status=${st}) AND (${cv}='all' OR (${cv}='present' AND e.effective_status='present') OR (${cv}='absent' AND e.effective_status<>'present'))`;
