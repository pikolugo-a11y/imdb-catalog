...
export async function rebuildSeriesQualityReadModelForRatingKey(sql,ratingKey){
  ...
  await sql`WITH ec AS (
    SELECT show_rating_key,
      ...
      count(*) FILTER(WHERE effective_status='missing_actionable' AND (air_date IS NOT NULL AND air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int actionable_missing,
      count(*) FILTER(WHERE effective_status='availability_unknown' AND (air_date IS NOT NULL AND air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int availability_unknown,
      count(*) FILTER(WHERE effective_status='not_available_es' AND (air_date IS NOT NULL AND air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int not_available_es
    ...
    SELECT p.grandparent_rating_key show_rating_key,
      ...
      count(*) FILTER(WHERE ref.show_rating_key IS NULL AND NOT EXISTS(
        ...
        o.decision IN ('special','not_needed')
        AND COALESCE(o.note,'') LIKE '%"ser005":1%'
        AND substring(o.note from '"plex_rating_key":"([^"]*)"')=p.rating_key
        AND substring(o.note from '"plex_fingerprint":"([^"]*)"')=COALESCE(p.fingerprint,'')
      ))::int unmapped
    FROM plex_items p
    ...
    LEFT JOIN series_episode_overrides o
      ON o.show_rating_key=p.grandparent_rating_key
      AND o.season_number=p.parent_index
      AND o.episode_number=p.item_index
