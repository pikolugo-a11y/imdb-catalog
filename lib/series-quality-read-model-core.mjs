...
export async function rebuildSeriesQualityReadModelForRatingKey(sql,ratingKey){
  ...
  await sql`WITH ec AS (
    SELECT show_rating_key,
      ...
      count(*) FILTER(WHERE effective_status='missing_actionable' AND (air_date IS NULL OR air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int actionable_missing,
      count(*) FILTER(WHERE effective_status='availability_unknown' AND (air_date IS NULL OR air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int availability_unknown,
      count(*) FILTER(WHERE effective_status='not_available_es' AND (air_date IS NULL OR air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int not_available_es
