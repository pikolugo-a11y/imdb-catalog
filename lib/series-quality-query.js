...
export async function getSeriesFilenameFormatWarnings({page=1,pageSize=50}={}){
  ...
  const[countRow]=await sql`WITH bad AS (
    ...
    WHERE p.active
      ...
      AND f.file_path !~ '^.+ - [0-9]{1,2}x[0-9]{1,3} - .+\\.[^.]+  const sql=db(),safePage=Math.max(1,Number(page)||1),safeSize=Math.max(1,Math.min(Number(pageSize)||50,100)),offset=(safePage-1)*safeSize;
...
export async function getSeriesQualityV3(filters={}){
  ...
  const rows=await sql`WITH file_stats AS (
    SELECT p.grandparent_rating_key show_rating_key,
           ...
           count(*) FILTER(WHERE f.file_path IS NOT NULL AND f.file_path !~ '^.+ - [0-9]{1,2}x[0-9]{1,3} - .+\\.[^.]+  const sql=db(),safePage=Math.max(1,Number(page)||1),safeSize=Math.max(1,Math.min(Number(pageSize)||50,100)),offset=(safePage-1)*safeSize;
  const[countRow]=await sql`SELECT count(*)::int total FROM (
    SELECT p.rating_key
    FROM plex_items p
    JOIN plex_files f ON f.rating_key=p.rating_key
