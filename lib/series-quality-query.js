...
async function getClassifiedSeries(sql=db()){
  const raw=await sql`SELECT rm.*,COALESCE(tm.imdb_id,rm.imdb_id) effective_imdb_id,(qo.decision='manual_complete') manual_complete,COALESCE(av.availability_due,0)::int availability_due,COALESCE(ep.episode_present,0)::int episode_present,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_missing_total,0) END::int episode_missing_total,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_actionable_now,0) END::int episode_actionable_now,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_pending_unknown,0) END::int episode_pending_unknown,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_pending_noes,0) END::int episode_pending_noes,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_pending_spain,0) END::int episode_pending_spain,CASE WHEN qo.decision='manual_complete' THEN 0 ELSE COALESCE(ep.episode_pending_premiere,0) END::int episode_pending_premiere FROM series_quality_read_model rm LEFT JOIN series_quality_overrides qo ON qo.show_rating_key=rm.show_rating_key LEFT JOIN plex_show_effective_ids pe ON pe.rating_key=rm.show_rating_key LEFT JOIN movies tm ON tm.type IN('Serie','Miniserie') AND tm.source_status->>'identity_mode'='tmdb_only' AND tm.tmdb_id=pe.tmdb_id LEFT JOIN (SELECT e.show_rating_key,count(*) FILTER(WHERE e.effective_status='availability_unknown' AND (e.air_date IS NULL OR e.air_date<=CURRENT_DATE) AND (a.checked_at IS NULL OR a.checked_at<=now()-(${SER004_RECHECK_DAYS} * interval '1 day')))::int availability_due FROM series_episode_effective_status e LEFT JOIN series_season_availability a ON a.show_rating_key=e.show_rating_key AND a.season_number=e.season_number AND a.country_code='ES' GROUP BY e.show_rating_key) av ON av.show_rating_key=rm.show_rating_key LEFT JOIN (SELECT e.show_rating_key,count(*) FILTER(WHERE e.effective_status='present')::int episode_present,count(*) FILTER(WHERE e.plex_diagnostic_status='missing')::int episode_missing_total,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NULL OR e.air_date<=CURRENT_DATE) AND e.effective_status='missing_actionable')::int episode_actionable_now,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NULL OR e.air_date<=CURRENT_DATE) AND e.effective_status='availability_unknown')::int episode_pending_unknown,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NULL OR e.air_date<=CURRENT_DATE) AND e.effective_status='not_available_es')::int episode_pending_noes,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NULL OR e.air_date<=CURRENT_DATE) AND e.effective_status IN('availability_unknown','not_available_es'))::int episode_pending_spain,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND e.air_date>CURRENT_DATE)::int episode_pending_premiere FROM series_episode_effective_status e GROUP BY e.show_rating_key) ep ON ep.show_rating_key=rm.show_rating_key ORDER BY rm.title`;
  return raw.map(r=>{const corrected={...r,availability_due:r.manual_complete?0:n(r.availability_due),actionable_missing:n(r.episode_actionable_now),availability_unknown:n(r.episode_pending_unknown),not_available_es:n(r.episode_pending_noes),unmapped_plex_episodes:r.manual_complete?0:n(r.unmapped_plex_episodes)};return{...corrected,...classifySeries(corrected)}});
}
...
export async function getSeriesQualityV3(filters={}){
  const sql=db(),q=String(filters.q||'').trim().toLowerCase(),state=allowed.has(String(filters.state))?String(filters.state):'attention',episodeState=episodeAllowed.has(String(filters.episode))?String(filters.episode):'all',page=Math.max(1,Number(filters.page)||1),pageSize=50;
  ...
  filtered.sort((a,b)=>(episodeField?(n(b[episodeField])-n(a[episodeField])):0)||(priority[a.primaryState]-priority[b.primaryState])||(n(b.episode_actionable_now)-n(a.episode_actionable_now))||String(a.title).localeCompare(String(b.title),'es'));
  const total=filtered.length,offset=(page-1)*pageSize,rows=filtered.slice(offset,offset+pageSize);
  const [lastRun]=await sql`SELECT process_code run_type,technical_status status,started_at,finished_at,metrics FROM process_runs WHERE process_code IN('PROC-SER-001','PROC-SER-002','PROC-SER-003','PROC-SER-004','PROC-SER-005','PROC-SER-006','PROC-SER-008') ORDER BY requested_at DESC LIMIT 1`;
  return{rows,counts,total,page,pageSize,pages:Math.max(1,Math.ceil(total/pageSize)),episodeState,lastRun:lastRun||null};
}
