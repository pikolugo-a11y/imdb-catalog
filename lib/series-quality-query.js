...
async function getClassifiedSeries(sql=db(),q=''){
  ...
  const raw=await sql`SELECT rm.*,COALESCE(tm.imdb_id,rm.imdb_id) effective_imdb_id,COALESCE(av.availability_due,0)::int availability_due FROM series_quality_read_model rm LEFT JOIN plex_show_effective_ids pe ON pe.rating_key=rm.show_rating_key LEFT JOIN movies tm ON tm.type IN('Serie','Miniserie') AND tm.source_status->>'identity_mode'='tmdb_only' AND tm.tmdb_id=pe.tmdb_id LEFT JOIN (SELECT e.show_rating_key,count(*) FILTER(WHERE e.effective_status='availability_unknown' AND (a.checked_at IS NULL OR a.checked_at<=now()-(${SER004_RECHECK_DAYS} * interval '1 day')))::int availability_due FROM series_episode_effective_status e LEFT JOIN series_season_availability a ON a.show_rating_key=e.show_rating_key AND a.season_number=e.season_number AND a.country_code='ES' GROUP BY e.show_rating_key) av ON av.show_rating_key=rm.show_rating_key WHERE (${needle}='' OR lower(COALESCE(rm.title,'')) LIKE ${'%'+needle+'%'}) ORDER BY rm.title`;
...
export async function getSeriesQualityV3(filters={}){
  ...
  const filtered=state==='all'?classified:state==='attention'?classified.filter(r=>attentionStates.has(r.primaryState)):state==='tracking'?classified.filter(r=>trackingStates.has(r.primaryState)):classified.filter(r=>r.primaryState===state);
  const priority={missing:0,unmapped:1,pre_quality:2,plex_sync:3,tmdb_refresh:4,unknown:5,uptodate:6};
  filtered.sort((a,b)=>(priority[a.primaryState]-priority[b.primaryState])||(Number(b.actionable_missing)-Number(a.actionable_missing))||String(a.title).localeCompare(String(b.title),'es'));
