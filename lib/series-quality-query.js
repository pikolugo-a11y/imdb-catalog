...
const episodeAllowed=new Set(['all','missing','actionable','spain','noes','premiere']);
...
function buildCounts(classified){
  ...
  const episodes={physical:0,present:0,physicalMissing:0,missing:0,actionable:0,spain:0,noes:0,noesSeries:0,premiere:0};
  for(const r of classified){
    ...
    episodes.physicalMissing+=n(r.episode_missing_physical_total);
    ...
    episodes.noes+=n(r.episode_review_noes);
    if(n(r.episode_review_noes)>0)episodes.noesSeries+=1;
...
async function getClassifiedSeries(sql=db()){
  const raw=await sql`SELECT rm.*,COALESCE(tm.imdb_id,rm.imdb_id) effective_imdb_id,COALESCE(av.availability_due,0)::int availability_due,COALESCE(ep.episode_present,0)::int episode_present,COALESCE(ep.episode_missing_physical_total,0)::int episode_missing_physical_total,COALESCE(ep.episode_missing_total,0)::int episode_missing_total,COALESCE(ep.episode_actionable_now,0)::int episode_actionable_now,COALESCE(ep.episode_pending_unknown,0)::int episode_pending_unknown,COALESCE(ep.episode_pending_noes,0)::int episode_pending_noes,COALESCE(ep.episode_pending_spain,0)::int episode_pending_spain,COALESCE(ep.episode_pending_premiere,0)::int episode_pending_premiere FROM series_quality_read_model rm LEFT JOIN plex_show_effective_ids pe ON pe.rating_key=rm.show_rating_key LEFT JOIN movies tm ON tm.type IN('Serie','Miniserie') AND tm.source_status->>'identity_mode'='tmdb_only' AND tm.tmdb_id=pe.tmdb_id LEFT JOIN (SELECT e.show_rating_key,count(*) FILTER(WHERE e.effective_status='availability_unknown' AND (e.air_date IS NOT NULL AND e.air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')) AND (a.checked_at IS NULL OR a.checked_at<=now()-(${SER004_RECHECK_DAYS} * interval '1 day')))::int availability_due FROM series_episode_effective_status e LEFT JOIN series_season_availability a ON a.show_rating_key=e.show_rating_key AND a.season_number=e.season_number AND a.country_code='ES' GROUP BY e.show_rating_key) av ON av.show_rating_key=rm.show_rating_key LEFT JOIN (SELECT e.show_rating_key,count(*) FILTER(WHERE e.effective_status='present')::int episode_present,count(*) FILTER(WHERE e.plex_diagnostic_status='missing')::int episode_missing_physical_total,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND e.effective_status<>'present')::int episode_missing_total,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NOT NULL AND e.air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')) AND e.effective_status='missing_actionable')::int episode_actionable_now,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NOT NULL AND e.air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')) AND e.effective_status='availability_unknown')::int episode_pending_unknown,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NOT NULL AND e.air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')) AND e.effective_status='not_available_es')::int episode_pending_noes,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND (e.air_date IS NOT NULL AND e.air_date<=CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')) AND e.effective_status='availability_unknown')::int episode_pending_spain,count(*) FILTER(WHERE e.plex_diagnostic_status='missing' AND e.effective_status<>'present' AND (e.air_date IS NULL OR e.air_date>CURRENT_DATE-(${SERIES_EPISODE_GRACE_DAYS} * interval '1 day')))::int episode_pending_premiere FROM series_episode_effective_status e GROUP BY e.show_rating_key) ep ON ep.show_rating_key=rm.show_rating_key ORDER BY rm.title`;
  const manualNo=await sql`SELECT a.show_rating_key,count(DISTINCT a.season_number)::int review_noes_seasons,count(re.episode_number)::int review_noes_episodes FROM series_season_availability a LEFT JOIN series_reference_episodes re ON re.show_rating_key=a.show_rating_key AND re.season_number=a.season_number WHERE a.manual_override=true AND a.status='EXCEPTION_NOT_AVAILABLE' GROUP BY a.show_rating_key`;
  const manualNoByKey=new Map(manualNo.map(r=>[String(r.show_rating_key),r]));
  return raw.map(r=>{const review=manualNoByKey.get(String(r.show_rating_key));const corrected={...r,actionable_missing:n(r.episode_actionable_now),availability_unknown:n(r.episode_pending_unknown),not_available_es:n(r.episode_pending_noes),episode_review_noes:n(review?.review_noes_episodes),review_noes_seasons:n(review?.review_noes_seasons)};return{...corrected,...classifySeries(corrected)}});
}
...
export async function getSeriesQualityV3(filters={}){
  ...
  const episodeField={missing:'episode_missing_total',actionable:'episode_actionable_now',spain:'episode_pending_spain',noes:'episode_review_noes',premiere:'episode_pending_premiere'}[episodeState]||null;
