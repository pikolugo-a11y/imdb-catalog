...
export function pikoScoreV3FreshnessDays(row){const a=ageYears(row);return a<0.25?14:a<1?30:a<3?90:a<10?180:365;}
...
export function isPikoScoreV3Due(row){
  ...
  if(row.ratings_refreshed_at){const refreshed=new Date(row.ratings_refreshed_at);if(Number.isNaN(refreshed.getTime()))return true;if(refreshed>calc)return true;if(Date.now()-refreshed.getTime()>=pikoScoreV3FreshnessDays(row)*86400000)return true;}else return true;
  return false;
}
...
export async function calculateAndSavePikoScoreV3ForTitle(imdbId){
  ...
  `;
  await audit('pikoscore','title',imdbId,'calculated_v3',{version:PIKOSCORE_V3_VERSION,score:result.score,confidence:result.confidence,market:result.market,market_vote_scale:result.marketVoteScale,source_count:result.sourceCount,family_count:result.familyCount,family_coverage:result.familyCoverage,contributions:result.contributions,ratings_refreshed_at:result.ratingsRefreshedAt,previous:result.previous});
  return result;
}
