...
/**
 * Evaluador experimental y read-only. No escribe final_rating ni sustituye
 * todavía PikoScore 2.x. Sirve para comparar 2.x vs 3.x con datos reales.
 */
export async function evaluatePikoScoreV3ForTitle(imdbId){
  ...
  return{
    imdbId,
    title:row.title_es||row.original_title||imdbId,
    experimental:true,
    version:PIKOSCORE_V3_VERSION,
    legacy:{score:row.final_rating==null?null:Number(row.final_rating),version:row.pikoscore_version||null,confidence:row.pikoscore_confidence==null?null:Number(row.pikoscore_confidence)},
    ...result,
  };
}
