...
export async function getPikoQualityState(){
  ...
  const physical=await sql`
    SELECT p.rating_key,p.item_type,p.fingerprint,p.plex_title,p.plex_year,p.parent_index,p.item_index,m.resolution,m.video_codec,
      COALESCE(mx.external_id,sr.imdb_id) lifecycle_imdb_id,
  ...
  const lifecycle=await getLifecycleForIds([...new Set(physical.map(r=>r.lifecycle_imdb_id).filter(Boolean))]),allowed=new Set(['TECH_PENDING','TECH_REVIEW','COMPLETE']),rows=physical.filter(r=>allowed.has(lifecycle.get(r.lifecycle_imdb_id)?.state));
