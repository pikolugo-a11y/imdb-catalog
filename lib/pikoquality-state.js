    ...
    sql.query(`
      WITH physical AS (
        SELECT p.rating_key,p.item_type,p.fingerprint,p.plex_title,p.plex_year,p.parent_index,p.item_index,
          ...
          COALESCE(q.status='evaluated' AND q.formula_version=$1 AND q.source_fingerprint IS NOT DISTINCT FROM p.fingerprint,false) AS valid
        FROM plex_items p
...
export async function getPikoQualityPendingPage({page=1,pageSize=25,query='',resolution='',codec='',status='',sort='priority'}={}){
  ...
  if(q)add(`%${q}%`,i=>`(m.title_es ILIKE $${i} OR m.title ILIKE $${i} OR m.original_title ILIKE $${i} OR p.plex_title ILIKE $${i} OR cl.imdb_id ILIKE $${i})`);
  if(resolution)add(String(resolution),i=>`pm.resolution=$${i}`);
  if(codec)add(String(codec).toLowerCase(),i=>`lower(pm.video_codec)=$${i}`);
  const escapedVersion=QUALITY_VERSION.replaceAll("'","''");
  if(status==='stale')where.push(`q.rating_key IS NOT NULL AND (q.status='stale' OR q.formula_version IS DISTINCT FROM '${escapedVersion}' OR q.source_fingerprint IS DISTINCT FROM p.fingerprint)`);
  ...
  const priorityCase=`CASE WHEN q.status='error' AND q.source_fingerprint IS NOT DISTINCT FROM p.fingerprint THEN 0 WHEN q.status='stale' OR q.formula_version IS DISTINCT FROM '${escapedVersion}' OR q.source_fingerprint IS DISTINCT FROM p.fingerprint THEN 1 WHEN p.rating_key IS NULL THEN 2 WHEN lower(coalesce(pm.resolution,'')) LIKE '%4k%' AND coalesce(pm.bitrate,0)>0 AND pm.bitrate<5000 THEN 3 ELSE 4 END`;
  const priorityReason=`CASE WHEN q.status='error' AND q.source_fingerprint IS NOT DISTINCT FROM p.fingerprint THEN 'Error técnico' WHEN q.status='stale' OR q.formula_version IS DISTINCT FROM '${escapedVersion}' OR q.source_fingerprint IS DISTINCT FROM p.fingerprint THEN 'PikoQuality desactualizado' WHEN p.rating_key IS NULL THEN 'Sin archivo Plex vigente' WHEN lower(coalesce(pm.resolution,'')) LIKE '%4k%' AND coalesce(pm.bitrate,0)>0 AND pm.bitrate<5000 THEN 'Bitrate bajo para 4K' ELSE 'Pendiente sin evaluar' END`;
  const order={year:`m.year DESC NULLS LAST,m.title_es,cl.imdb_id`,title:`m.title_es ASC NULLS LAST,m.year DESC,cl.imdb_id`,priority:`${priorityCase},m.year DESC NULLS LAST,m.title_es,cl.imdb_id`}[sort]||`m.year DESC NULLS LAST,m.title_es,cl.imdb_id`;
  const joins=`FROM catalog_lifecycle cl JOIN movies m USING(imdb_id) LEFT JOIN plex_catalog_status pcs USING(imdb_id) LEFT JOIN plex_items p ON p.rating_key=pcs.rating_key AND p.active LEFT JOIN LATERAL(SELECT resolution,video_codec,bitrate,audio_codec,audio_channels FROM plex_media z WHERE z.rating_key=p.rating_key ORDER BY media_index LIMIT 1) pm ON true LEFT JOIN LATERAL(SELECT bit_depth FROM plex_streams s WHERE s.rating_key=p.rating_key AND s.stream_type=1 ORDER BY s.bitrate DESC NULLS LAST LIMIT 1) vs ON true LEFT JOIN piko_quality q ON q.rating_key=pcs.rating_key`;
  const base=`${joins} WHERE ${where.join(' AND ')}`;
  const [overallRows,countRows]=await Promise.all([
    sql.query(`SELECT count(*)::int total ${joins} WHERE cl.lifecycle_state='TECH_PENDING' AND m.type='Película'`),
    sql.query(`SELECT count(*)::int total ${base}`,params)
  ]);
  const overallTotal=Number(overallRows[0]?.total||0);
  ...
  const rows=await sql.query(`SELECT cl.imdb_id,m.title_es,m.title,m.original_title,m.year,pcs.rating_key,p.plex_title,p.plex_year,p.fingerprint,pm.resolution,pm.video_codec,pm.bitrate,pm.audio_codec,pm.audio_channels,vs.bit_depth,q.score,q.status,q.formula_version,q.source_fingerprint,${priorityCase} priority_rank,${priorityReason} priority_reason ${base} ORDER BY ${order} LIMIT ${safeSize} OFFSET ${effectiveOffset}`,params);
  const facets=await sql.query(`SELECT array_remove(array_agg(DISTINCT pm.resolution ORDER BY pm.resolution),NULL) resolutions,array_remove(array_agg(DISTINCT lower(pm.video_codec) ORDER BY lower(pm.video_codec)),NULL) codecs ${joins} WHERE cl.lifecycle_state='TECH_PENDING' AND m.type='Película'`);
  return{rows,total,overallTotal,page:effectivePage,pageSize:safeSize,pageCount,resolutions:facets[0]?.resolutions||[],codecs:facets[0]?.codecs||[]};
}
