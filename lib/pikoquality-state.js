...
export async function getPikoQualityPendingPage({page=1,pageSize=25,query='',resolution='',codec='',status='',sort='priority'}={}){
  ...
  const facets=await sql.query(`SELECT array_remove(array_agg(DISTINCT pm.resolution ORDER BY pm.resolution),NULL) resolutions,array_remove(array_agg(DISTINCT lower(pm.video_codec) ORDER BY lower(pm.video_codec)),NULL) codecs ${joins} WHERE p.active AND p.item_type='movie' AND pts.snapshot_status='ready'`);
