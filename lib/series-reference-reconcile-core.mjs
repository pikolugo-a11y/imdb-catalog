export async function reconcileSeriesReferencesFromPlexCore(sql){
  ...
  for(const s of stale){
    await sql.transaction([
      ...
      sql`UPDATE series_reference SET imdb_id=COALESCE(${s.new_imdb},imdb_id),tmdb_id=COALESCE(${s.new_tmdb},tmdb_id),tvdb_id=COALESCE(${s.new_tvdb},tvdb_id),title=COALESCE(${s.plex_title},title),year=COALESCE(${s.plex_year},year),official_seasons=NULL,official_episodes=NULL,avg_runtime_minutes=NULL,reference_source='plex_identity_changed',refreshed_at='1970-01-01'::timestamptz WHERE show_rating_key=${s.show_rating_key}`
    ]);
