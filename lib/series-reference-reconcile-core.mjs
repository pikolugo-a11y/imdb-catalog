export async function reconcileSeriesReferencesFromPlexCore(sql){
  const created=await sql`
    ...
    FROM plex_ids p
    JOIN movies m ON m.type IN('Serie','Miniserie') AND (
      (m.source_status->>'identity_mode'='tmdb_only' AND m.tmdb_id IS NOT NULL AND m.tmdb_id=p.tmdb_id)
      OR (COALESCE(m.source_status->>'identity_mode','normal')<>'tmdb_only' AND m.imdb_id=p.imdb_id)
    )
