...
function nameOf(value){if(!value)return null;if(typeof value==='string')return value.trim()||null;if(typeof value==='object')return String(value.name||value.title||'').trim()||null;return null;}
...
// Metadata auxiliar para CALIDAD → Datos. Es deliberadamente tolerante a variantes del payload
// y no modifica el contrato ni el flujo existente de ratings de MDBList.
export function parseMDBListMetadata(payload){
  const directors=namesOf(firstDefined(payload,['directors','director']));
  const cast=namesOf(firstDefined(payload,['cast','actors']));
  const countries=namesOf(firstDefined(payload,['countries','country']));
  const genres=namesOf(firstDefined(payload,['genres','genre']));
  return {
    ...
    runtime:numeric(firstDefined(payload,['runtime','runtime_minutes','runtimeMinutes','episode_runtime','episodeRuntime'])),
    ...
    release_date:isoDate(firstDefined(payload,['release_date','releaseDate','released','release'])),
    poster:firstDefined(payload,['poster','poster_url','posterUrl']),
    backdrop:firstDefined(payload,['backdrop','backdrop_url','backdropUrl','fanart','fanart_url','fanartUrl']),
