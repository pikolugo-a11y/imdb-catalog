...
export function parseMDBListExtras(payload){const digitalRelease=isoDate(firstDefined(payload,['digital_release','digitalRelease','released_digital','releasedDigital'])),imdbPopularity=finiteNumber(firstDefined(payload,['imdb_popularity','imdbPopularity','imdbpopular','imdb_rank','imdbRank'])),tmdbPopularity=finiteNumber(firstDefined(payload,['tmdb_popularity','tmdbPopularity','tmdbpopular'])),justWatchRank=finiteNumber(firstDefined(payload,['justwatch_rank','justwatchRank','justwatch','justwatch_popularity']));return Object.fromEntries(Object.entries({digital_release:digitalRelease,imdb_popularity:imdbPopularity,tmdb_popularity:tmdbPopularity,justwatch_rank:justWatchRank}).filter(([,value])=>value!==null));}

export async function fetchMDBListRatings({imdbId,mediaType='movie',signal}={}){const apiKey=process.env.MDBLIST_API_KEY;if(!apiKey)throw new Error('MDBLIST_API_KEY no está configurada');const type=['show','series','tv','miniseries'].includes(String(mediaType).toLowerCase())?'show':'movie';const url=`https://api.mdblist.com/imdb/${type}/${encodeURIComponent(imdbId)}?apikey=${encodeURIComponent(apiKey)}`;const response=await fetch(url,{headers:{Accept:'application/json'},signal,cache:'no-store'});if(!response.ok){const body=await response.text().catch(()=>'');const error=new Error(`MDBList HTTP ${response.status}${body?`: ${body.slice(0,300)}`:''}`);error.status=response.status;throw error;}const payload=await response.json();return{payload,ratings:parseMDBListRatings(payload),extras:parseMDBListExtras(payload)};}
