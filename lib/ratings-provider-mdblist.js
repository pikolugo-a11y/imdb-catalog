...
async function fetchMDBListPayload({imdbId,mediaType='movie',signal,auditImdbId}={}){const apiKey=process.env.MDBLIST_API_KEY;if(!apiKey)throw new Error('MDBLIST_API_KEY no está configurada');const type=mdbMediaType(mediaType);const url=`https://api.mdblist.com/imdb/${type}/${encodeURIComponent(imdbId)}?apikey=${encodeURIComponent(apiKey)}`;if(auditImdbId){const {response,body,text}=await loggedJsonFetch({imdbId:auditImdbId,source:'MDBList',url,options:{headers:{Accept:'application/json'},signal}});if(!response.ok){const error=new Error(`MDBList HTTP ${response.status}${text?`: ${text.slice(0,300)}`:''}`);error.status=response.status;throw error;}return body;}const response=await fetch(url,{headers:{Accept:'application/json'},signal,cache:'no-store'});if(!response.ok){const body=await response.text().catch(()=>'');const error=new Error(`MDBList HTTP ${response.status}${body?`: ${body.slice(0,300)}`:''}`);error.status=response.status;throw error;}return response.json();}
...
export async function fetchMDBListMetadata({imdbId,mediaType='movie',signal,auditImdbId}={}){const payload=await fetchMDBListPayload({imdbId,mediaType,signal,auditImdbId});return{payload,metadata:parseMDBListMetadata(payload)};}
