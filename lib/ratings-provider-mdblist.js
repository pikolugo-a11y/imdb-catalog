...
/**
 * Extras pequeños que merece la pena conservar de la MISMA respuesta MDBList.
 * Los alias son deliberadamente tolerantes: MDBList ha usado nombres distintos
 * en listados/integraciones y validaremos los nombres definitivos con la primera
 * respuesta real. Si un campo no existe, simplemente no se persiste.
 */
export function parseMDBListExtras(payload) {
  const digitalRelease = isoDate(firstDefined(payload, [
    'digital_release', 'digitalRelease', 'released_digital', 'releasedDigital',
  ]));
  const imdbPopularity = finiteNumber(firstDefined(payload, [
    'imdb_popularity', 'imdbPopularity', 'imdbpopular', 'imdb_rank', 'imdbRank',
  ]));
  const tmdbPopularity = finiteNumber(firstDefined(payload, [
    'tmdb_popularity', 'tmdbPopularity', 'tmdbpopular',
  ]));
  const justWatchRank = finiteNumber(firstDefined(payload, [
    'justwatch_rank', 'justwatchRank', 'justwatch', 'justwatch_popularity',
  ]));
...
export async function fetchMDBListRatings({ imdbId, mediaType = 'movie', signal } = {}) {
  ...
  return { payload, ratings: parseMDBListRatings(payload), extras: parseMDBListExtras(payload) };
}
