...
async function tmdb(path,{trace,apiGate}={}){
  ...
  try{
    ...
    if(apiGate?.success)await apiGate.success('tmdb');
    return r.json();
  }catch(error){
    if(apiGate?.failure)await apiGate.failure('tmdb',error,{lane:'batch'}).catch(()=>{});
    throw error;
  }finally{
    if(apiGate?.release)await apiGate.release(permit).catch(()=>{});
  }
}
