...
async function tmdb(path,{trace,apiGate,lane='batch'}={}){
  const token=process.env.TMDB_API_TOKEN;
  if(!token)throw new Error('TMDB_API_TOKEN no está configurado');
  const gate=requireApiGate(apiGate,'tmdb');
  const permit=await gate.acquire('tmdb',{lane,owner:'saga-refresh'});
  try{
    ...
    if(!r.ok){
      ...
      await gate.failure('tmdb',error,{lane}).catch(()=>{});
      throw error;
    }
    await gate.success('tmdb');
    return r.json();
  }finally{
    await gate.release(permit).catch(()=>{});
  }
}
...
async function imdbForTmdb(sql,tmdbId,{trace,apiGate,lane}={}){
  ...
  let ext;
  try{ext=await tmdb(`/movie/${tmdbId}/external_ids`,{trace,apiGate,lane});}
  ...
  if(corrected&&trace?.event)await trace.event({eventType:'step_completed',step:'repair_member_identity',message:'Identidad IMDb de miembro de saga corregida desde TMDb',data:{tmdb_movie_id:String(tmdbId),previous_imdb_id:known.imdb_id,canonical_imdb_id:canonical}}).catch(()=>{});
  return{imdbId:canonical,corrected};
}

export async function refreshSagaCollectionCanonical(sql,collectionId,{trace,apiGate,lane='batch'}={}){
  const id=String(collectionId||'').trim();
  if(!/^\d+$/.test(id))throw Object.assign(new Error('TMDb collection ID inválido'),{permanent:true});
  let d;
  try{d=await tmdb(`/collection/${id}?language=es-ES`,{trace,apiGate,lane});}
  catch(error){
  ...
  for(const p of parts){
    pos++;
    const identity=await imdbForTmdb(sql,p.id,{trace,apiGate,lane});
