...
export async function getTechnicalDashboard(sql){
  await ensureTechnicalControl(sql);
  const [control,counts,recent,runs]=await Promise.all([
    ...
    `,
    sql`SELECT * FROM plex_technical_runs ORDER BY started_at DESC LIMIT 8`
  ]);
  const blank=()=>({total:0,ready:0,pending:0,stale:0,error:0,missing:0,processing:0});
  const byType={movie:blank(),episode:blank()};
  for(const row of counts){const t=byType[row.item_type]||blank();const key=String(row.snapshot_status||'missing');t.total+=Number(row.count)||0;if(key in t)t[key]+=Number(row.count)||0;byType[row.item_type]=t}
  const total=blank();for(const t of Object.values(byType))for(const k of Object.keys(total))total[k]+=t[k]||0;
  const r=recent[0]||{};const perMinute5=Math.round(((Number(r.ready_5m)||0)/5)*10)/10;const perMinute15=Math.round(((Number(r.ready_15m)||0)/15)*10)/10;
  const heartbeatAgeMs=control?.heartbeat_at?Date.now()-new Date(control.heartbeat_at).getTime():null;
  return{control,byType,total,velocity:{perMinute5,perMinute15,lastCaptureAt:r.last_capture_at||null},workerOnline:heartbeatAgeMs!=null&&heartbeatAgeMs<90000,currentRun:runs.find(x=>x.status==='running')||runs[0]||null,runs};
}
