...
async function dispatchDue(sql,{includeDelayed=true}={}){
...
export async function runActivityDispatchTick(){
  const sql=db();await reconcileDispatched(sql);
  const dispatched=await dispatchDue(sql,{includeDelayed:false});
  return{demand:[],forecast:[],dispatched,safeProcesses:SAFE_AUTOMATIC_PROCESS_CODES,mode:'dispatch'};
}
...
export async function runActivityPlanner(){
  ...
  return{demand,forecast,dispatched,safeProcesses:SAFE_AUTOMATIC_PROCESS_CODES,mode:'full'};
}