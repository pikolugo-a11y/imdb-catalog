...
async function chooseSlot(sql,now,windowDays,load){
...
async function replanDelayed(sql){
  ...
  for(const row of rows){const policy=POLICIES[row.process_code];if(!policy)continue;if(row.latest_at&&new Date(row.latest_at).getTime()<=Date.now()){await sql.query(`UPDATE process_plans SET status='expired',updated_at=now() WHERE plan_id=$1`,[row.plan_id]);continue;}const next=await chooseSlot(sql,Date.now(),Math.max(1,policy.windowDays),Number(row.estimated_load||0));if(row.latest_at&&next>new Date(row.latest_at))continue;await sql.query(`UPDATE process_plans SET status='planned',planned_at=$2,updated_at=now(),metadata=metadata||jsonb_build_object('auto_replanned',true,'auto_replanned_at',now()) WHERE plan_id=$1`,[row.plan_id,next]);}
}
...
async function planPending(sql){
  ...
  for(const row of rows){const policy=POLICIES[row.process_code];if(!policy)continue;const now=Math.max(Date.now(),row.earliest_at?new Date(row.earliest_at).getTime():Date.now()),slot=await chooseSlot(sql,now,policy.windowDays,Number(row.estimated_load||0));if(row.latest_at&&slot>new Date(row.latest_at))continue;await sql.query(`UPDATE process_plans SET status='planned',planned_at=$2,updated_at=now(),metadata=metadata||jsonb_build_object('auto_planned',true,'auto_planned_at',now()) WHERE plan_id=$1`,[row.plan_id,slot]);}
}
async function dispatchDue(sql){
  const rows=await sql.query(`SELECT * FROM process_plans WHERE status IN('planned','delayed') AND planned_at<=now() AND (latest_at IS NULL OR latest_at>=now()) ORDER BY deliberate_peak DESC,priority DESC,planned_at ASC LIMIT 12`),results=[];
  for(const row of rows){const policy=POLICIES[row.process_code];if(!policy)continue;try{const result=await policy.dispatch(Number(row.planned_volume));const runId=result?.run?.run_id||result?.runId||null;if(runId){await sql.query(`UPDATE process_plans SET status='dispatched',dispatch_run_id=$2::uuid,updated_at=now() WHERE plan_id=$1`,[row.plan_id,runId]);results.push({planId:row.plan_id,processCode:row.process_code,runId});}else{await sql.query(`UPDATE process_plans SET status='completed',updated_at=now(),metadata=metadata||jsonb_build_object('dispatch_empty',true) WHERE plan_id=$1`,[row.plan_id]);}}catch(error){await sql.query(`UPDATE process_plans SET status='delayed',updated_at=now(),metadata=metadata||jsonb_build_object('last_dispatch_error',$2,'last_dispatch_error_at',now()) WHERE plan_id=$1`,[row.plan_id,String(error?.message||error).slice(0,300)]);results.push({planId:row.plan_id,processCode:row.process_code,error:String(error?.message||error)});}}
  return results;
}
