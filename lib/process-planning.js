...
export async function runActivityDispatchTick(){
  const sql=db(),settings=await loadAutomationSettings(sql);await reconcileDispatched(sql);
  const dispatched=await dispatchDue(sql,settings,{includeDelayed:false});
  return{demand:[],forecast:[],dispatched,safeProcesses:SAFE_AUTOMATIC_PROCESS_CODES.filter(code=>settings.processes[code]?.enabled),mode:'dispatch'};
}

export async function runActivityPlanner(){
  const sql=db(),settings=await loadAutomationSettings(sql),activeCodes=SAFE_AUTOMATIC_PROCESS_CODES.filter(code=>settings.processes[code]?.enabled);await reconcileDispatched(sql);await replanDelayed(sql,settings);
  ...
  return{demand,forecast,dispatched,safeProcesses:activeCodes,mode:'full'};
}