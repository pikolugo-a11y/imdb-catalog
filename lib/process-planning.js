...
import {selectPikoRelevanceDueEligibleIds,startPikoRelevanceBatch} from './pikorelevancia-batch';
...
import {loadAutomationSettings,automationProfile} from './activity-automation-settings';
import {wakeWorkerPool} from './worker-wake-client';

const DAY=86400000,HOUR=3600000,MADRID_TZ='Europe/Madrid';
export const PLANNER_HORIZON_DAYS=30;
export const SAFE_AUTOMATIC_PROCESS_CODES=['PROC-MOV-001','PROC-SER-002','PROC-SER-003','PROC-SER-004','PROC-DATA-002','PROC-PER-001','PROC-PQ-001','PROC-REL-001'];
...
const POLICIES={
  ...
  'PROC-REL-001':{domain:'calidad',title:'Actualización de PikoRelevancia',block:10,windowDays:7,fallbackSeconds:35,eligible:sql=>selectPikoRelevanceDueEligibleIds(sql),dispatch:n=>startPikoRelevanceBatch({limit:n,triggerSource:'quality_scheduler'})},
...
function madridKey(value){return new Intl.DateTimeFormat('en-CA',{timeZone:MADRID_TZ,year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(value));}
function addMadridDays(key,days){const d=new Date(`${key}T12:00:00Z`);d.setUTCDate(d.getUTCDate()+days);return madridKey(d);}
function madridLocalToUtc(key,hour){
  const [year,month,day]=key.split('-').map(Number),targetEpoch=Date.UTC(year,month-1,day,hour,0),formatter=new Intl.DateTimeFormat('en-GB',{timeZone:MADRID_TZ,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});let candidate=targetEpoch;
...
function slotCandidates(now,days,hours){
  const out=[],startKey=madridKey(now);for(let d=0;d<days;d++){const key=addMadridDays(startKey,d);for(const h of hours){const x=madridLocalToUtc(key,h);if(x.getTime()>now+15*60000)out.push(x);}}return out.sort((a,b)=>a-b);
}
...
async function chooseSlot(sql,now,windowDays,hours){
  const candidates=slotCandidates(now,windowDays,hours);if(!candidates.length)return new Date(now+HOUR);
...
async function replanDelayed(sql,settings){
  ...
  for(const row of rows){const policy=POLICIES[row.process_code],config=settings.processes[row.process_code];if(!policy||!config?.enabled)continue;if(row.latest_at&&new Date(row.latest_at).getTime()<=Date.now()){await sql.query(`UPDATE process_plans SET status='expired',updated_at=now() WHERE plan_id=$1`,[row.plan_id]);continue;}const next=await chooseSlot(sql,Date.now(),Math.max(1,policy.windowDays),automationProfile(config.profile).hours);if(row.latest_at&&next>new Date(row.latest_at))continue;await sql.query(`UPDATE process_plans SET status='planned',planned_at=$2,updated_at=now(),metadata=metadata||jsonb_build_object('auto_replanned',true,'auto_replanned_at',now()) WHERE plan_id=$1`,[row.plan_id,next]);}
}
...
async function futureBuckets(sql,code){
  ...
  if(code==='PROC-REL-001')return sql.query(`SELECT (date_trunc('day',a.next_review_at)+interval '1 day') due_at,count(*)::int volume FROM series_relevance_assessments a JOIN catalog_read_model c ON c.imdb_id=a.imdb_id LEFT JOIN catalog_exclusions ex ON ex.imdb_id=a.imdb_id WHERE a.formula_version=$1 AND c.type IN ('Serie','Miniserie') AND ex.imdb_id IS NULL AND a.next_review_at IS NOT NULL AND a.next_review_at>now() AND a.next_review_at<=now()+interval '29 days' GROUP BY 1 ORDER BY 1`,[PIKORELEVANCE_VERSION]);
...
async function planPending(sql,settings){
  ...
  for(const row of rows){const policy=POLICIES[row.process_code],config=settings.processes[row.process_code];if(!policy||!config?.enabled)continue;const now=Math.max(Date.now(),row.earliest_at?new Date(row.earliest_at).getTime():Date.now()),slot=await chooseSlot(sql,now,policy.windowDays,automationProfile(config.profile).hours);if(row.latest_at&&slot>new Date(row.latest_at))continue;await sql.query(`UPDATE process_plans SET status='planned',planned_at=$2,updated_at=now(),metadata=metadata||jsonb_build_object('auto_planned',true,'auto_planned_at',now()) WHERE plan_id=$1`,[row.plan_id,slot]);}
}
async function dispatchDue(sql,settings,{includeDelayed=true}={}){
  const statusFilter=includeDelayed?`status IN('planned','delayed')`:`status='planned'`;
  const rows=await sql.query(`SELECT * FROM process_plans WHERE ${statusFilter} AND planned_at<=now() AND (latest_at IS NULL OR latest_at>=now()) ORDER BY deliberate_peak DESC,priority DESC,planned_at ASC LIMIT 200`),groups=new Map(),results=[];
  for(const row of rows){if(!POLICIES[row.process_code]||!settings.processes[row.process_code]?.enabled)continue;const group=groups.get(row.process_code)||{rows:[],volume:0};group.rows.push(row);group.volume+=Number(row.planned_volume||0);groups.set(row.process_code,group);}
...
async function wakeActiveWorkersForRecovery(sql){
  const pools=await sql.query(`SELECT DISTINCT c.worker_pool FROM batch_run_control c JOIN process_runs pr ON pr.run_id=c.run_id WHERE c.closed_at IS NULL AND c.desired_state='running' AND pr.technical_status='running'`);
  const results=[];
  for(const row of pools){
    const pool=String(row.worker_pool||'');
    if(!pool)continue;
    results.push(await wakeWorkerPool(pool,{reason:'planner_recovery'}).catch(error=>({ok:false,pool,error:String(error?.message||error)})));
  }
  const [technical]=await sql.query(`SELECT requested_state,armed FROM plex_technical_control WHERE id=1 LIMIT 1`).catch(()=>[]);
  if(technical?.armed&&technical?.requested_state==='running')results.push(await wakeWorkerPool('technical',{reason:'planner_recovery'}).catch(error=>({ok:false,pool:'technical',error:String(error?.message||error)})));
  return results;
}

export async function runActivityDispatchTick(){
  const sql=db(),settings=await loadAutomationSettings(sql);await reconcileDispatched(sql);const workerRecovery=await wakeActiveWorkersForRecovery(sql);
  const dispatched=await dispatchDue(sql,settings,{includeDelayed:false});
  return{demand:[],forecast:[],dispatched,workerRecovery,safeProcesses:SAFE_AUTOMATIC_PROCESS_CODES.filter(code=>settings.processes[code]?.enabled),mode:'dispatch'};
}

export async function runActivityPlanner(){
  const sql=db(),settings=await loadAutomationSettings(sql),activeCodes=SAFE_AUTOMATIC_PROCESS_CODES.filter(code=>settings.processes[code]?.enabled);await reconcileDispatched(sql);const workerRecovery=await wakeActiveWorkersForRecovery(sql);await replanDelayed(sql,settings);
  const demand=[];for(const code of activeCodes)demand.push(await ensureDemand(sql,code,POLICIES[code]));
  const forecast=[];for(const code of ['PROC-SER-003','PROC-SER-004','PROC-DATA-002','PROC-PER-001'].filter(code=>settings.processes[code]?.enabled))forecast.push(await ensureFutureDemand(sql,code,POLICIES[code]));
  await planPending(sql,settings);const dispatched=await dispatchDue(sql,settings);
  return{demand,forecast,dispatched,workerRecovery,safeProcesses:activeCodes,mode:'full'};
}