...
function sanitizeBody(value){
  ...
  if(value&&typeof value==='object'){
    ...
    for(const [k,v] of Object.entries(value)){
      ...
      out[k]=(key==='authorization'||key.includes('api_key')||key==='apikey'||key==='token'||key==='access_token')?'***':sanitizeBody(v);
    }
    return out;
  }
  return value;
...
export async function loggedJsonFetch({imdbId,source,url,options={}}){
  ...
  await audit('data_quality','title',imdbId,'provider_http_response',{source,url:safeUrl,status:response.status,ok:response.ok,duration_ms:Date.now()-started,content_type:response.headers.get('content-type'),body:sanitizeBody(body)});
