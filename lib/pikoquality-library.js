...
const bandSql=s=>`CASE WHEN ${s}>=95 THEN 'honors' WHEN ${s}>=85 THEN 'outstanding' WHEN ${s}>=70 THEN 'notable' WHEN ${s}>=60 THEN 'good' WHEN ${s}>=50 THEN 'sufficient' ELSE 'fail' END`;
const prioritySql=(ps,pq)=>`CASE WHEN ${ps} IS NULL THEN NULL ELSE round((100 * power(GREATEST(0,LEAST(10,${ps}))/10.0,1.35) * power(GREATEST(0,10-(${pq}))/10.0,1.15))::numeric,1) END`;
...
export async function getPikoQualityLibrary({page=1,pageSize=20,q='',kind='',band='',priority='',sort='priority',load=false}={}){
  ...
  if(priority==='high')where.push(`priority_score>=55`);else if(priority==='medium')where.push(`priority_score>=30 AND priority_score<55`);else if(priority==='low')where.push(`priority_score<30`);
  ...
  const rows=await sql.query(`${base} SELECT *,CASE WHEN priority_score>=55 THEN 'Alta' WHEN priority_score>=30 THEN 'Media' ELSE 'Baja' END priority_label FROM scored ${w} ORDER BY ${order} LIMIT ${safeSize} OFFSET ${offset}`,params);
