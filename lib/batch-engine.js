...
export async function resumeAllGenericBatches(){
  const sql=db();await sql.query(`UPDATE batch_engine_control SET desired_state='running',pause_reason=NULL,changed_by='centro_operaciones',changed_at=now(),updated_at=now() WHERE singleton_id=1`);await Promise.all(['api','fast','plex'].map(pool=>wakeWorkerPool(pool,{reason:'engine_resumed'}).catch(()=>null)));return true;
}
