...
export async function dispatchFullSeriesRefresh(source='web'){
  ...
  const total=Number(count?.total||0),run=await startRun('series_v2_refresh',source,{stage:'queued',mode:'full',total_series:total,progress_pct:0,lifecycle_gate:'identity_valid'});
  ...
  try{
    ...
    await audit('quality','dashboard','series','refresh_dispatched',{run_id:run.id,total_series:total,lifecycle_gate:'identity_valid'});
