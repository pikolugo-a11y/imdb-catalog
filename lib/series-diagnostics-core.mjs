...
export async function rebuildSeriesDiagnostics(sql,showRatingKey){
  ...
  const oldDurations=await sql`SELECT season_number,episode_number,actual_duration_minutes FROM series_diagnostics WHERE show_rating_key=${showRatingKey}`;
  ...
  for(const r of refs){
    ...
    const actual=matched.length===1?dur.get(`${matched[0].season_number}-${matched[0].episode_number}`)||null:null;
    rows.push({show_rating_key:String(showRatingKey),season_number:Number(r.season_number),episode_number:Number(r.episode_number),status:matched.length>1?'covered_combined':matched.length===1?'present':'missing',confidence:matched.length?'confirmed':'medium',reason:matched.length>1?'Episodio oficial combinado dividido en varios archivos de Plex':matched.length===1?'Episodio presente en Plex':'No se encontró correspondencia en Plex',covered_by_rating_key:matched.length?matched.map(x=>x.rating_key).join(','):null,expected_name:r.name||null,expected_runtime_minutes:r.runtime_minutes||null,actual_duration_minutes:actual,search_hint:null,fingerprint:matched.length===1?matched[0].fingerprint||null:null});
  }
