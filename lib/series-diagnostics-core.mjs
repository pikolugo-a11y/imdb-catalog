import {automaticCombinedReason,buildAutomaticCombinedCoverage} from './series-combined-auto-detection.mjs';

function manualDoubleEvidence(note){
  try{
    const x=JSON.parse(String(note||''));
    if(x?.manual_combined!==1&&x?.manual_double!==1)return null;
    const combinedSize=[2,3].includes(Number(x.combined_size))?Number(x.combined_size):2;
    const targets=Array.isArray(x.target_episodes)?x.target_episodes.map(Number).filter(Number.isInteger):[Number(x.target_episode)];
    return{...x,combined_size:combinedSize,target_episodes:targets};
  }catch{return null}
}
...
export async function rebuildSeriesDiagnostics(sql,showRatingKey){
  ...
  const automaticCombined=buildAutomaticCombinedCoverage({refs,plex});
  const used=new Set();
  const rows=[];
  ...
  for(const r of refs){
    ...
    if(manual){
      const p=manual.p,size=Number(manual.ev?.combined_size)||2,label=size===3?'triple':size===2?'doble':'combinado';
      ...
      rows.push({
        ...
        reason:`Decisión manual: capítulo ${label}; S${String(r.season_number).padStart(2,'0')}E${String(r.episode_number).padStart(2,'0')} está incluido en S${String(p.season_number).padStart(2,'0')}E${String(p.episode_number).padStart(2,'0')} de Plex`,
    ...
    const automatic=automaticCombined.coverage.get(officialKey);
    if(automatic){
      used.add(String(automatic.p.rating_key));
      rows.push({
        show_rating_key:String(showRatingKey),
        season_number:Number(r.season_number),
        episode_number:Number(r.episode_number),
        status:'covered_combined',
        confidence:automatic.evidence.confidence,
        reason:automaticCombinedReason({target:r,evidence:automatic.evidence}),
        covered_by_rating_key:String(automatic.p.rating_key),
        expected_name:r.name||null,
        expected_runtime_minutes:r.runtime_minutes||null,
        actual_duration_minutes:Number(automatic.p.actual_duration_minutes||0)||null,
        search_hint:null,
        fingerprint:automatic.p.fingerprint||null
      });
      continue;
    }
    ...
    rows.push({
      ...
      status:'missing',
      confidence:'medium',
      reason:'No existe en Plex un episodio con la misma temporada y número ni evidencia suficiente de cobertura combinada',
  ...
  const combinedRows=rows.filter(x=>x.status==='covered_combined');
  return{
    official:rows.length,
    matched:rows.filter(x=>x.status!=='missing').length,
    combined:combinedRows.length,
    automaticCombined:combinedRows.filter(x=>String(x.reason||'').startsWith('Cobertura combinada detectada automáticamente')).length,
    manualCombined:combinedRows.filter(x=>String(x.reason||'').startsWith('Decisión manual:')).length,
    reconciled:0,
