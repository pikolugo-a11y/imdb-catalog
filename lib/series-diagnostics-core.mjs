import {reconcileReciprocalEpisodeNumbering} from './series-diagnostics-reconcile.mjs';
...
export function buildUniqueExactTitleReservations({refs=[],plex=[],blockedPlexKeys=new Set()}={}){
  const candidates=[];
  for(const r of refs){
    const expected=Number(r.runtime_minutes||0);if(expected<=0)continue;
    for(const p of plex){
      if(Number(p.season_number)!==Number(r.season_number)||blockedPlexKeys.has(String(p.rating_key))||!baseEqual(p.plex_title,r.name))continue;
      const actual=Number(p.actual_duration_minutes||0);if(!runtimeClose(actual,expected,.22))continue;
      candidates.push({officialKey:`${r.season_number}-${r.episode_number}`,plexKey:String(p.rating_key),plex:p});
    }
  }
  const byOfficial=new Map(),byPlex=new Map();
  for(const c of candidates){byOfficial.set(c.officialKey,(byOfficial.get(c.officialKey)||0)+1);byPlex.set(c.plexKey,(byPlex.get(c.plexKey)||0)+1)}
  const reservations=new Map();
  for(const c of candidates)if(byOfficial.get(c.officialKey)===1&&byPlex.get(c.plexKey)===1)reservations.set(c.officialKey,c.plex);
  return reservations;
}
...
export async function rebuildSeriesDiagnostics(sql,showRatingKey){
  ...
  const plexByKey=new Map(plex.map(p=>[String(p.rating_key),p])),manualCoverage=new Map();
  for(const o of manualOverrides){const ev=manualDoubleEvidence(o.note),p=ev?plexByKey.get(String(ev.plex_rating_key)):null;if(!p)continue;const valid=String(ev.plex_fingerprint||'')===String(p.fingerprint||'')&&Number(ev.source_season)===Number(p.season_number)&&Number(ev.source_episode)===Number(p.episode_number)&&Number(o.season_number)===Number(ev.target_season)&&Number(o.episode_number)===Number(ev.target_episode);if(valid)manualCoverage.set(`${o.season_number}-${o.episode_number}`,{p,ev})}
  const blockedPlexKeys=new Set([...manualCoverage.values()].map(x=>String(x.p.rating_key)));
  const titleReservations=buildUniqueExactTitleReservations({refs,plex,blockedPlexKeys});
  const reservedPlexKeys=new Set([...titleReservations.values()].map(p=>String(p.rating_key)));
  let rows=[];let used=new Set();const coveredOfficial=new Map();
  for(let ri=0;ri<refs.length;ri++){
    const r=refs[ri],officialKey=`${r.season_number}-${r.episode_number}`;
    const manual=manualCoverage.get(officialKey);
    if(manual){const p=manual.p;rows.push({show_rating_key:String(showRatingKey),season_number:Number(r.season_number),episode_number:Number(r.episode_number),status:'covered_combined',confidence:'confirmed',reason:`Decisión manual: capítulo doble; S${String(r.season_number).padStart(2,'0')}E${String(r.episode_number).padStart(2,'0')} está incluido en S${String(p.season_number).padStart(2,'0')}E${String(p.episode_number).padStart(2,'0')} de Plex`,covered_by_rating_key:String(p.rating_key),expected_name:r.name||null,expected_runtime_minutes:r.runtime_minutes||null,actual_duration_minutes:Number(p.actual_duration_minutes||0)||null,search_hint:null,fingerprint:p.fingerprint||null});continue;}
    ...
    const season=bySeason.get(Number(r.season_number))||[],reserved=titleReservations.get(officialKey);
    const available=season.filter(p=>!used.has(String(p.rating_key))&&(!reservedPlexKeys.has(String(p.rating_key))||String(reserved?.rating_key||'')===String(p.rating_key)));
    let matched=[];
    if(reserved&&available.some(p=>String(p.rating_key)===String(reserved.rating_key)))matched=[reserved];
    const exact=!matched.length?available.find(p=>Number(p.episode_number)===Number(r.episode_number)&&goodTitle(p.plex_title,r.name)):null;
  ...
  const reconciliation=reconcileReciprocalEpisodeNumbering({rows,plex,used});
  rows=reconciliation.rows;used=reconciliation.used;
  ...
  return{official:rows.length,matched:rows.filter(x=>x.status!=='missing').length,combined:rows.filter(x=>x.status==='covered_combined').length,reconciled:reconciliation.reconciled,unmatchedPlex:plex.filter(p=>!used.has(String(p.rating_key))).map(p=>({rating_key:p.rating_key,season_number:p.season_number,episode_number:p.episode_number,title:p.plex_title}))};
}
