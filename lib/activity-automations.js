...
export async function getActivityAutomations(){
  ...
  const [runs,plans,systemRuns]=await Promise.all([
    ...
    sql.query(`SELECT process_code,min(planned_at) FILTER(WHERE status IN('planned','delayed')) next_at,COALESCE(sum(planned_volume) FILTER(WHERE status IN('pending_planning','planned','delayed','dispatched')),0)::bigint pending_volume,count(*) FILTER(WHERE status IN('pending_planning','planned','delayed','dispatched'))::int pending_blocks FROM process_plans WHERE process_code=ANY($1::text[]) GROUP BY process_code`,[codes]),
