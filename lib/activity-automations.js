...
function nextHourlyTick(now=new Date()){
  const next=new Date(now);next.setUTCMinutes(0,0,0);next.setUTCHours(next.getUTCHours()+1);return next;
}
...
export async function getActivityAutomations(){
  ...
  const now=new Date();
  const system=SYSTEM_AUTOMATIONS.map(def=>({...def,lastRun:summarizeRun(systemRunMap.get(def.code)),nextAt:def.code==='PROC-PLAN-002'?nextHourlyTick(now):def.code==='PROC-HOME-001'?nextDashboardSnapshot(now):null}));
