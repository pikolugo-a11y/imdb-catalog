...
export async function refreshSeriesUnitary({imdbId=null,ratingKey=null}={}){
  ...
  try{
    ...
    await audit('quality','series',s.imdb_id,'unitary_refresh_completed',{rating_key:s.show_rating_key,seasons:seasons.length,episodes:episodes.length,lifecycle:lifecycle?.state});
    return{ok:true,imdbId:s.imdb_id,ratingKey:s.show_rating_key,seasons:seasons.length,episodes:episodes.length,lifecycle};
  }catch(e){
