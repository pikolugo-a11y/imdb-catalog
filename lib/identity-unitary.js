...
import {searchFilmAffinityIdentity} from './filmaffinity-identity-search';
...
export async function resolveIdentityUnitary(imdbId){
  ...
  if(!faId){
    const direct=await searchFilmAffinityIdentity({...row,tmdb_id:tmdbId}).catch(()=>null);
    if(direct?.faId){faId=String(direct.faId);methods.push(direct.method||'fa_direct_search');}
  }

  if(!faId){
    const brave=await searchFaBrave({...row,tmdb_id:tmdbId},{singleQuery:false}).catch(()=>null);
