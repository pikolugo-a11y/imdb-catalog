...
import {searchFilmAffinityUnitary} from './filmaffinity-unitary-search';
...
export async function resolveIdentityUnitary(imdbId){
  ...
  if(!faId){
    const found=await searchFilmAffinityUnitary({...row,tmdb_id:tmdbId}).catch(e=>({faId:null,method:'fa_unitary_error',error:e?.message||String(e)}));
    faDiagnostics={method:found?.method||null,confidence:Number(found?.confidence||0),requests:Number(found?.requests||0),elapsed_ms:Number(found?.elapsedMs||0),status:found?.status??null,error:found?.error||null,candidates:Number(found?.candidates||0),query:found?.query||null};
    if(found?.faId){faId=String(found.faId);methods.push(found.method||'fa_unitary_search');}
  }
