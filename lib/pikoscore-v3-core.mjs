...
function sourceConfidence(rating,row,market){
  ...
  if(!cfg)return 0;
  if(cfg.voteTarget==null)return 0.72;
...
function familyResult(family,ratings,row,market){
  const members=ratings.filter(r=>SOURCE_CONFIG[r.source]?.family===family).map(r=>{
    ...
    // Regla central V3: el volumen de votos NO cambia la nota ni el peso base
    // de una plataforma. Solo cambia la confianza que atribuimos a esa señal.
    const weight=cfg.baseWeight;
...
export function computePikoScoreV3({ratings=[],country=null,year=null,release_date=null}={}){
  ...
  const familyCoverage=availableWeight;
  ...
  return{
    ...
    contributions:contributions.map(f=>({
      ...
      sources:f.members.map(m=>({source:m.source,rating:m.value,votes:m.votes,confidence:Number(m.confidence.toFixed(3)),baseWeight:Number(m.weight.toFixed(3)),provider:m.provider})),
    })),
  };
}
