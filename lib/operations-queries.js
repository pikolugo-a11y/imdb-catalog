...
export async function getOperationsOverview(filters={}){
  ...
  const[summary,runs,errors,processes]=await Promise.all([
    ...
    FROM process_runs`,
    sql`SELECT r.run_id,r.parent_run_id,r.process_code,r.run_kind,r.trigger_source,r.executor,r.technical_status,r.functional_result,
               r.entity_type,r.entity_id,r.requested_at,r.started_at,r.finished_at,r.duration_ms,r.items_total,r.items_processed,
               r.items_succeeded,r.items_failed,r.items_pending,r.external_calls,r.retry_count,r.error_count,
               brc.desired_state AS batch_desired_state,
               (SELECT desired_state FROM batch_engine_control WHERE singleton_id=1) AS engine_desired_state
        FROM process_runs r
        LEFT JOIN batch_run_control brc ON brc.run_id=r.run_id AND brc.closed_at IS NULL
        WHERE (${status}='' OR r.technical_status=${status})
          AND (${kind}='' OR r.run_kind=${kind})
          AND (${process}='' OR r.process_code ILIKE ${'%'+process+'%'})
          AND (${entity}='' OR r.entity_id ILIKE ${'%'+entity+'%'})
