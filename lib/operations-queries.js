...
const seriesKeySql=`COALESCE(NULLIF(split_part(r.entity_id,':S',1),''),r.context->>'rating_key')`;
...
export async function getOperationsOverview(filters={}){
  ...
  const[summary,health,activeRuns,incidentRows,searchResults]=await Promise.all([
    ...
    sql`SELECT r.run_id,r.process_code,r.run_kind,r.trigger_source,r.executor,r.technical_status,r.functional_result,
               ...
               (r.process_code='PROC-PQ-002') AS controllable_technical,
               COALESCE(
                 NULLIF(r.after_compact->>'entity_label',''),
                 (SELECT sr.title FROM series_reference sr WHERE sr.show_rating_key::text=COALESCE(NULLIF(split_part(r.entity_id,':S',1),''),r.context->>'rating_key') LIMIT 1),
                 (SELECT COALESCE(m.title_es,m.title,m.original_title) FROM movies m WHERE m.imdb_id=COALESCE(r.entity_id,r.after_compact->>'imdb_id') LIMIT 1),
    ...
    hasFilters?sql`SELECT DISTINCT r.run_id,r.process_code,r.run_kind,r.trigger_source,r.executor,r.technical_status,r.functional_result,
               ...
               CASE
                 ...
                 WHEN EXISTS(SELECT 1 FROM series_reference sr WHERE sr.show_rating_key::text=COALESCE(NULLIF(split_part(r.entity_id,':S',1),''),r.context->>'rating_key') AND COALESCE(sr.title,'') ILIKE ${like}) THEN 'Serie'
                 WHEN EXISTS(SELECT 1 FROM movies m WHERE m.imdb_id=COALESCE(r.entity_id,r.after_compact->>'imdb_id') AND (COALESCE(m.title_es,'') ILIKE ${like} OR COALESCE(m.title,'') ILIKE ${like} OR COALESCE(m.original_title,'') ILIKE ${like})) THEN 'Título'
        ...
        WHERE r.requested_at>=now()-${since}::interval
          ...
          AND (${q}='' OR (
            ...
            OR EXISTS(SELECT 1 FROM series_reference sr WHERE sr.show_rating_key::text=COALESCE(NULLIF(split_part(r.entity_id,':S',1),''),r.context->>'rating_key') AND COALESCE(sr.title,'') ILIKE ${like})
            OR EXISTS(SELECT 1 FROM movies m WHERE m.imdb_id=COALESCE(r.entity_id,r.after_compact->>'imdb_id') AND (COALESCE(m.title_es,'') ILIKE ${like} OR COALESCE(m.title,'') ILIKE ${like} OR COALESCE(m.original_title,'') ILIKE ${like}))
...
export async function getRunDetail(runId,paging={}){
  ...
  const[runs,eventStats,events,errorStats,errors,childStats,children,itemStats,batchItems,laterSuccess]=await Promise.all([
    sql`SELECT r.*,EXISTS(SELECT 1 FROM batch_run_control brc WHERE brc.run_id=r.run_id AND brc.closed_at IS NULL) AS controllable_batch FROM process_runs r WHERE r.run_id=${runId}::uuid LIMIT 1`,
