...
export async function getOperationsOverview(filters={}){
  ...
  const since=String(filters.period||'30d')==='24h'?'24 hours':String(filters.period||'30d')==='7d'?'7 days':'30 days';
  const like='%'+q+'%';

  const[summary,health,incidents,searchResults]=await Promise.all([
    ...
    SELECT
      ...
      (SELECT count(*)::int FROM active_errors) AS active_incidents
    FROM process_runs`,
    sql`SELECT
      ...
      (SELECT count(*)::int FROM batch_run_items WHERE status IN ('leased','running') AND lease_expires_at IS NOT NULL AND lease_expires_at<now()) AS expired_leases,
    ...
    SELECT process_code,COALESCE(step,'') step,COALESCE(error_code,error_class,'ERROR') error_key,COALESCE(source,'') source,
           ...
           max(message) message,
           min(error_id::text) sample_error_id,
           (array_agg(run_id ORDER BY occurred_at DESC))[1] sample_run_id
    FROM active
    GROUP BY process_code,COALESCE(step,''),COALESCE(error_code,error_class,'ERROR'),COALESCE(source,'')
    ORDER BY max(occurred_at) DESC
    LIMIT 20`,
    q?sql`SELECT DISTINCT r.run_id,r.process_code,r.run_kind,r.trigger_source,r.executor,r.technical_status,r.functional_result,
        ...
        WHERE r.requested_at>=now()-${since}::interval
          ...
          AND (${source}='' OR EXISTS(SELECT 1 FROM process_run_errors e WHERE e.run_id=r.run_id AND COALESCE(e.source,'') ILIKE ${'%'+source+'%'}))
          AND (
  ...
  return{summary:summary[0]||{},health:health[0]||{},incidents,searchResults,query:q};
}
