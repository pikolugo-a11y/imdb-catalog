...
export async function getQualityHomeSnapshot(){
  ...
  const [stateRows,[meta],series,people,[tech]]=await Promise.all([
    ...
    sql`WITH pending_titles AS (
      ...
      SELECT t.imdb_id,
        CASE WHEN t.type='Película' THEN NOT EXISTS(
        ...
        ) END AS truly_pending
      FROM pending_titles t
    )
    SELECT count(*) FILTER(WHERE truly_pending)::int pending,count(*) FILTER(WHERE NOT truly_pending)::int resolved FROM checked`
  ]);
  ...
  const rawTechPending=Number(counts.TECH_PENDING||0),effectiveTechPending=Number(tech?.pending||0),resolvedTech=Math.max(0,Number(tech?.resolved||0));
