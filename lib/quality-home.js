...
export async function getQualityHomeSnapshot(){
  const sql=db();
  const [stateRows,[meta],series,people,[tech],[relevance]]=await Promise.all([
    ...
    sql`SELECT count(*)::int pending FROM catalog_read_model c LEFT JOIN catalog_exclusions ex ON ex.imdb_id=c.imdb_id LEFT JOIN series_relevance_assessments a ON a.imdb_id=c.imdb_id AND a.formula_version=${PIKORELEVANCE_VERSION} WHERE c.type IN ('Serie','Miniserie') AND ex.imdb_id IS NULL AND a.imdb_id IS NULL`
  ]);
  ...
  return {...buildQualityHome({total:Number(meta?.total||0),materialized,missing:Number(meta?.missing||0),counts,stageCounts,trackingExtras,integrity:{orphaned:Number(meta?.orphaned||0),unknown,incompatible:Number(meta?.incompatible||0)}}),relevancePending:Number(relevance?.pending||0)};
}
