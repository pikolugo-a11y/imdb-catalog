...
import {getPikoQualityLibrary} from './pikoquality-library';

export async function getQualityHomeSnapshot(){
  const sql=db();
  const [stateRows,[meta],identity,validation,data,movies,series,pikoquality]=await Promise.all([
    ...
    getIdentityWorkflowStats(),
    getIdentityValidationStats(),
    getDataQualityOverview(sql),
    getMovieQualitySummary(sql),
    getSeriesQualityCounts(sql),
    getPikoQualityLibrary({load:false})
  ]);
  const counts={};
  let unknown=0;
  for(const row of stateRows){
    counts[row.lifecycle_state]=Number(row.count||0);
    if(!LIFECYCLE[row.lifecycle_state])unknown+=Number(row.count||0);
  }
  const materialized=Object.values(counts).reduce((a,b)=>a+Number(b||0),0);
  const stageCounts={
    recovery:Number(meta?.missing||0),
    identity:Number(identity?.affected_catalog||0),
    validation:Number(validation?.total||0),
    data:Number(data?.incomplete||0)+Number(data?.ratingsPending||0)+Number(data?.scoreReady||0),
    movies:Number(movies?.affected||0),
    series:Number(series?.attention||0),
    pikoquality:Number(pikoquality?.summary?.high_priority||0)
  };
  return buildQualityHome({
    total:Number(meta?.total||0),materialized,missing:Number(meta?.missing||0),counts,stageCounts,
    integrity:{orphaned:Number(meta?.orphaned||0),unknown,incompatible:Number(meta?.incompatible||0)}
  });
}
