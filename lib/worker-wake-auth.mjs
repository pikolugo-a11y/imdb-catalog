...
function wakeKey(databaseUrl){
  ...
  let shared=value;
  try{const parsed=new URL(value);shared=decodeURIComponent(parsed.password||'')||value}catch{}
  return createHash('sha256').update(`pikofilm-worker-wake-v1|${shared}`).digest();
}
