...
export async function imdbRatingsFromOfficialDataset(imdbIds,{timeoutMs=30000}={}){
  const targets=[...new Set((imdbIds||[]).map(String).filter(id=>/^tt\d+$/.test(id)))];
  if(!targets.length)return new Map();
  const wanted=new Set(targets),found=new Map();
  const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const response=await fetch(DATASET,{signal:controller.signal,cache:'no-store',headers:{'User-Agent':'PikoFilm/3.0 personal-noncommercial-batch-ratings'}});
    if(!response.ok||!response.body)return found;
    ...
    for await(const line of rl){
      ...
      const firstTab=line.indexOf('\t');if(firstTab<0)continue;const id=line.slice(0,firstTab);if(!wanted.has(id))continue;
      const parts=line.split('\t'),rating=Number(parts[1]),votes=Number(parts[2]);
      if(Number.isFinite(rating)&&Number.isInteger(votes))found.set(id,{rating,votes,source:'title.ratings.tsv.gz-on-demand'});
      if(found.size===wanted.size)break;
    }
    return found;
  }catch{return found}finally{clearTimeout(timer)}
}

export async function imdbRatingFromOfficialDataset(imdbId,{timeoutMs=12000}={}){
  return (await imdbRatingsFromOfficialDataset([imdbId],{timeoutMs})).get(String(imdbId))||null;
}
