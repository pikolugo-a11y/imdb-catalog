...
export function deriveSeriesQualityState(row,now=new Date()){
  ...
  if(row?.manual_complete)return{state:'AL_DIA',label:'Cuadrada manualmente',severity:'ok',reason:'Cobertura aceptada manualmente para esta serie; la evidencia física de Plex se conserva sin modificar',attention_count:0,pending_count:0,is_reliable:true,evaluated_at:evaluatedAt,referenceFreshness:freshness,manual_complete:true};
  ...
  if(pendingCount>0)return{state:'REVISION_PENDIENTE',label:'En seguimiento automático',severity:'warn',reason:`${pendingCount} episodio${pendingCount===1?'':'s'} por confirmar`,attention_count:0,pending_count:pendingCount,is_reliable:true,evaluated_at:evaluatedAt,referenceFreshness:freshness};
...
export function classifySeries(row,now=new Date()){
  const plexTrusted=Boolean(row.plex_detail_trusted);
  const freshness=getReferenceFreshness(row,now);
  const manualComplete=Boolean(row.manual_complete);
  const missing=manualComplete?0:n(row.actionable_missing),unknown=manualComplete?0:n(row.availability_unknown),availabilityDue=manualComplete?0:n(row.availability_due??unknown),extra=manualComplete?0:n(row.unmapped_plex_episodes??row.extra_episodes);
  const flags=[];
  ...
  if(manualComplete){primaryState='uptodate';severity='ok';diagnosis={code:'MANUAL_COMPLETE',label:'Cuadrada manualmente',detail:'PikoFilm considera cubierta la serie por decisión manual sin alterar la evidencia física de Plex',severity};nextAction={code:'VIEW_DETAIL',label:'Ver detalle'};}
  else if(!row.has_reference){primaryState='pre_quality';severity='warn';diagnosis={code:'PRE_QUALITY',label:'Preparando referencia',detail:'Todavía no existe la referencia oficial TMDb de episodios para esta serie',severity};nextAction={code:'AUTO_PREPARE_REFERENCE',label:'Seguimiento automático',automatic:true};}
  else if(!plexTrusted){primaryState='plex_sync';severity='warn';diagnosis={code:'PLEX_STALE',label:'Inventario Plex en seguimiento',detail:row.plex_changed?'Plex cambió · PikoFilm actualizará el detalle':'Falta comprobar el inventario de episodios',severity};nextAction={code:'AUTO_SYNC_PLEX_DETAIL',label:'Seguimiento automático',automatic:true};}
  else if(!freshness.isTrusted){primaryState='tmdb_refresh';severity='warn';diagnosis={code:'TMDB_STALE',label:freshness.label,detail:freshness.reason,severity};nextAction={code:'AUTO_REFRESH_TMDB',label:'Seguimiento automático',automatic:true};}
  ...
  else if(availabilityDue){primaryState='unknown';severity='warn';diagnosis={code:'UNKNOWN',label:`${availabilityDue} por confirmar`,detail:'Disponibilidad en España pendiente de una comprobación automática',severity};nextAction={code:'AUTO_REVIEW_AVAILABILITY',label:'Seguimiento automático',automatic:true};}
  else if(unknown){primaryState='uptodate';severity='ok';diagnosis={code:'UNKNOWN_COOLDOWN',label:'Comprobado recientemente',detail:`${unknown} episodio${unknown===1?'':'s'} siguen sin evidencia positiva; PikoFilm volverá a comprobarlos al vencer el plazo`,severity};nextAction={code:'VIEW_DETAIL',label:'Ver detalle'};}
...
export function nextReferenceCheck({status,lastAirDate,nextAirDate,refreshedAt}={}){
  ...
  if(['ended','canceled','cancelled'].includes(normalized)){
    const last=date(lastAirDate);
    if(!last)return new Date(base.getTime()+180*DAY);
    const ageYears=Math.max(0,(base.getTime()-last.getTime())/(365.25*DAY));
    const days=ageYears<2?180:ageYears<10?365:1095;
    return new Date(base.getTime()+days*DAY);
  }
  if(['returning series','in production','planned','pilot'].includes(normalized))return new Date(base.getTime()+7*DAY);
  return new Date(base.getTime()+30*DAY);
}
