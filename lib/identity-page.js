...
const STATUSES=new Set(['','untried','not_found','error','review','processing','resolved']);
...
function outcomeToStatus(row){
  ...
  if(row.tmdb_id)return'resolved';
...
export async function getIdentityCatalogPage(filters={}){
  const sql=db();
  const p=parseIdentityFilters(filters),isImdb=/^tt\d+$/i.test(p.q),like='%'+p.q+'%',searching=Boolean(p.q);
  const base=sql`ex.imdb_id IS NULL
    AND (${searching} OR (cl.lifecycle_state='IDENTITY_PENDING' AND m.tmdb_id IS NULL))
    ...
    AND (${p.q}='' OR (${isImdb} AND lower(m.imdb_id)=${p.q}) OR (NOT ${isImdb} AND (
      ...
      lower(COALESCE(m.original_title,'')) LIKE ${like} OR lower(m.imdb_id) LIKE ${like} OR lower(COALESCE(m.tmdb_id,'')) LIKE ${like}
    )))
  ...
  const statePredicate=sql`(${p.status}='' OR
    ...
    (${p.status}='resolved' AND NOT(${processing}) AND m.tmdb_id IS NOT NULL) OR
    (${p.status}='not_found' AND m.tmdb_id IS NULL AND NOT(${processing}) AND pr.technical_status='succeeded' AND pr.functional_result='not_found') OR
    (${p.status}='error' AND m.tmdb_id IS NULL AND NOT(${processing}) AND pr.technical_status IN('failed','partial')) OR
    (${p.status}='untried' AND m.tmdb_id IS NULL AND NOT(${processing}) AND pr.run_id IS NULL)
  )`;
  ...
  const rows=await sql`SELECT m.imdb_id,m.type,COALESCE(m.title_es,m.title,m.original_title) AS display_title,m.title AS main_title,m.original_title,m.year,m.tmdb_id,
      COALESCE(m.source_status->>'identity_mode','normal') AS identity_mode,
  ...
  const [statusRow]=await sql`SELECT
      count(*) FILTER(WHERE m.tmdb_id IS NULL AND NOT(${processing}) AND pr.run_id IS NULL)::int untried,
      count(*) FILTER(WHERE m.tmdb_id IS NULL AND NOT(${processing}) AND pr.technical_status='succeeded' AND pr.functional_result='not_found')::int not_found,
      count(*) FILTER(WHERE m.tmdb_id IS NULL AND NOT(${processing}) AND pr.technical_status IN('failed','partial'))::int error,
      0::int review,
      count(*) FILTER(WHERE ${processing})::int processing,
      count(*) FILTER(WHERE m.tmdb_id IS NOT NULL AND NOT(${processing}))::int resolved
    FROM catalog_lifecycle cl JOIN movies m USING(imdb_id)
    LEFT JOIN catalog_exclusions ex USING(imdb_id)
    LEFT JOIN catalog_read_model crm ON crm.imdb_id=m.imdb_id
    ${telemetry}
  ...
  return{
    ...
    statusCounts:statusRow||{untried:0,not_found:0,error:0,review:0,processing:0,resolved:0},filters:p
  };
}
