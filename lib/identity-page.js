...
export async function getIdentityCatalogPage(filters={}){
  ...
  const [countRows,rows]=await Promise.all([
    ...
    sql`SELECT m.imdb_id,m.type,COALESCE(m.title_es,m.title,m.original_title) AS display_title,m.original_title,m.year,m.tmdb_id,m.fa_id,
      (m.tmdb_id IS NULL) AS missing_tmdb,(m.fa_id IS NULL) AS missing_fa,cl.blocking_reason
...
export async function getIdentityWorkflowStats(){
  ...
  const[r]=await sql`SELECT
    ...
    WHERE cl.lifecycle_state='IDENTITY_PENDING' AND ex.imdb_id IS NULL`;
  return r||{affected_catalog:0,missing_tmdb:0,missing_fa:0,movies:0,series:0};
}
