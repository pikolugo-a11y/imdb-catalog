...
export async function getMovieDetailExtras(imdbId){
  ...
  const [copy]=await sql`
    SELECT pcs.rating_key,p.plex_title,p.added_at,
      ...
      att.id attention_id,att.finding_type attention_type,att.severity attention_severity
    ...
    LEFT JOIN LATERAL(
      SELECT f.id,f.finding_type,f.severity
      FROM movie_quality_findings f
      JOIN catalog_lifecycle cl ON cl.imdb_id=f.imdb_id AND cl.lifecycle_state='MOVIE_FILE_REVIEW'
      WHERE f.imdb_id=${imdbId}
        AND f.rating_key=p.rating_key
        AND f.status='pending'
        AND f.finding_type IN('duration','filename','duplicate')
      ORDER BY CASE f.severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END,f.risk_score DESC NULLS LAST,f.last_seen_at DESC
      LIMIT 1
    ) att ON true
