...
function deriveOrigin(row){if(row.row_kind==='plex_unidentified')return 'plex';const s=row.source_snapshot||{};if(s.manual===true||s.manual==='true'||row.matched_rule==='manual')return 'manual';if(s.origin==='plex'||row.matched_rule==='plex')return 'plex';if(s.origin==='saga'||row.matched_rule==='saga_manual')return 'saga';return 'discovery'}
...
export async function getNewsV1(filters={}){
  ...
  const candidateRows=await sql`
    SELECT 'candidate'::text row_kind,c.imdb_id,NULL::text rating_key,c.candidate_type,c.year,c.imdb_rating,c.imdb_votes,c.first_seen_at,c.became_eligible_at,c.last_seen_at,c.eligibility_status,c.source_snapshot,
      ...
      COALESCE(c.source_snapshot->>'matchedRule',CASE WHEN c.source_snapshot->>'origin'='plex' THEN 'plex' WHEN c.source_snapshot->>'origin'='saga' THEN 'saga_manual' WHEN c.source_snapshot->>'manual'='true' THEN 'manual' ELSE 'general' END) matched_rule,
  ...
  const stats={
    ...
    saga:raw.filter(r=>r.origin==='saga').length,
...
export async function getNewsCandidate(imdbId){const sql=db();const [r]=await sql`SELECT c.*,COALESCE(c.source_snapshot->>'title',c.source_snapshot->>'originalTitle',c.imdb_id) title,COALESCE(c.source_snapshot->>'matchedRule',CASE WHEN c.source_snapshot->>'origin'='plex' THEN 'plex' WHEN c.source_snapshot->>'origin'='saga' THEN 'saga_manual' WHEN c.source_snapshot->>'manual'='true' THEN 'manual' ELSE 'general' END) matched_rule FROM catalog_candidates c LEFT JOIN movies m ON m.imdb_id=c.imdb_id LEFT JOIN catalog_exclusions ex ON ex.imdb_id=c.imdb_id WHERE c.imdb_id=${imdbId} AND m.imdb_id IS NULL AND ex.imdb_id IS NULL LIMIT 1`;return r||null}
