...
function deriveOrigin(row){if(row.row_kind==='plex_unidentified')return 'plex';const s=row.source_snapshot||{};if(s.restoredFromExclusion===true||s.restoredFromExclusion==='true')return 'manual';if(s.manual===true||s.manual==='true'||row.matched_rule==='manual')return 'manual';if(s.origin==='plex'||row.matched_rule==='plex')return 'plex';if(s.origin==='saga'||row.matched_rule==='saga_manual')return 'saga';return 'discovery'}
function deriveState(row){
  if(row.row_kind==='plex_unidentified')return 'identity_pending';
  const s=row.source_snapshot||{},origin=deriveOrigin(row);
  if(s.restoredFromExclusion===true||s.restoredFromExclusion==='true')return row.eligibility_status==='eligible'?'eligible':'preparing';
  if(origin==='manual'){
    if(s.manualResolveStatus==='failed')return 'error';
    if(s.manualResolveStatus==='complete')return row.eligibility_status==='eligible'?'eligible':'preparing';
    if(s.authoritativeStatus==='failed')return 'error';
    if(s.authoritativeStatus!=='complete'&&!s.manualAuthoritativeResolvedAt)return 'preparing';
    return row.eligibility_status==='eligible'?'eligible':'preparing';
  }
  if(s.authoritativeStatus==='failed')return 'error';
  if(row.eligibility_status==='processing')return 'preparing';
  if(origin==='plex'&&row.eligibility_status!=='eligible')return 'preparing';
  if((s.authoritativeStatus==='pending'||s.authoritativeRequestedAt)&&row.imdb_rating==null&&row.imdb_votes==null)return 'preparing';
  return 'eligible';
}
...
export async function getNewsV1(filters={}){
  ...
  const candidateRows=await sql`
    SELECT 'candidate'::text row_kind,c.imdb_id,NULL::text rating_key,c.candidate_type,c.year,c.imdb_rating,c.imdb_votes,c.first_seen_at,c.became_eligible_at,c.last_seen_at,c.eligibility_status,c.source_snapshot,
      ...
      COALESCE(c.source_snapshot->>'matchedRule',CASE WHEN c.source_snapshot->>'origin'='plex' THEN 'plex' WHEN c.source_snapshot->>'origin'='saga' THEN 'saga_manual' WHEN c.source_snapshot->>'manual'='true' OR c.source_snapshot->>'restoredFromExclusion'='true' THEN 'manual' ELSE 'general' END) matched_rule,
    ...
    WHERE ((m.imdb_id IS NULL AND ex.imdb_id IS NULL) OR c.source_snapshot->>'restoredFromExclusion'='true')
  ...
  const plexPending=await sql`
    SELECT 'plex_unidentified'::text row_kind,NULL::text imdb_id,p.rating_key,
      ...
      p.plex_year AS year,NULL::numeric imdb_rating,NULL::bigint imdb_votes,p.added_at first_seen_at,NULL::timestamptz became_eligible_at,COALESCE(p.plex_updated_at,p.last_seen_at,p.synced_at) last_seen_at,
  ...
  if(['manual','plex','saga','discovery'].includes(source))all=all.filter(r=>r.origin===source);
  ...
  const stats={total:raw.length,movies:raw.filter(r=>r.candidate_type==='movie').length,series:raw.filter(r=>['tvSeries','tvMiniSeries'].includes(r.candidate_type)).length,spanish_rescues:raw.filter(r=>r.matched_rule==='spain').length,manual:raw.filter(r=>r.origin==='manual').length,plex:raw.filter(r=>r.origin==='plex').length,saga:raw.filter(r=>r.origin==='saga').length,discovery:raw.filter(r=>r.origin==='discovery').length,identity_pending:raw.filter(r=>r.news_state==='identity_pending').length,preparing:raw.filter(r=>r.news_state==='preparing').length,eligible:raw.filter(r=>r.news_state==='eligible').length,error:raw.filter(r=>r.news_state==='error').length};

  const [excludedStats]=await sql`SELECT count(*)::int total FROM catalog_exclusions e LEFT JOIN catalog_candidates c ON c.imdb_id=e.imdb_id WHERE NOT (COALESCE(c.source_snapshot->>'restoredFromExclusion','false')='true' AND c.eligibility_status IN('eligible','processing'))`;
  return {rows,stats,filteredTotal:all.length,excludedCount:excludedStats?.total||0,page,pageSize};
}

export async function getNewsCandidate(imdbId){const sql=db();const [r]=await sql`SELECT c.*,COALESCE(c.source_snapshot->>'title',c.source_snapshot->>'originalTitle',c.imdb_id) title,COALESCE(c.source_snapshot->>'matchedRule',CASE WHEN c.source_snapshot->>'origin'='plex' THEN 'plex' WHEN c.source_snapshot->>'origin'='saga' THEN 'saga_manual' WHEN c.source_snapshot->>'manual'='true' OR c.source_snapshot->>'restoredFromExclusion'='true' THEN 'manual' ELSE 'general' END) matched_rule FROM catalog_candidates c LEFT JOIN movies m ON m.imdb_id=c.imdb_id LEFT JOIN catalog_exclusions ex ON ex.imdb_id=c.imdb_id WHERE c.imdb_id=${imdbId} AND ((m.imdb_id IS NULL AND ex.imdb_id IS NULL) OR c.source_snapshot->>'restoredFromExclusion'='true') LIMIT 1`;return r||null}
