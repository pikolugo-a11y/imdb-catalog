...
export async function getNewsV1(filters={}){
  ...
  const [stats]=await sql`SELECT count(*)::int total,count(*) FILTER(WHERE c.candidate_type='movie')::int movies,count(*) FILTER(WHERE c.candidate_type IN('tvSeries','tvMiniSeries'))::int series,count(*) FILTER(WHERE c.source_snapshot->>'matchedRule'='spain')::int spanish_rescues,count(*) FILTER(WHERE c.source_snapshot->>'manual'='true' AND COALESCE(c.source_snapshot->>'manualActive','true')='true')::int manual,count(*) FILTER(WHERE c.source_snapshot->>'origin'='plex')::int plex FROM catalog_candidates c LEFT JOIN movies m ON m.imdb_id=c.imdb_id LEFT JOIN catalog_exclusions ex ON ex.imdb_id=c.imdb_id WHERE m.imdb_id IS NULL AND ex.imdb_id IS NULL AND (c.eligibility_status IN('eligible','processing') OR (c.source_snapshot->>'manual'='true' AND COALESCE(c.source_snapshot->>'manualActive','true')='true'))`;
  ...
  return {rows,stats:stats||{total:0,movies:0,series:0,spanish_rescues:0,manual:0,plex:0},excludedCount:excludedStats?.total||0,latestRun:latestRun||null,lastSuccess:lastSuccess||null,nextAllowedAt,discoveryAllowed,testOverrideAvailable,testOverrideUpdatedAt:overrideRow?.updated_at||null,page,pageSize};
}
