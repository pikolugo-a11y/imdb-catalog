...
export function buildQualityHome(snapshot={}){
  ...
  const trackingExtras=Object.fromEntries(Object.entries(snapshot.trackingExtras||{}).map(([k,v])=>[k,Number(v||0)]));
  const stages=QUALITY_STAGE_META.map(meta=>{const count=stageCounts[meta.id]||0;return{...meta,count,status:statusFor(meta.id,count,counts),tracking:Number(trackingExtras[meta.id]||0)}});
  const total=Number(snapshot.total||0),excluded=Number(counts.EXCLUDED||0),activeTotal=Math.max(0,total-excluded),complete=Math.min(activeTotal,Number(counts.COMPLETE||0)),progressPct=activeTotal?Math.max(0,Math.min(100,(complete/activeTotal)*100)):100,pending=Math.max(0,activeTotal-complete),areasPending=stages.filter(s=>s.count>0||s.tracking>0).length;
  ...
  const trackingCount=trackingStages.reduce((n,s)=>n+s.count,0)+Object.values(trackingExtras).reduce((n,v)=>n+Number(v||0),0);
  const healthyAreas=stages.filter(s=>s.status.key==='healthy'&&!s.tracking).length;
  ...
  return{total,activeTotal,excluded,complete,pending,progressPct,materialized:Number(snapshot.materialized||0),missing,integrity,stages,centerStages,specializedStages,stageCounts,trackingExtras,areasPending,attentionCount,trackingCount,healthyAreas,priorityItems,globalStatus};
}
