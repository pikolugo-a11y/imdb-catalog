...
export function buildQualityHome(snapshot={}){
  ...
  const attentionStages=stages.filter(s=>s.status.key==='attention');
  const priorityItems=[];
  if(missing>0)priorityItems.push({id:'recovery',label:'Recuperación Lifecycle',count:missing,href:'/calidad/sin-estado',severity:'blocked'});
  if(integrity.total>0)priorityItems.push({id:'integrity',label:'Integridad estructural',count:integrity.total,href:'/calidad',severity:'blocked'});
  for(const s of attentionStages)priorityItems.push({id:s.id,label:s.label,count:s.count,href:s.href,severity:'attention'});

  const globalStatus=missing>0||integrity.total>0
    ?{key:'blocked',label:'Requiere intervención',icon:'!'}
    :attentionStages.length>0
      ?{key:'attention',label:'Hay revisiones prioritarias',icon:'!'}
      :areasPending>0
        ?{key:'pending',label:'Hay trabajo pendiente',icon:'•'}
        :{key:'healthy',label:'Todo al día',icon:'✓'};

  return{total,activeTotal,excluded,complete,pending,progressPct,materialized:Number(snapshot.materialized||0),missing,integrity,stages,stageCounts,areasPending,priorityItems,globalStatus};
}
