...
export async function getDashboard() {
  ...
  const [row] = await sql`
    SELECT
      ...
      (SELECT count(DISTINCT show_rating_key)::int FROM series_episode_effective_status WHERE effective_status <> 'present') AS series_with_issues,
      (SELECT count(*)::int FROM series_episode_effective_status WHERE effective_status = 'missing_actionable') AS missing_episodes_actionable,
      (SELECT count(*)::int FROM series_episode_effective_status WHERE effective_status = 'availability_unknown') AS missing_episodes_unknown,
...
export async function getSeriesQuality(limit = 100) {
  ...
  return sql`
    SELECT r.show_rating_key, r.title, r.year, r.official_seasons, r.official_episodes,
           count(*) FILTER (WHERE e.effective_status = 'present')::int AS present,
           count(*) FILTER (WHERE e.effective_status = 'missing_actionable')::int AS actionable_missing,
           count(*) FILTER (WHERE e.effective_status = 'not_yet_available')::int AS not_yet_available,
           count(*) FILTER (WHERE e.effective_status = 'availability_unknown')::int AS availability_unknown,
           count(*) FILTER (WHERE e.effective_status = 'exception')::int AS exceptions,
    ...
    ORDER BY actionable_missing DESC, availability_unknown DESC, r.title
    LIMIT ${limit}
  `;
}
...
export async function getSeriesDetail(showRatingKey) {
  ...
  const episodes = await sql`
    SELECT e.season_number, e.episode_number, e.plex_diagnostic_status, e.plex_confidence,
           e.reason, e.covered_by_rating_key, e.expected_name, e.expected_runtime_minutes,
           e.actual_duration_minutes, e.air_date, e.availability_status, e.available_from,
           e.availability_source, e.availability_confidence, e.availability_checked_at,
           e.effective_status, r.overview
    FROM series_episode_effective_status e
    LEFT JOIN series_reference_episodes r
      ON r.show_rating_key = e.show_rating_key
     AND r.season_number = e.season_number
     AND r.episode_number = e.episode_number
    WHERE e.show_rating_key = ${showRatingKey}
