...
WITH x AS (
 SELECT e.imdb_id,
 COALESCE(m.title_es,m.title,m.original_title,c.source_snapshot->>'title',c.source_snapshot->>'originalTitle',e.imdb_id) display_title,
 COALESCE(m.original_title,c.source_snapshot->>'originalTitle') original_title,
 COALESCE(m.year,c.year) item_year,
 COALESCE(m.type,CASE WHEN c.candidate_type='movie' THEN 'Película' WHEN c.candidate_type='tvMiniSeries' THEN 'Miniserie' WHEN c.candidate_type='tvSeries' THEN 'Serie' ELSE NULL END) item_type,
 COALESCE(m.poster_path,c.source_snapshot->>'posterPath') poster_path,
 COALESCE(ir.rating,c.imdb_rating) imdb_rating,
 COALESCE(ir.votes,c.imdb_votes) imdb_votes,
 m.final_rating,e.reason,e.excluded_at,e.excluded_by,(m.imdb_id IS NOT NULL) in_catalog,(c.imdb_id IS NOT NULL) in_candidates
...
SELECT imdb_id,display_title,original_title,item_year year,item_type type,poster_path,imdb_rating,imdb_votes,final_rating,reason,excluded_at,excluded_by,in_catalog,in_candidates FROM x
...
export async function getExcludedStatsV3(filters={}){const sql=db(),p=parse(filters);const[row]=await sql`WITH x AS (SELECT e.imdb_id,COALESCE(m.title_es,m.title,m.original_title,c.source_snapshot->>'title',c.source_snapshot->>'originalTitle',e.imdb_id) display_title,COALESCE(m.original_title,c.source_snapshot->>'originalTitle') original_title,COALESCE(m.type,CASE WHEN c.candidate_type='movie' THEN 'Película' WHEN c.candidate_type='tvMiniSeries' THEN 'Miniserie' WHEN c.candidate_type='tvSeries' THEN 'Serie' ELSE NULL END) item_type,e.reason,e.excluded_at FROM catalog_exclusions e LEFT JOIN movies m USING(imdb_id) LEFT JOIN catalog_candidates c USING(imdb_id)) SELECT count(*)::int total,count(*) FILTER(WHERE item_type='Película')::int movies,count(*) FILTER(WHERE item_type=ANY(${seriesTypes}))::int series,count(*) FILTER(WHERE excluded_at>=now()-interval '30 days')::int last30 FROM x WHERE (${p.q}='' OR lower(COALESCE(display_title,'')) LIKE ${'%'+p.q+'%'} OR lower(COALESCE(original_title,'')) LIKE ${'%'+p.q+'%'} OR lower(imdb_id) LIKE ${'%'+p.q+'%'}) AND (${p.type}='' OR (${p.type}='movie' AND item_type='Película') OR (${p.type}='series' AND item_type=ANY(${seriesTypes}))) AND (${p.reason}='' OR lower(COALESCE(reason,'')) LIKE ${'%'+p.reason+'%'}) AND (${p.from}::date IS NULL OR excluded_at::date>=${p.from}::date) AND (${p.to}::date IS NULL OR excluded_at::date<=${p.to}::date)`;return row||{total:0,movies:0,series:0,last30:0}}
