...
export async function intakeNewsCandidate(input,{sql=db()}={}){
  ...
  if(origin==='plex'&&context.plexRatingKey){const[linked]=await sql`SELECT pcs.imdb_id FROM plex_catalog_status pcs JOIN movies m ON m.imdb_id=pcs.imdb_id WHERE pcs.rating_key=${String(context.plexRatingKey)} AND pcs.status='in_plex' LIMIT 1`;if(linked)return{status:'catalogued',imdbId,canonicalImdbId:linked.imdb_id,deduplicatedBy:'plex_rating_key'}}
