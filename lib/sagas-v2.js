...
async function imdbForTmdb(sql,tmdbId,trace,issues){
  ...
  try{
    ...
    if(canonical&&known?.imdb_id&&known.imdb_id!==canonical){
      issues.identityCorrections++;
      await trace.event({eventType:'step_completed',step:'repair_member_identity',message:'Identidad IMDb de miembro de saga corregida desde TMDb',data:{tmdb_movie_id:String(tmdbId),previous_imdb_id:known.imdb_id,canonical_imdb_id:canonical}}).catch(()=>{});
    }
    return canonical;
  }catch(error){
...
export async function refreshSagas({limit=120,requestKey,collectionId}={}){
  const safeLimit=Math.min(120,Math.max(1,Number(limit)||120));
  const targetCollectionId=collectionId==null?null:String(collectionId).trim();
  if(targetCollectionId&&!/^\d+$/.test(targetCollectionId))throw new Error('TMDb collection ID inválido');
  const key=requestKey||`saga-refresh:${targetCollectionId||'batch'}:${Math.floor(Date.now()/3000)}`;
  const observed=await executeObservedProcess({
    processCode:'PROC-SAGA-001',
    runKind:'system',
    triggerSource:'sagas_manual',
    executor:'vercel',
    entityType:targetCollectionId?'saga_collection':'saga_universe',
    entityId:targetCollectionId||'tmdb',
    correlationKey:key,
    idempotencyKey:`PROC-SAGA-001:${key}`,
    context:{surface:'/sagas',operation:'refresh_sagas_tmdb',limit:targetCollectionId?1:safeLimit,concurrency:targetCollectionId?1:6,target_collection_id:targetCollectionId}
  },async trace=>{
    const sql=db();
    const issues={externalIdErrors:0,duplicateMembers:0,identityCorrections:0};
    await trace.event({eventType:'step_started',step:'select_collections',message:targetCollectionId?'Seleccionando colección TMDb solicitada':'Seleccionando colecciones TMDb a refrescar'});
    const ids=targetCollectionId
      ?await sql`SELECT tmdb_collection_id FROM saga_collections WHERE tmdb_collection_id::text=${targetCollectionId} LIMIT 1`
      :await sql`
      ...
      LEFT JOIN (
        SELECT sm.tmdb_collection_id,
               count(*)::int actual_member_count,
               count(*) FILTER(
                 WHERE sm.imdb_id LIKE 'tt%'
                   AND EXISTS(
                     SELECT 1 FROM movies m
                     WHERE m.imdb_id=sm.imdb_id
                       AND m.tmdb_id::text IS DISTINCT FROM sm.tmdb_movie_id::text
                   )
      ...
      ORDER BY
        CASE
          WHEN COALESCE(sm.identity_mismatch_count,0)>0 THEN 0
          WHEN sc.tmdb_collection_id IS NOT NULL AND COALESCE(sm.actual_member_count,0)<>COALESCE(sc.member_count,0) THEN 1
          ELSE 2
        END,
        sc.refreshed_at ASC NULLS FIRST,
        x.tmdb_collection_id
      LIMIT ${safeLimit}`;
    if(targetCollectionId&&ids.length===0)throw new Error(`Saga TMDb ${targetCollectionId} no encontrada en caché local`);
    await trace.event({eventType:'step_completed',step:'select_collections',message:'Colecciones seleccionadas',data:{selected:ids.length,limit:targetCollectionId?1:safeLimit,target_collection_id:targetCollectionId}});

    let added=0,updated=0,members=0,outsideCatalog=0;
    await trace.event({eventType:'step_started',step:'refresh_collections',message:'Refrescando colecciones y miembros desde TMDb',data:{selected:ids.length,concurrency:targetCollectionId?1:6}});
    const results=await pool(ids,targetCollectionId?1:6,async row=>{
      ...
      for(const p of parts){
        pos++;
        const imdbId=await imdbForTmdb(sql,p.id,trace,issues);
        const [m]=imdbId?await sql`SELECT m.imdb_id FROM movies m LEFT JOIN catalog_exclusions ex USING(imdb_id) WHERE ex.imdb_id IS NULL AND m.imdb_id=${imdbId} LIMIT 1`:[];
        if(!m)localOutside++;
    ...
    return{
      technicalStatus,
      functionalResult,
      metrics:{collections_selected:ids.length,collections_refreshed:refreshedCollections,collections_not_found:notFoundCollections,collections_added:added,collections_updated:updated,members,outside_catalog:outsideCatalog,collection_errors:collectionErrors,external_id_errors:issues.externalIdErrors,identity_corrections:issues.identityCorrections,duplicate_members_ignored:issues.duplicateMembers,...stats},
      after:{collections:refreshedCollections,collections_not_found:notFoundCollections,members,outside_catalog:outsideCatalog,errors:totalErrors,identity_corrections:issues.identityCorrections,duplicate_members_ignored:issues.duplicateMembers,...stats},
