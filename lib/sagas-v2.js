...
async function imdbForTmdb(sql,tmdbId,trace,issues){
  const [known]=await sql`SELECT external_id imdb_id FROM tmdb_external_ids WHERE tmdb_id::text=${String(tmdbId)} AND media_type='movie' AND provider='imdb' AND external_id LIKE 'tt%' LIMIT 1`;
  if(known?.imdb_id)return known.imdb_id;
  try{
    const ext=await tmdb(`/movie/${tmdbId}/external_ids`,trace);
    const imdb=String(ext?.imdb_id||'');
    if(/^tt\d+$/.test(imdb)){
      await sql`INSERT INTO tmdb_external_ids(tmdb_id,media_type,provider,external_id,updated_at) VALUES(${String(tmdbId)},'movie','imdb',${imdb},now()) ON CONFLICT DO NOTHING`;
      return imdb;
    }
  }catch(error){
...
export async function refreshSagas({limit=120,requestKey}={}){
  ...
  const observed=await executeObservedProcess({
    ...
    const sql=db();
    const issues={externalIdErrors:0};
    await trace.event({eventType:'step_started',step:'select_collections',message:'Seleccionando colecciones TMDb a refrescar'});
    const ids=await sql`SELECT x.tmdb_collection_id FROM (SELECT DISTINCT mc.tmdb_collection_id FROM movie_collections mc LEFT JOIN catalog_exclusions ex ON ex.imdb_id=mc.imdb_id WHERE ex.imdb_id IS NULL AND mc.tmdb_collection_id IS NOT NULL UNION SELECT sc.tmdb_collection_id FROM saga_collections sc) x LEFT JOIN saga_collections sc ON sc.tmdb_collection_id=x.tmdb_collection_id ORDER BY sc.refreshed_at ASC NULLS FIRST,x.tmdb_collection_id LIMIT ${safeLimit}`;
    ...
    const results=await pool(ids,6,async row=>{
      ...
      const d=await tmdb(`/collection/${collectionId}?language=es-ES`,trace);
      const parts=[...(d.parts||[])].sort((a,b)=>String(a.release_date||'9999').localeCompare(String(b.release_date||'9999')));
      ...
      const ops=[sql`DELETE FROM saga_collection_members WHERE tmdb_collection_id=${row.tmdb_collection_id}`];
      let pos=0;
      for(const p of parts){
        ...
        if(!m)outsideCatalog++;
        ops.push(sql`INSERT INTO saga_collection_members(tmdb_collection_id,tmdb_movie_id,imdb_id,title,original_title,year,poster_path,position,updated_at) VALUES(${row.tmdb_collection_id},${String(p.id)},${imdbId},${p.title||p.name||null},${p.original_title||p.original_name||null},${Number(String(p.release_date||'').slice(0,4))||null},${p.poster_path||null},${pos},now())`);
      }
    ...
    for(const failure of failed){
      ...
      await recordProcessError(trace.runId,{error:failure.error,step:'refresh_collection',entityType:'saga_collection',entityId:collectionId,source:failure.error?.source||'tmdb',retryable:Boolean(failure.error?.retryable),detail:{tmdb_collection_id:collectionId}}).catch(()=>{});
    }
    const collectionErrors=failed.length;
    const successfulCollections=ids.length-collectionErrors;
    await trace.event({eventType:'step_completed',step:'refresh_collections',message:'Refresco de colecciones finalizado',data:{selected:ids.length,refreshed:successfulCollections,collection_errors:collectionErrors,external_id_errors:issues.externalIdErrors,members,outside_catalog:outsideCatalog}});
    ...
    const functionalResult=ids.length===0?'no_change':successfulCollections>0?'updated':'pending';
    return{
      technicalStatus,
      functionalResult,
      metrics:{collections_selected:ids.length,collections_refreshed:successfulCollections,collections_added:added,collections_updated:updated,members,outside_catalog:outsideCatalog,collection_errors:collectionErrors,external_id_errors:issues.externalIdErrors,...stats},
      after:{collections:successfulCollections,members,outside_catalog:outsideCatalog,errors:totalErrors,...stats},
      message:totalErrors?'Sagas refrescadas con incidencias':'Sagas refrescadas correctamente'
    };
  });
