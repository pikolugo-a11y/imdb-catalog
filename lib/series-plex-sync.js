...
async function loadSectionEpisodeInventory(base,token,section,trace){
  const rows=[];let start=0,expectedTotal=null,pages=0;
  while(pages<200){
    const body=await get(base,token,`/library/sections/${section.key}/all?type=4&includeGuids=0&includeMedia=0&X-Plex-Container-Start=${start}&X-Plex-Container-Size=${EPISODE_PAGE_SIZE}`,trace);
    const container=body?.MediaContainer||{},raw=list(body).filter(x=>x.type==='episode'),normalized=raw.map(e=>normalizePlexEpisode(e,{sectionId:Number(section.key),fingerprint:fingerprint(e)}));
    if(normalized.some(x=>!x))throw Object.assign(new Error(`Plex devolvió un episodio sin identidad completa en ${section.title||section.key}`),{processStep:'episode_inventory'});
    if(expectedTotal==null){const total=Number(container.totalSize??container.TotalSize);if(Number.isFinite(total)&&total>=0)expectedTotal=total}
    rows.push(...normalized);pages++;
    const received=raw.length;start+=received;
    if(expectedTotal!=null&&start>=expectedTotal)break;
    if(received<EPISODE_PAGE_SIZE)break;
    if(received===0)break;
  }
  ...
  if(expectedTotal!=null&&rows.length<expectedTotal)throw Object.assign(new Error(`Inventario de episodios incompleto en ${section.title||section.key}: ${rows.length}/${expectedTotal}`),{processStep:'episode_inventory'});
  return rows;
}
...
async function applyEpisodeInventoryDiff({base,token,sql,episodeSections,changedShowKeys,trace}){
  ...
  const sectionIds=episodeSections.map(x=>Number(x.section)),incoming=episodeSections.flatMap(x=>x.episodes),existing=await sql`SELECT rating_key,library_section_id,plex_title,plex_year,plex_updated_at,parent_rating_key,grandparent_rating_key,parent_index,item_index,fingerprint,active FROM plex_items WHERE item_type='episode' AND library_section_id=ANY(${sectionIds})`;
  ...
  for(const row of activeAffected)await rebuildSeriesDiagnostics(sql,String(row.show_rating_key));
...
export async function syncPlexSeriesFastCore({trace=null}={}){
  ...
  for(const section of sections){
    ...
    try{
      ...
      episodeSections.push({section:Number(section.key),episodes,showKeys});await trace?.event?.({eventType:'step_completed',step:'sync_episode_inventory',entityType:'series_library',entityId:sectionKey,message:`Inventario ligero: ${episodes.length} episodios`,data:{episodes:episodes.length}});
    }catch(e){episodeFailed.push({section:sectionKey,title:sectionTitle,error:e?.message||String(e)});await audit('quality','series',sectionKey,'plex_series_fast_episode_inventory_failed',{error:e?.message||String(e)});await trace?.event?.({eventType:'error',step:'sync_episode_inventory',entityType:'series_library',entityId:sectionKey,message:e?.message||'Error leyendo inventario de episodios',data:{source:e?.source||'plex',retryable:Boolean(e?.retryable)}})}
  }
  ...
  await trace?.event?.({eventType:'step_started',step:'apply_episode_diff',message:'Aplicando sólo diferencias de episodios'});
...
export async function syncPlexSeriesDetailCore(ratingKey,{trace=null}={}){
  ...
  try{
    await trace?.event?.({eventType:'step_started',step:'discover_plex',message:'Localizando servidor Plex'});trace?.externalCall?.(1);const base=await discoverPlexUrlCore(token,process.env.PLEX_URL||process.env.PLEX_BASE_URL||'');await trace?.event?.({eventType:'step_completed',step:'discover_plex',message:'Servidor Plex localizado'});
    ...
    const detailed=[];for(let pos=0;pos<seen.length;pos+=8){const chunk=seen.slice(pos,pos+8),rows=await Promise.all(chunk.map(async episodeKey=>{const d=list(await get(base,token,`/library/metadata/${episodeKey}?includeMedia=1`,trace))[0]||{};return{rating_key:episodeKey,media:medias(d)}}));detailed.push(...rows)}
    ...
    const mediaRows=detailed.flatMap(x=>x.media.map(m=>({rating_key:x.rating_key,media_index:m.media_index,plex_media_id:m.plex_media_id,duration_ms:m.duration_ms,bitrate:m.bitrate,width:m.width,height:m.height,resolution:m.resolution,aspect_ratio:m.aspect_ratio,container:m.container,video_codec:m.videoCodec,video_profile:m.video_profile,video_frame_rate:m.video_frame_rate,video_dynamic_range:m.video_dynamic_range,audio_codec:m.audio_codec,audio_profile:m.audio_profile,audio_channels:m.audio_channels,optimized_for_streaming:m.optimized_for_streaming}))),fileRows=detailed.flatMap(x=>x.media.flatMap(m=>m.parts.filter(p=>p.file_path).map(p=>({rating_key:x.rating_key,media_index:m.media_index,part_index:p.part_index,plex_part_id:p.plex_part_id,file_path:p.file_path,file_size_bytes:p.file_size_bytes,duration_ms:p.duration_ms,container:p.container,accessible:p.accessible,exists_on_server:p.exists_on_server}))));
    if(mediaRows.length)await sql`WITH x AS(SELECT * FROM jsonb_to_recordset(${JSON.stringify(mediaRows)}::jsonb) AS t(rating_key text,media_index int,plex_media_id text,duration_ms bigint,bitrate int,width int,height int,resolution text,aspect_ratio float8,container text,video_codec text,video_profile text,video_frame_rate text,video_dynamic_range text,audio_codec text,audio_profile text,audio_channels int,optimized_for_streaming bool)) INSERT INTO plex_media SELECT * FROM x`;
    if(fileRows.length)await sql`WITH x AS(SELECT * FROM jsonb_to_recordset(${JSON.stringify(fileRows)}::jsonb) AS t(rating_key text,media_index int,part_index int,plex_part_id text,file_path text,file_size_bytes bigint,duration_ms bigint,container text,accessible bool,exists_on_server bool)) INSERT INTO plex_files SELECT * FROM x`;
