...
const PLEX_INVENTORY_TIMEOUTS=[1000000,1000000,1000000];
async function get(base,token,path,trace=null,timeouts=PLEX_REQUEST_TIMEOUTS){return fetchPlexJsonWithRetry({url:base+path,path,headers:headers(token),trace,timeouts})}
...
function mediaFileRows(media=[]){return media.flatMap(m=>m.parts.filter(p=>p.file_path).map(p=>({media_index:m.media_index,part_index:p.part_index,plex_part_id:p.plex_part_id||'',file_path:p.file_path,file_size_bytes:p.file_size_bytes??null,duration_ms:p.duration_ms??null,container:p.container||null,accessible:p.accessible??null,exists_on_server:p.exists_on_server??null})))}
function physicalFileSignature(rows=[]){return JSON.stringify(rows.map(r=>({media_index:Number(r.media_index)||0,part_index:Number(r.part_index)||0,plex_part_id:String(r.plex_part_id||''),file_path:String(r.file_path||''),file_size_bytes:r.file_size_bytes==null?null:Number(r.file_size_bytes),duration_ms:r.duration_ms==null?null:Number(r.duration_ms),container:r.container||null,accessible:r.accessible==null?null:Boolean(r.accessible),exists_on_server:r.exists_on_server==null?null:Boolean(r.exists_on_server)})).sort((a,b)=>a.media_index-b.media_index||a.part_index-b.part_index||a.file_path.localeCompare(b.file_path)))}
async function replaceEpisodeMedia({sql,detailed=[],removedKeys=[]}){
  const detailKeys=detailed.map(x=>String(x.rating_key)),affected=[...new Set([...detailKeys,...removedKeys.map(String)])];
  ...
  const mediaRows=detailed.flatMap(x=>x.media.map(m=>({rating_key:String(x.rating_key),media_index:m.media_index,plex_media_id:m.plex_media_id,duration_ms:m.duration_ms,bitrate:m.bitrate,width:m.width,height:m.height,resolution:m.resolution,aspect_ratio:m.aspect_ratio,container:m.container,video_codec:m.video_codec,video_profile:m.video_profile,video_frame_rate:m.video_frame_rate,video_dynamic_range:m.video_dynamic_range,audio_codec:m.audio_codec,audio_profile:m.audio_profile,audio_channels:m.audio_channels,optimized_for_streaming:m.optimized_for_streaming}))),fileRows=detailed.flatMap(x=>mediaFileRows(x.media).map(p=>({rating_key:String(x.rating_key),...p})));
...
async function cleanupStaleSeriesReferences(sql,trace=null){
  const stale=await sql`
    SELECT oldr.show_rating_key old_rating_key,oldr.imdb_id,pcs.rating_key new_rating_key
    FROM series_reference oldr
    JOIN plex_items oldp
      ON oldp.rating_key=oldr.show_rating_key
     AND oldp.item_type='show'
     AND oldp.active=false
    JOIN plex_catalog_status pcs
      ON pcs.imdb_id=oldr.imdb_id
     AND pcs.status='in_plex'
     AND pcs.rating_key IS NOT NULL
     AND pcs.rating_key<>oldr.show_rating_key
    JOIN plex_items newp
      ON newp.rating_key=pcs.rating_key
     AND newp.item_type='show'
     AND newp.active=true
    JOIN series_reference newr
      ON newr.show_rating_key=pcs.rating_key
     AND newr.imdb_id=oldr.imdb_id
    WHERE NOT EXISTS(
      SELECT 1 FROM plex_items ep
      WHERE ep.grandparent_rating_key=oldr.show_rating_key
        AND ep.item_type='episode'
        AND ep.active=true
    )
      AND NOT EXISTS(
        SELECT 1 FROM series_episode_overrides o
        WHERE o.show_rating_key=oldr.show_rating_key
      )
    ORDER BY oldr.show_rating_key`;
  if(!stale.length)return{removed:0,items:[]};
  const keys=stale.map(x=>String(x.old_rating_key));
  await trace?.event?.({eventType:'step_started',step:'cleanup_stale_series_references',message:`Retirando ${keys.length} referencias Plex antiguas ya sustituidas`,data:{rating_keys:keys}});
  await sql.transaction([
    sql`DELETE FROM series_episode_availability WHERE show_rating_key=ANY(${keys})`,
    sql`DELETE FROM series_diagnostics WHERE show_rating_key=ANY(${keys})`,
    sql`DELETE FROM series_season_availability WHERE show_rating_key=ANY(${keys})`,
    sql`DELETE FROM series_reference_episodes WHERE show_rating_key=ANY(${keys})`,
    sql`DELETE FROM series_quality_read_model WHERE show_rating_key=ANY(${keys})`,
    sql`DELETE FROM series_reference WHERE show_rating_key=ANY(${keys})`
  ]);
  const imdbIds=[...new Set(stale.map(x=>String(x.imdb_id)).filter(Boolean))];
  if(imdbIds.length)await recomputeLifecycleForIds(imdbIds);
  await trace?.event?.({eventType:'step_completed',step:'cleanup_stale_series_references',message:`${keys.length} referencias Plex antiguas retiradas`,data:{items:stale}});
  return{removed:keys.length,items:stale};
}

async function loadEpisodePage(base,token,section,start,trace){const body=await get(base,token,`/library/sections/${section.key}/all?type=4&includeGuids=0&includeMedia=1&X-Plex-Container-Start=${start}&X-Plex-Container-Size=${EPISODE_PAGE_SIZE}`,trace,PLEX_INVENTORY_TIMEOUTS),container=body?.MediaContainer||{},raw=list(body).filter(x=>x.type==='episode'),normalized=raw.map(e=>{const row=normalizePlexEpisode(e,{sectionId:Number(section.key),fingerprint:fingerprint(e)});return row?{...row,_media_included:Array.isArray(e?.Media),_media:medias(e)}:null});if(normalized.some(x=>!x))throw Object.assign(new Error(`Plex devolvió un episodio sin identidad completa en ${section.title||section.key}`),{processStep:'episode_inventory'});const total=Number(container.totalSize??container.TotalSize);return{normalized,received:raw.length,total:Number.isFinite(total)&&total>=0?total:null}}
...
async function refreshChangedEpisodeMedia({base,token,sql,episodeKeys,removedKeys,trace}){
  ...
  return replaceEpisodeMedia({sql,detailed,removedKeys});
}

async function applyEpisodeInventoryDiff({base,token,sql,episodeSections,changedShowKeys,trace}){
  if(!episodeSections.length)return{examined:0,created:0,changed:0,missing:0,seriesAffected:0,physicalChanged:0,mediaDetailed:0,mediaRows:0,fileRows:0,reconciled:0};
  const sectionIds=episodeSections.map(x=>Number(x.section)),incoming=episodeSections.flatMap(x=>x.episodes);
  const existing=await sql`SELECT p.rating_key,p.library_section_id,p.plex_title,p.plex_year,p.plex_updated_at,p.parent_rating_key,p.grandparent_rating_key,p.parent_index,p.item_index,p.fingerprint,p.active,EXISTS(SELECT 1 FROM plex_media m WHERE m.rating_key=p.rating_key) has_media FROM plex_items p WHERE p.item_type='episode' AND p.library_section_id=ANY(${sectionIds})`;
  const existingFiles=await sql`SELECT f.rating_key,f.media_index,f.part_index,f.plex_part_id,f.file_path,f.file_size_bytes,f.duration_ms,f.container,f.accessible,f.exists_on_server FROM plex_files f JOIN plex_items p ON p.rating_key=f.rating_key WHERE p.item_type='episode' AND p.library_section_id=ANY(${sectionIds})`;
  const filesByKey=new Map();
  for(const row of existingFiles){const key=String(row.rating_key);if(!filesByKey.has(key))filesByKey.set(key,[]);filesByKey.get(key).push(row)}
  const diff=planEpisodeInventoryDiff({incoming,existing,successfulSectionIds:sectionIds});
  const physicalChangedRows=incoming.filter(row=>row?._media_included&&physicalFileSignature(mediaFileRows(row._media))!==physicalFileSignature(filesByKey.get(String(row.rating_key))||[]));
  const affectedShowKeys=[...new Set([...diff.affectedShowKeys,...physicalChangedRows.map(x=>String(x.grandparent_rating_key)).filter(Boolean)])];
  ...
  const bulkDetailed=physicalChangedRows.map(row=>({rating_key:String(row.rating_key),media:row._media}));
  const bulkPhysical=await replaceEpisodeMedia({sql,detailed:bulkDetailed,removedKeys});
  const fallbackDetailKeys=diff.upserts.filter(row=>!row?._media_included).map(x=>String(x.rating_key));
  const fallbackPhysical=fallbackDetailKeys.length?await refreshChangedEpisodeMedia({base,token,sql,episodeKeys:fallbackDetailKeys,removedKeys:[],trace}):{detailed:0,mediaRows:0,fileRows:0};
  const physical={detailed:bulkPhysical.detailed+fallbackPhysical.detailed,mediaRows:bulkPhysical.mediaRows+fallbackPhysical.mediaRows,fileRows:bulkPhysical.fileRows+fallbackPhysical.fileRows};
  if(physicalChangedRows.length)await sql`UPDATE plex_items SET synced_at=now(),last_seen_at=now() WHERE rating_key=ANY(${physicalChangedRows.map(x=>String(x.rating_key))})`;
  const activeAffected=affectedShowKeys.length?await sql`SELECT DISTINCT r.show_rating_key,r.imdb_id FROM series_reference r JOIN plex_items p ON p.rating_key=r.show_rating_key AND p.item_type='show' AND p.active WHERE r.show_rating_key=ANY(${affectedShowKeys})`:[];
  ...
  return{examined:incoming.length,created:diff.created.length,changed:diff.changed.length,missing:diff.missing.length,seriesAffected:affectedShowKeys.length,physicalChanged:physicalChangedRows.length,mediaDetailed:physical.detailed,mediaRows:physical.mediaRows,fileRows:physical.fileRows,reconciled:activeAffected.length};
}
...
export async function syncPlexSeriesFastCore({trace=null}={}){
  ...
  for(const section of sections){
    ...
    let showKeys=[];
    try{
      const shows=list(await get(base,token,`/library/sections/${section.key}/all?includeGuids=0&X-Plex-Container-Start=0&X-Plex-Container-Size=20000`,trace,PLEX_INVENTORY_TIMEOUTS)).filter(x=>x.type==='show');examined+=shows.length;
  ...
  const staleReferences=await cleanupStaleSeriesReferences(sql,trace);
  ...
  const partial=failed.length>0||episodeFailed.length>0,result={examined,changed,created,missing,staleReferencesRemoved:staleReferences.removed,libraries:sections.length,librariesSucceeded:successful.length,librariesFailed:failed.length,episodeExamined:episode.examined,episodeCreated:episode.created,episodeChanged:episode.changed,episodeMissing:episode.missing,episodeSeriesAffected:episode.seriesAffected,episodePhysicalChanged:episode.physicalChanged,episodeMediaDetailed:episode.mediaDetailed,episodeMediaRows:episode.mediaRows,episodeFileRows:episode.fileRows,episodeReconciled:episode.reconciled,episodeLibrariesSucceeded:episodeSections.length,episodeLibrariesFailed:episodeFailed.length,durationMs:Date.now()-started,partial,failedLibraries:failed.slice(0,20),failedEpisodeLibraries:episodeFailed.slice(0,20)};
...
export async function syncPlexSeriesFast(){
  ...
  const observed=await executeObservedProcess({processCode:'PROC-SER-001',runKind:'individual',triggerSource:'calidad_series_manual',executor:'vercel',entityType:'series',entityId:'global',correlationKey:requestKey,idempotencyKey:requestKey,context:{surface:'/calidad/series',operation:'sync_plex_series_fast'}},async trace=>{
    ...
    return{technicalStatus:result.partial?'partial':'succeeded',functionalResult:result.changed||result.created||result.missing||episodeDelta?'updated':'no_change',metrics:{examined:result.examined,changed:result.changed,created:result.created,missing:result.missing,stale_references_removed:result.staleReferencesRemoved||0,libraries:result.libraries,libraries_succeeded:result.librariesSucceeded,libraries_failed:result.librariesFailed,episodes_examined:result.episodeExamined,episodes_created:result.episodeCreated,episodes_changed:result.episodeChanged,episodes_missing:result.episodeMissing,episode_files_changed:result.episodePhysicalChanged,episode_series_affected:result.episodeSeriesAffected,episode_media_detailed:result.episodeMediaDetailed,episode_libraries_failed:result.episodeLibrariesFailed},after:{partial:result.partial,failed_libraries:result.failedLibraries,failed_episode_libraries:result.failedEpisodeLibraries},message:result.partial?'Sincronización Plex de Series completada parcialmente':'Sincronización Plex de Series completada',...result};
  });
  if(observed.reused)return{examined:0,changed:0,created:0,missing:0,libraries:0,librariesSucceeded:0,librariesFailed:0,episodeExamined:0,episodeCreated:0,episodeChanged:0,episodeMissing:0,episodeSeriesAffected:0,episodePhysicalChanged:0,episodeMediaDetailed:0,episodeMediaRows:0,episodeFileRows:0,episodeReconciled:0,episodeLibrariesSucceeded:0,episodeLibrariesFailed:0,durationMs:0,partial:false,reused:true,runId:observed.runId};
