...
export async function getPeopleDashboard(filters={}){
  ...
  // Keep discovery/ranking bounded on the persisted catalog universe. Only the
  // 50 visible people are then hydrated from person_filmography. For refreshed
  // people those canonical metrics replace legacy metrics, exactly as in detail.
  const rows=await sql`
    WITH base AS (
      ...
      SELECT b.*,
        (COALESCE(b.legacy_avg_score,0)*12 + ln(1+GREATEST(b.legacy_role_movies,0))*14 + ln(1+GREATEST(COALESCE(b.popularity,0),0))*5) AS relevance_score,
      ...
      ORDER BY
        ...
        CASE WHEN ${sort}='score' THEN legacy_avg_score END DESC NULLS LAST,
    ...
    SELECT p.tmdb_person_id,p.name,p.profile_path,p.known_for_department,p.birthday,p.deathday,p.place_of_birth,p.popularity,
      p.filmography_refreshed_at,p.filmography_count,p.relevance_score,p.total_count,
    ...
    ORDER BY
      ...
      CASE WHEN ${sort}='score' THEN p.legacy_avg_score END DESC NULLS LAST,
