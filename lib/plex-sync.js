...
async function diagnosticProbe(sql,token,baseUrl){
  const [stored]=await sql`SELECT p.rating_key,p.plex_title,p.plex_year,p.plex_updated_at,p.item_type,(SELECT x.external_id FROM plex_external_ids x WHERE x.rating_key=p.rating_key AND x.provider='imdb' LIMIT 1) AS imdb FROM plex_items p WHERE p.rating_key=${DIAGNOSTIC_RATING_KEY} LIMIT 1`;
  const base=await discoverPlexUrlCore(token,baseUrl);
  const started=Date.now();
  const response=await fetch(`${base}/library/metadata/${DIAGNOSTIC_RATING_KEY}?includeGuids=1`,{headers:plexHeaders(token),cache:'no-store',signal:AbortSignal.timeout(15000)});
  if(!response.ok)throw new Error(`Diagnóstico Plex ${DIAGNOSTIC_RATING_KEY} respondió ${response.status}`);
  const body=await response.json();
  const item=(body?.MediaContainer?.Metadata||body?.MediaContainer?.Video||[])[0]||{};
  const rawGuids=(item.Guid||[]).map(g=>String(g.id||'')).filter(Boolean);
  ...
  const wouldSelectNormally=!stored||String(stored.plex_title||'')!==String(item.title||'')||Number(stored.plex_year||0)!==Number(item.year||0)||storedUpdatedAt!==liveUpdatedAt;
  const probe={mode:'READ_ONLY_FORCED_DIAGNOSTIC',rating_key:DIAGNOSTIC_RATING_KEY,stored_before:{imdb:stored?.imdb||null,title:stored?.plex_title||null,year:stored?.plex_year||null,updated_at:storedUpdatedAt,type:stored?.item_type||null},plex_live:{imdb:liveImdb,title:item.title||null,year:item.year||null,updated_at:liveUpdatedAt,raw_guids:rawGuids},normal_detector_would_select:wouldSelectNormally,normal_detector_reasons:{title_changed:Boolean(stored&&String(stored.plex_title||'')!==String(item.title||'')),year_changed:Boolean(stored&&Number(stored.plex_year||0)!==Number(item.year||0)),updated_at_changed:Boolean(stored&&storedUpdatedAt!==liveUpdatedAt)},identity_observation:stored?.imdb===liveImdb?'SAME_IMDB':stored?.imdb&&liveImdb?'IMDB_CHANGED':stored?.imdb&&!liveImdb?'IMDB_REMOVED':!stored?.imdb&&liveImdb?'IMDB_ADDED':'NO_IMDB',fetch_ms:Date.now()-started,read_only:true};
  console.log('[PLEX_DIAGNOSTIC_94028]',JSON.stringify(probe));
  return probe;
}
...
export async function syncPlexFast(){
  ...
  try{diagnostic94028=await diagnosticProbe(sql,token,baseUrl)}catch(e){diagnostic94028={mode:'READ_ONLY_FORCED_DIAGNOSTIC',rating_key:DIAGNOSTIC_RATING_KEY,error:String(e?.message||e),read_only:true};console.error('[PLEX_DIAGNOSTIC_94028]',e)}
  ...
  const diagnosticLog=`PLEX_DIAGNOSTIC_94028 ${JSON.stringify(diagnostic94028)}`;
  await sql`UPDATE plex_sync_runs SET notes=LEFT(COALESCE(notes,'')||E'\n'||${diagnosticLog},12000) WHERE id=(SELECT max(id) FROM plex_sync_runs)`;
