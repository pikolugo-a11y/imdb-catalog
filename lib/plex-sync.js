...
async function pget(base,token,path){const r=await fetch(base+path,{headers:headers(token),cache:'no-store',signal:AbortSignal.timeout(120000)});if(!r.ok)throw new Error(`Plex ${path} respondió ${r.status}`);return r.json()}
...
async function syncSection(sql,base,token,section){
  ...
  try{
    ...
    return{section:section.title,type:kind,total:items.length,new:newCount,changed:changedCount,unchanged:unchangedCount,missing:missing.length,details:changed.length,filenames:filenameCount};
...
export async function syncPlexFast(){
  ...
  return{ok:true,results,total:results.reduce((a,x)=>a+x.total,0),new:results.reduce((a,x)=>a+x.new,0),changed:results.reduce((a,x)=>a+x.changed,0),missing:results.reduce((a,x)=>a+x.missing,0),filenames:results.reduce((a,x)=>a+(x.filenames||0),0),seriesIdentityChanges};
}
