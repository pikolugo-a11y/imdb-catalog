...
export async function syncPlexFast(){
  ...
  // Snapshot de identidad ANTES de tocar Plex. Es imprescindible para distinguir
  // una corrección de identidad de una simple actualización de fichero/metadatos.
  ...
  // Recovery safety: una sección interrumpida debe volver a entrar en deep refresh.
  ...
  // Identidades históricamente incompletas también se fuerzan por la ruta normal.
  ...
  // MODO PRUEBA: sólo permitimos UNA corrección de identidad por ejecución.
  // Si Plex trae más, restauramos sus GUID anteriores para que no se procesen
  // silenciosamente; quedarán pendientes para la siguiente ejecución/prueba.
  ...
  if(selected){
    ...
    const trace={
      mode:'identity_change_probe_1',rating_key:selected.rating_key,title:item?.plex_title||null,type:item?.item_type||null,year:item?.plex_year||null,
      before:selected.before,after:selected.after,detected_at:new Date().toISOString(),deferred_identity_changes:deferred.length,
      actions:[]
    };
    trace.actions.push({step:'plex_identity_changed',ok:true});

    // La identidad antigua NO se borra del catálogo: simplemente deja de estar
    // vinculada a este ratingKey. Recalcularemos su presencia mediante plex_catalog_status.
    if(oldImdb&&oldImdb!==newImdb){
    ...
    if(newImdb){
      ...
      if(existing){
        // No borramos la obra ni datos maestros. Para una corrección de identidad
        // forzamos el inicio real del pipeline de calidad/identidad.
        ...
        trace.actions.push({step:'lifecycle_reset',imdb_id:newImdb,destination:'IDENTITY_PENDING',existing_catalog_title:true,ok:true});
      }else{
        // Si la nueva identidad aún no existe en catálogo, Novedades es realmente
        // su puerta de entrada; seedPlexNewsCandidates la recogerá en postproceso.
        trace.actions.push({step:'route_to_news',imdb_id:newImdb,destination:'NOVEDADES',existing_catalog_title:false,ok:true});
      }
    }
    identityChange=trace;
  }

  return{...result,identityChangeProbe:{limit:1,detected:changed.length,processed:identityChange?1:0,deferred:deferred.length,change:identityChange,elapsed_ms:Date.now()-started}};
}
