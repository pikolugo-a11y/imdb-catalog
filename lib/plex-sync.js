...
export async function syncPlexFast(){
  ...
  // La identidad canónica de Plex para PikoFilm es IMDb. TMDb/TVDb se refrescan y
  // se guardan, pero NUNCA disparan por sí solos un reset ni un envío a Novedades.
  // Para evitar falsos positivos por metadatos incompletos de Plex, exigimos que
  // exista IMDb antes y después, y que ambos valores sean distintos.
  for(const [ratingKey,newIds] of after){
    const oldIds=before.get(ratingKey),oldImdb=oldIds?.imdb||null,newImdb=newIds?.imdb||null;
    if(oldImdb&&newImdb&&oldImdb!==newImdb)changed.push({rating_key:ratingKey,before:oldIds,after:newIds});
  }

  // Primera validación controlada: máximo UN cambio IMDb por ejecución.
  const selected=changed[0]||null,deferred=changed.slice(1);
  // Los cambios IMDb adicionales se dejan pendientes para la siguiente ejecución.
  // Restauramos sólo el IMDb antiguo (no TMDb/TVDb) y forzamos un deep refresh
  // posterior poniendo plex_updated_at a NULL. Así no perdemos la evidencia del
  // cambio ni convertimos diferencias auxiliares en cambios de identidad.
  for(const c of deferred){
    await sql`DELETE FROM plex_external_ids WHERE rating_key=${c.rating_key} AND provider='imdb'`;
    if(c.before?.imdb)await sql`INSERT INTO plex_external_ids(rating_key,provider,external_id) VALUES(${c.rating_key},'imdb',${c.before.imdb}) ON CONFLICT DO NOTHING`;
    await sql`UPDATE plex_items SET plex_updated_at=NULL WHERE rating_key=${c.rating_key}`;
  }
  ...
  if(selected){
    ...
    const log=`IMDB_IDENTITY_CHANGE_PROBE ${JSON.stringify(trace)}`;
