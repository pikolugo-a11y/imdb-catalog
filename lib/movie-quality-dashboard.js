...
export async function getMovieQualityDashboard(filters={}){
  ...
  return{rows:rows.map(({total_count,...r})=>r),summary:{pending_analysis:pendingCount,review_affected:reviewAffected,affected:pendingCount+reviewAffected,pending:Number(review?.findings||0),priority:Number(review?.priority||0),last_analysis:review?.last_analysis||null,types},settings,total,page,pages:Math.max(1,Math.ceil(total/pageSize)),pageSize};
}
