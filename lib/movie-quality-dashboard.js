...
export async function getMovieQualityDashboard(filters={}){
  const sql=db(),q=String(filters.q||'').trim().toLowerCase(),type=String(filters.type||''),rawStatus=filters.status===undefined?'pending':String(filters.status||''),status=rawStatus==='all'?'':rawStatus,risk=String(filters.risk||''),page=Math.max(1,Number(filters.page)||1),pageSize=10,offset=(page-1)*pageSize;
