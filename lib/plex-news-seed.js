...
const hasMinimums=r=>Boolean(r.imdb_id&&r.title&&r.year&&r.candidate_type&&r.imdb_rating!=null&&r.imdb_votes!=null);
...
export async function seedPlexNewsCandidates({resolveLimit=25}={}){
  ...
  const normalized=rows.map(r=>({
    ...r,
    title:r.source_snapshot?.title||r.plex_title||r.imdb_id,
    year:r.year||r.plex_year||null,
    candidate_type:r.candidate_type||toCandidateType(r.item_type)
  }));
  ...
  const needDataset=pending.filter(r=>{
    const d=omdb.get(r.imdb_id);return (r.imdb_rating??d?.imdb_rating)==null||(r.imdb_votes??d?.imdb_votes)==null
  }).map(r=>r.imdb_id);
  const ratings=await imdbRatingsFromOfficialDataset(needDataset,{timeoutMs:30000});

  for(const r of pending){
    try{
      const d=omdb.get(r.imdb_id)||null,rating=ratings.get(r.imdb_id)||null;
      const final={
        imdb_id:r.imdb_id,
        title:d?.title||r.title||r.plex_title||r.imdb_id,
        year:d?.year||r.year||r.plex_year||null,
        candidate_type:d?.candidate_type||r.candidate_type||toCandidateType(r.item_type),
        imdb_rating:r.imdb_rating??d?.imdb_rating??rating?.rating??null,
        imdb_votes:r.imdb_votes??d?.imdb_votes??rating?.votes??null
      };
      const ready=hasMinimums(final);
      const snap={origin:'plex',matchedRule:'plex',title:final.title,originalTitle:final.title,plexRatingKey:r.rating_key,plexDetectedAt:new Date().toISOString(),omdbStatus:d?'complete':'unavailable',omdbResolvedAt:d?new Date().toISOString():null,minimums:{imdb:true,title:Boolean(final.title&&final.title!==r.imdb_id),year:Boolean(final.year),type:Boolean(final.candidate_type),rating:final.imdb_rating!=null,votes:final.imdb_votes!=null},imdb_ratings_source:rating?.source||null,imdb_ratings_updated_at:rating?new Date().toISOString():null};
