...
export async function enrichTitleTmdbOnly(internalId){
  ...
  const ops=[
    sql`UPDATE movies SET type=${row.type==='Miniserie'?'Miniserie':'Serie'},title=${d.title},title_es=${d.title},original_title=${d.originalTitle},year=${d.year},runtime=${d.runtime},country=${d.country},final_rating=${d.rating},imdb_rating=NULL,imdb_votes=NULL,imdb_url=NULL,fa_id=NULL,fa_rating=NULL,fa_votes=NULL,fa_url=NULL,tmdb_id=${d.id},tmdb_rating=${d.rating},tmdb_votes=${d.votes},tmdb_url=${`https://www.themoviedb.org/tv/${d.id}`},wikidata_id=NULL,source_status=${sourceStatus}::jsonb,source_generated_at=now(),synced_at=now(),poster_path=${d.poster},backdrop_path=${d.backdrop},artwork_synced_at=now(),artwork_source='tmdb' WHERE imdb_id=${internalId}`,
