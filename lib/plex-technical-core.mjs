...
export function buildProbeFingerprint(item){
  ...
  const reduced={
    rating_key:snapshot.rating_key,
    updated_at:n(item?.updatedAt),
    medias:snapshot.medias.map(m=>({
      id:m.plex_media_id,duration:m.duration_ms,bitrate:m.bitrate,width:m.width,height:m.height,
      video_codec:m.video_codec,audio_codec:m.audio_codec,
      parts:m.parts.map(p=>({id:p.plex_part_id,file:p.file_path,size:p.file_size_bytes,duration:p.duration_ms,container:p.container}))
    }))
  };
