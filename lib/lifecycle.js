...
export const LIFECYCLE={
  ...
  PIKOSCORE_PENDING:{label:'Listo para PikoScore',area:'/calidad/datos',tone:'warn'},
...
function dataComplete(r){return [present(r.title_es),present(r.original_title),Number(r.year)>1800,present(r.type),positive(r.imdb_rating),positive(r.imdb_votes),positive(r.fa_rating),positive(r.fa_votes),positive(r.tmdb_rating),positive(r.tmdb_votes),positive(r.runtime),present(r.country),Boolean(r.has_genres),present(r.overview),Boolean(r.poster_path||r.external_poster_url)].every(Boolean)}
...
export function classifyLifecycle(r){
  ...
  if(!positive(r.final_rating))return 'PIKOSCORE_PENDING';
  ...
  if(r.tech_issue)return 'TECH_REVIEW';return 'COMPLETE';
}

function blockingReason(r,state){
  if(state==='EXCLUDED')return 'Título excluido';
  if(state==='IDENTITY_PENDING'){const missing=[];if(!r.tmdb_id)missing.push('TMDb');if(!r.fa_id)missing.push('FilmAffinity');return missing.length?`Falta identidad: ${missing.join(', ')}`:'IMDb inválido o ausente'}
  ...
  if(state==='PIKOSCORE_PENDING')return 'Datos completos; falta calcular PikoScore';
  ...
  if(state==='TECH_REVIEW')return 'Incidencia de calidad técnica';return null;
}
...
async function rawRows(ids=null){const sql=db();return sql`
  SELECT m.imdb_id,m.type,m.title_es,m.original_title,m.year,m.runtime,m.country,m.final_rating,m.imdb_rating,m.imdb_votes,m.fa_id,m.fa_rating,m.fa_votes,m.tmdb_id,m.tmdb_rating,m.tmdb_votes,m.poster_path,mm.overview,mm.release_date,iv.validation_status,(ex.imdb_id IS NOT NULL) excluded,
    ...
    COALESCE(m.source_status #>> '{data_quality_external_poster,url}','') external_poster_url,COALESCE(m.source_status #>> '{data_quality_external_director,value}','') external_director,COALESCE(m.source_status #>> '{data_quality_external_cast,value}','') external_cast,
  ...
  FROM movies m LEFT JOIN movie_metadata mm USING(imdb_id) LEFT JOIN identity_validation iv USING(imdb_id) LEFT JOIN catalog_exclusions ex USING(imdb_id) LEFT JOIN plex_catalog_status pcs USING(imdb_id) LEFT JOIN piko_quality pq ON pq.rating_key=pcs.rating_key
...
export async function getLifecycleForIds(ids=[]){
  ...
  const now=Date.now(),refreshIds=clean.filter(id=>{const v=map.get(id);return !v||!v.computed_at||(now-new Date(v.computed_at).getTime())>MAX_AGE_MS});
  if(refreshIds.length){const computed=await recomputeLifecycleForIds(refreshIds);for(const[id,v]of computed)map.set(id,v)}return map;
}
