import 'server-only';
import {db} from './db';
import {isPikoScoreDue} from './pikoscore';
...
export function classifyLifecycle(r){
  ...
  if(isPikoScoreDue(r))return 'PIKOSCORE_PENDING';
...
function blockingReason(r,state){
  ...
  if(state==='PIKOSCORE_PENDING')return 'Datos completos; PikoScore pendiente o caducado';
