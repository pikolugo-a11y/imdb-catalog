...
async function rawRows(ids=null){
  ...
  const rows=await sql`
    SELECT m.imdb_id,m.type,m.title_es,m.original_title,m.year,m.runtime,m.country,m.final_rating,
      ...
      (ex.imdb_id IS NOT NULL) excluded,pcs.status plex_status,pcs.rating_key,pq.status pq_status,
      ...
      EXISTS(SELECT 1 FROM movie_quality_findings f WHERE f.imdb_id=m.imdb_id AND f.status IN('open','waiting_sync','exception')) tech_issue,
