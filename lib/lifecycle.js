...
function blockingReason(r,state){
  ...
  if(state==='IDENTITY_PENDING'){
    const missing=[];if(!r.tmdb_id)missing.push('TMDb');if(!r.fa_id)missing.push('FilmAffinity');
    return missing.length?`Falta identidad: ${missing.join(', ')}`:'IMDb inválido o ausente';
  }
  ...
  if(state==='TECH_PENDING')return 'Calidad técnica pendiente';
  if(state==='TECH_REVIEW')return 'Incidencia de calidad técnica';
  return null;
}
...
async function rawRows(ids=null){
  ...
  return sql`
    SELECT m.imdb_id,m.type,m.title_es,m.original_title,m.year,m.runtime,m.country,m.final_rating,
      ...
      mm.overview,mm.release_date,iv.validation_status,(ex.imdb_id IS NOT NULL) excluded,
...
export async function recomputeLifecycleForIds(ids=[]){
  const clean=[...new Set(ids.filter(x=>/^tt\d+$/.test(String(x||''))))];
  if(!clean.length)return new Map();
  const sql=db(),rows=await rawRows(clean),out=new Map();
  for(const r of rows){
    const state=classifyLifecycle(r),reason=blockingReason(r,state);
    const [saved]=await sql`INSERT INTO catalog_lifecycle(imdb_id,lifecycle_state,previous_state,blocking_reason,state_changed_at,computed_at)
      VALUES(${r.imdb_id},${state},NULL,${reason},now(),now())
      ON CONFLICT(imdb_id) DO UPDATE SET
        previous_state=CASE WHEN catalog_lifecycle.lifecycle_state<>EXCLUDED.lifecycle_state THEN catalog_lifecycle.lifecycle_state ELSE catalog_lifecycle.previous_state END,
        lifecycle_state=EXCLUDED.lifecycle_state,
        blocking_reason=EXCLUDED.blocking_reason,
        state_changed_at=CASE WHEN catalog_lifecycle.lifecycle_state<>EXCLUDED.lifecycle_state THEN now() ELSE catalog_lifecycle.state_changed_at END,
        computed_at=now()
      RETURNING imdb_id,lifecycle_state,previous_state,blocking_reason,state_changed_at,computed_at`;
    out.set(r.imdb_id,{...saved,state:saved.lifecycle_state,...LIFECYCLE[saved.lifecycle_state]});
  }
  return out;
}

export async function getLifecycleForIds(ids=[]){
  const clean=[...new Set(ids.filter(Boolean))];if(!clean.length)return new Map();
  const sql=db();
  ...
  const map=new Map(rows.map(r=>[r.imdb_id,{...r,state:r.lifecycle_state,...LIFECYCLE[r.lifecycle_state]}]));
  const missing=clean.filter(id=>!map.has(id));
  if(missing.length){const computed=await recomputeLifecycleForIds(missing);for(const [id,v] of computed)map.set(id,v)}
  return map;
}
...
export async function reconcileLifecycleBatch({after=null,limit=500}={}){
  const sql=db();
  const rows=await sql`SELECT imdb_id FROM movies WHERE (${after}::text IS NULL OR imdb_id>${after}) ORDER BY imdb_id LIMIT ${Math.max(1,Math.min(Number(limit)||500,2000))}`;
  const ids=rows.map(r=>r.imdb_id);await recomputeLifecycleForIds(ids);
  return{processed:ids.length,last:ids.at(-1)||null,done:ids.length===0};
}

export async function getLifecycleSnapshot(){
  const sql=db(),counts={};
  const rows=await sql`SELECT lifecycle_state,count(*)::int count FROM catalog_lifecycle GROUP BY lifecycle_state`;
  for(const r of rows)counts[r.lifecycle_state]=Number(r.count||0);
  const [totals]=await sql`SELECT count(*)::int total FROM movies`;
  const materialized=Object.values(counts).reduce((a,b)=>a+Number(b||0),0);
  return{total:Number(totals?.total||0),materialized,missing:Math.max(0,Number(totals?.total||0)-materialized),counts,states:Object.entries(LIFECYCLE).map(([state,meta])=>({state,...meta,count:counts[state]||0}))};
}
