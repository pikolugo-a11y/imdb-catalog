...
function dataComplete(r){return [present(r.title_es),present(r.original_title),Number(r.year)>1800,positive(r.imdb_rating),positive(r.imdb_votes),positive(r.fa_rating),positive(r.fa_votes),positive(r.tmdb_rating),positive(r.tmdb_votes),positive(r.final_rating),positive(r.runtime),present(r.country),Boolean(r.has_genres),present(r.overview),Boolean(r.poster_path||r.external_poster_url),present(r.release_date),Boolean(r.has_director||r.external_director),Boolean(r.has_cast||r.external_cast)].every(Boolean)}
