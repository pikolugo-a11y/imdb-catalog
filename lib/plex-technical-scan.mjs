...
const canonicalPhysical=parts=>(Array.isArray(parts)?parts:[]).map(p=>({
  id:p?.id==null?null:String(p.id),
  file:p?.file==null?null:String(p.file),
  size:p?.size==null?null:Number(p.size),
  duration:p?.duration==null?null:Number(p.duration),
...
function probeRow(item,section){
  return{
    ...
    physical_identity:buildPhysicalIdentity(item),
  ...
  const inserted=await sql`
    WITH x AS (
      SELECT * FROM jsonb_to_recordset(${payload}::jsonb) AS t(
        ...
        parent_rating_key text,grandparent_rating_key text,parent_index int,item_index int,added_at timestamptz
      )
    )
...
async function upsertProbeChunk(sql,rows){
  if(!rows.length)return{created:0,changed:0,migrated:0};
  await ensurePhysicalItems(sql,rows);
  const payload=JSON.stringify(rows);
  const existing=await sql`
    WITH x AS (SELECT * FROM jsonb_to_recordset(${payload}::jsonb) AS t(rating_key text))
    SELECT s.rating_key,s.probe_fingerprint,s.technical_fingerprint,s.snapshot_status,
      COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          'id',f.plex_part_id,'file',f.file_path,'size',f.file_size_bytes,'duration',f.duration_ms
        ) ORDER BY f.media_index,f.part_index)
        FROM plex_files f WHERE f.rating_key=s.rating_key
      ),'[]'::jsonb) AS physical_identity
    FROM plex_technical_state s JOIN x USING(rating_key)
  `;
  const old=new Map(existing.map(r=>[String(r.rating_key),r]));
  const createdKeys=[],changedKeys=[],migratedKeys=[];
  const prepared=rows.map(r=>{
    const prev=old.get(r.rating_key);
    if(!prev){createdKeys.push(r.rating_key);return{...r,probe_changed:true};}
    if(prev.probe_fingerprint===r.probe_fingerprint)return{...r,probe_changed:false};
    ...
    return{...r,probe_changed:physicalChanged};
  });
  const preparedPayload=JSON.stringify(prepared);
  await sql`
    WITH x AS (
      SELECT * FROM jsonb_to_recordset(${preparedPayload}::jsonb) AS t(rating_key text,probe_fingerprint text,probe_changed boolean)
    )
    ...
    ON CONFLICT(rating_key) DO UPDATE SET
      ...
      snapshot_status=CASE
        WHEN x.probe_changed THEN CASE WHEN plex_technical_state.technical_fingerprint IS NULL THEN 'pending' ELSE 'stale' END
        ELSE plex_technical_state.snapshot_status
      END,
      needs_refresh=CASE WHEN x.probe_changed THEN true ELSE plex_technical_state.needs_refresh END,
      last_probe_at=now(),
      last_error=CASE WHEN x.probe_changed THEN NULL ELSE plex_technical_state.last_error END,
      updated_at=now()
  `;
  if(changedKeys.length){
    await sql`UPDATE piko_quality SET status='stale',updated_at=now() WHERE rating_key=ANY(${changedKeys}) AND status='evaluated'`;
  }
  return{created:createdKeys.length,changed:changedKeys.length,migrated:migratedKeys.length};
}

async function scanSectionPaged({sql,base,token,section,pageSize=2000}){
  const started=Date.now();
  let start=0,total=0,created=0,changed=0,migrated=0,pages=0;
  ...
  for(;;){
    ...
    total+=rows.length;created+=result.created;changed+=result.changed;migrated+=result.migrated;pages++;
    start+=items.length;
    if(items.length===0||items.length<pageSize||(expected>0&&start>=expected))break;
  }
  const elapsedMs=Date.now()-started;
  return{total,created,changed,migrated,pages,elapsed_ms:elapsedMs,items_per_second:elapsedMs?Number((total/(elapsedMs/1000)).toFixed(1)):0};
}
...
export async function scanPlexTechnicalLibrary({sql,token,baseUrl='',pageSize=2000}){
  ...
  let movies=0,episodes=0,changed=0,created=0,migrated=0,pages=0;
  const section_stats=[];
  for(const section of sections){
    ...
    created+=result.created;changed+=result.changed;migrated+=result.migrated;pages+=result.pages;
    section_stats.push({section:section.title,type:section.type,...result});
  }
  const total=movies+episodes;
  const elapsedMs=Date.now()-started;
  return{sections:sections.length,movies,episodes,total,changed,created,migrated,pages,page_size:pageSize,elapsed_ms:elapsedMs,items_per_second:elapsedMs?Number((total/(elapsedMs/1000)).toFixed(1)):0,section_stats};
}
