...
export const GDELT_MIN_INTERVAL_MS=Math.max(1000,Number(process.env.PIKORELEVANCE_GDELT_MIN_INTERVAL_MS)||15000);
export const GDELT_RETRY_BASE_MS=30000;
let gdeltQueue=Promise.resolve(),gdeltLastStartedAt=0;
async function withGdeltRateLimit(trace,fn){
  let release;
...
async function gdeltSpain({title,originalTitle,trace}={}){
  ...
  const r=await withGdeltRateLimit(trace,()=>resilientJson(u.toString(),{timeoutMs:20000,retries:2,trace,baseBackoffMs:GDELT_RETRY_BASE_MS,maxBackoffMs:180000}));
  if(!r.ok)return{status:'error',reason:r.error};
