...
export const PIKORELEVANCE_VERSION='0.3.0';
...
const DEFAULT_CONFIG={
  weights:{
    ...
    momentum:7,
    origin_es:2,
    spanish_accessibility:9,
  },
  thresholds:{very_high:80,high:65,medium:50,low:35,min_confidence:60},
  reviewDays:{active:30,age1:30,age3:60,age5:120,age10:180,age20:365,age30:730,unknown:90},
};
...
function mergeConfig(base,extra){
  ...
  delete incomingWeights.language_es;
  delete incomingWeights.spanish_translation;
  return{
    ...base,...e,
    weights:{...base.weights,...incomingWeights},
    thresholds:{...base.thresholds,...(e.thresholds||{})},
    reviewDays:{...base.reviewDays,...(e.reviewDays||{})},
  };
}
...
export function buildMediaCloudQuery(title,originalTitle){
  ...
  return `${titleBlock} AND serie AND language:es`;
}
export function parseMediaCloudCount(body){
  const relevant=Number(body?.count?.relevant ?? body?.count);
  const total=Number(body?.count?.total);
  if(!Number.isFinite(relevant)||relevant<0)return null;
  return{
    relevant,
...
async function mediaCloudSpain({title,originalTitle,trace}={}){
  ...
  const count=parseMediaCloudCount(r.body);
  if(!count)return{status:'error',reason:'invalid_count_response'};
  return{
    status:'ok',
    mentions:count.relevant,
    total_stories:count.total,
...
export function scorePikoRelevanceV0(input,config=DEFAULT_CONFIG){
  ...
  addFactor(f,'mediacloud_es',{weight:w.mediacloud_es,available:mediaCloudOk,points:points.press(mediacloud.mentions,w.mediacloud_es),evidence:mediaCloudOk?{mentions:mediacloud.mentions,total_stories:mediacloud.total_stories,collection_id:mediacloud.collection_id,query:mediacloud.query,start_date:mediacloud.start_date,end_date:mediacloud.end_date}:null});
  ...
  const originalSpanish=String(tmdb.original_language||'').toLowerCase()==='es';
  const spanishAccessible=originalSpanish||Boolean(tmdb.spanish_translation);
  addFactor(f,'spanish_accessibility',{weight:w.spanish_accessibility,available:tmdbOk,points:spanishAccessible?w.spanish_accessibility:0,evidence:tmdbOk?{original_language:tmdb.original_language||null,translated:Boolean(tmdb.spanish_translation),accessible:spanishAccessible}:null});
...
export function pikoRelevanceReviewDays({year,status,now=new Date(),reviewDays=DEFAULT_CONFIG.reviewDays}={}){
  const s=String(status||'').toLowerCase();
  if(['returning series','in production','planned'].includes(s))return Number(reviewDays.active)||30;
  const y=Number(year),current=new Date(now).getUTCFullYear();
  if(!Number.isFinite(y)||y<1800)return Number(reviewDays.unknown)||90;
  const age=Math.max(0,current-y);
  if(age<=1)return Number(reviewDays.age1)||30;
  if(age<=3)return Number(reviewDays.age3)||60;
  if(age<=5)return Number(reviewDays.age5)||120;
  if(age<=10)return Number(reviewDays.age10)||180;
  if(age<=20)return Number(reviewDays.age20)||365;
  if(age<=30)return Number(reviewDays.age30)||730;
  return null;
}
export function nextPikoRelevanceReviewAt({year,status,now=new Date(),reviewDays=DEFAULT_CONFIG.reviewDays}={}){
  const days=pikoRelevanceReviewDays({year,status,now,reviewDays});
...
export async function computePikoRelevanceCanonical(sql,imdbId,{trace=null}={}){
  ...
  const scored=scorePikoRelevanceV0(input,config),assessedAt=nowIso(),next=nextPikoRelevanceReviewAt({year:basic.year,status:tmdb.series_status,now:new Date(assessedAt),reviewDays:config.reviewDays});
