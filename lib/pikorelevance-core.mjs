...
const GDELT_MIN_INTERVAL_MS=Math.max(1000,Number(process.env.PIKORELEVANCE_GDELT_MIN_INTERVAL_MS)||15000);
...
async function gdeltSpain({title,originalTitle,trace}={}){
  ...
  const r=await withGdeltRateLimit(trace,()=>resilientJson(u.toString(),{timeoutMs:20000,retries:2,trace,baseBackoffMs:30000,maxBackoffMs:180000}));
  if(!r.ok)return{status:'error',reason:r.error};
