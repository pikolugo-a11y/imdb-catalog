...
async function resilientJson(url,{headers={},timeoutMs=10000,retries=1,trace=null,baseBackoffMs=1000,maxBackoffMs=60000,fetchImpl=fetch,sleepFn=sleep}={}){
  ...
  for(let attempt=0;attempt<=retries;attempt++){
    ...
    try{
      ...
      await sleepFn(retryDelayMs({attempt,retryAfter,baseBackoffMs,maxBackoffMs}));
    }catch(error){
      last=error;
      if(attempt>=retries)break;
      await sleepFn(retryDelayMs({attempt,baseBackoffMs,maxBackoffMs}));
...
const GDELT_MIN_INTERVAL_MS=Math.max(1000,Number(process.env.PIKORELEVANCE_GDELT_MIN_INTERVAL_MS)||5000);
let gdeltQueue=Promise.resolve(),gdeltLastStartedAt=0;
async function withGdeltRateLimit(fn){
  let release;
  const previous=gdeltQueue;
  gdeltQueue=new Promise(resolve=>{release=resolve});
  await previous;
  try{
    const waitMs=Math.max(0,GDELT_MIN_INTERVAL_MS-(Date.now()-gdeltLastStartedAt));
    if(waitMs)await sleep(waitMs);
    gdeltLastStartedAt=Date.now();
    return await fn();
  }finally{release()}
}
...
async function gdeltSpain({title,originalTitle,trace}={}){
  ...
  const r=await withGdeltRateLimit(()=>resilientJson(u.toString(),{timeoutMs:20000,retries:2,trace,baseBackoffMs:5000,maxBackoffMs:120000}));
  if(!r.ok)return{status:'error',reason:r.error};
