...
export async function getSagasDashboard(filters={}){
  ...
  const raw=await sql`
    ...
    SELECT sc.tmdb_collection_id,sc.name,sc.poster_path,sc.backdrop_path,
           count(m.*)::int total,
           count(m.*) FILTER(WHERE NOT m.not_yet_actionable)::int actionable_total,
           count(m.*) FILTER(WHERE m.not_yet_actionable)::int not_yet_available,
           count(m.*) FILTER(WHERE m.effective_status='in_plex')::int owned,
           count(m.*) FILTER(WHERE m.effective_status='acquiring')::int acquiring,
           count(m.*) FILTER(WHERE m.effective_status IS DISTINCT FROM 'in_plex' AND NOT m.not_yet_actionable)::int missing,
           round((100.0*count(m.*) FILTER(WHERE m.effective_status='in_plex')/NULLIF(count(m.*) FILTER(WHERE NOT m.not_yet_actionable),0))::numeric,1) pct,
           ...
           round(((100.0*count(m.*) FILTER(WHERE m.effective_status='in_plex')/NULLIF(count(m.*) FILTER(WHERE NOT m.not_yet_actionable),0))-(count(m.*) FILTER(WHERE m.effective_status IS DISTINCT FROM 'in_plex' AND NOT m.not_yet_actionable)*4)+COALESCE(avg(m.current_score) FILTER(WHERE m.effective_status IS DISTINCT FROM 'in_plex' AND NOT m.not_yet_actionable),0)*2)::numeric,1) completion_score
