...
export async function getSagasDashboard(filters={}){
  ...
  const raw=await sql`
    ...
    SELECT sc.tmdb_collection_id,sc.name,sc.poster_path,sc.backdrop_path,
           ...
           round(((100.0*count(m.*) FILTER(WHERE m.in_catalog AND m.effective_status='in_plex')/NULLIF(count(m.*) FILTER(WHERE m.in_catalog AND NOT m.not_yet_actionable),0))-(count(m.*) FILTER(WHERE m.in_catalog AND m.effective_status IS DISTINCT FROM 'in_plex' AND NOT m.not_yet_actionable)*4)+COALESCE(avg(m.current_score) FILTER(WHERE m.in_catalog AND m.effective_status IS DISTINCT FROM 'in_plex' AND NOT m.not_yet_actionable),0)*2)::numeric,1) completion_score
...
export async function getSagaDetailV3(id){
  ...
  const titles=await sql`
    SELECT sm.*,c.display_title,c.effective_status,c.resolution,c.poster_path catalog_poster,c.rating_key,
           ...
           mm.release_date,mm.tmdb_status,
           CASE WHEN sm.imdb_id IS NULL OR ex.imdb_id IS NOT NULL THEN false WHEN c.effective_status='in_plex' THEN false WHEN mm.release_date IS NOT NULL AND mm.release_date > current_date THEN true WHEN COALESCE(mm.tmdb_status,'') <> '' AND lower(mm.tmdb_status) <> 'released' THEN true WHEN mm.release_date IS NOT NULL AND mm.release_date > current_date - interval '90 days' THEN true ELSE false END AS not_yet_actionable,
           CASE WHEN sm.imdb_id IS NULL OR ex.imdb_id IS NOT NULL THEN 'outside_catalog' WHEN mm.release_date IS NOT NULL AND mm.release_date > current_date THEN 'upcoming' WHEN COALESCE(mm.tmdb_status,'') <> '' AND lower(mm.tmdb_status) <> 'released' THEN 'upcoming' WHEN mm.release_date IS NOT NULL AND mm.release_date > current_date - interval '90 days' THEN 'recent' ELSE 'available' END AS availability_phase,
