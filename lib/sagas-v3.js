...
export async function getSagasDashboard(filters={}){
  ...
  const raw=await sql`
    WITH members AS (
      SELECT sm.tmdb_collection_id,sm.imdb_id,sm.poster_path,sm.year,
             (sm.imdb_id IS NOT NULL AND iv.validation_status='valid' AND ex.imdb_id IS NULL) AS in_catalog,
             c.effective_status,COALESCE(m.final_rating,c.final_rating) current_score,
             mm.release_date,mm.tmdb_status,
             CASE
               WHEN sm.imdb_id IS NULL OR iv.validation_status IS DISTINCT FROM 'valid' OR ex.imdb_id IS NOT NULL THEN false
  ...
  const stats={
    ...
    not_yet_available:all.reduce((a,r)=>a+n(r.not_yet_available),0),
...
export async function getSagaDetailV3(id){
  ...
  const titles=await sql`
    SELECT sm.*,c.display_title,c.effective_status,c.resolution,c.poster_path catalog_poster,c.rating_key,
           ...
           END AS not_yet_actionable,
           CASE
             WHEN sm.imdb_id IS NULL OR iv.validation_status IS DISTINCT FROM 'valid' OR ex.imdb_id IS NOT NULL THEN 'outside_catalog'
             ...
             WHEN mm.release_date IS NOT NULL AND mm.release_date > current_date - interval '90 days' THEN 'recent'
             ELSE 'available'
