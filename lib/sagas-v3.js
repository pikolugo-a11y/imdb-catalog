...
export async function getSagaDetailV3(id){
  ...
  const titles=await sql`
    SELECT sm.*,c.display_title,c.effective_status,c.resolution,c.poster_path catalog_poster,c.rating_key,
           (m.imdb_id IS NOT NULL AND ex.imdb_id IS NULL) AS in_catalog,
           COALESCE(m.final_rating,c.final_rating) final_rating,mm.release_date,mm.tmdb_status,
           sm.imdb_id external_imdb_id,cc.eligibility_status novedades_status,
           CASE WHEN m.imdb_id IS NULL OR ex.imdb_id IS NOT NULL THEN false WHEN c.effective_status='in_plex' THEN false WHEN mm.release_date IS NOT NULL AND mm.release_date>current_date THEN true WHEN COALESCE(mm.tmdb_status,'')<>'' AND lower(mm.tmdb_status)<>'released' THEN true WHEN mm.release_date IS NOT NULL AND mm.release_date>current_date-interval '90 days' THEN true ELSE false END not_yet_actionable,
           CASE WHEN m.imdb_id IS NULL OR ex.imdb_id IS NOT NULL THEN 'outside_catalog' WHEN mm.release_date IS NOT NULL AND mm.release_date>current_date THEN 'upcoming' WHEN COALESCE(mm.tmdb_status,'')<>'' AND lower(mm.tmdb_status)<>'released' THEN 'upcoming' WHEN mm.release_date IS NOT NULL AND mm.release_date>current_date-interval '90 days' THEN 'recent' ELSE 'available' END availability_phase,
