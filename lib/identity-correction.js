...
export async function correctIdentityIds({oldImdbId,newImdbId,tmdbId,trace=null}){
  ...
  if(newTmdb){
    ...
    if(!verification.ok){await trace?.event?.({eventType:'step_warning',step:'validate_identity_ids',message:'TMDb corresponde a un IMDb distinto',data:{actual_imdb_id:verification.actualImdbId}});return{changed:false,blocked:true,reason:'mismatch',verification,before:{imdb_id:oldId,tmdb_id:before.tmdb_id?String(before.tmdb_id):null}}}
  }
  ...
  return{changed:true,savedImdbId,verification,before:{imdb_id:oldId,tmdb_id:before.tmdb_id?String(before.tmdb_id):null},after:{imdb_id:savedImdbId,tmdb_id:newTmdb},lifecycle:next};
}
