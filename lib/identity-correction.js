...
export async function correctIdentityIds({oldImdbId,newImdbId,tmdbId,trace=null,forceMismatch=false,tmdbOnly=false}){
  ...
  const modePatch=useTmdbOnly?JSON.stringify({identity_mode:'tmdb_only',identity_mode_set_at:new Date().toISOString(),identity_mode_source:'manual'}):null;
  if(useTmdbOnly){
    ...
    await sql`UPDATE movies SET source_status=(COALESCE(source_status,'{}'::jsonb)-'identity_mode'-'identity_mode_set_at'-'identity_mode_source'),imdb_url=${`https://www.imdb.com/title/${savedImdbId}/`},synced_at=now() WHERE imdb_id=${savedImdbId}`;
  }
