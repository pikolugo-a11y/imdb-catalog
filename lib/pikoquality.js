  ...
  try{
    ...
    const seasons=await sql`
      WITH scored AS (
        ...
        SELECT grandparent_rating_key,parent_index,CAST(count(*) AS integer) analyzed_count,CAST(round(CAST(0.75*percentile_cont(0.5) WITHIN GROUP(ORDER BY score)+0.25*percentile_cont(0.1) WITHIN GROUP(ORDER BY score) AS numeric)) AS integer) score FROM scored GROUP BY grandparent_rating_key,parent_index
      )
    ...
    const shows=await sql`
      ...
      agg AS (SELECT grandparent_rating_key,CAST(count(*) AS integer) analyzed_count,CAST(round(CAST(0.75*percentile_cont(0.5) WITHIN GROUP(ORDER BY score)+0.25*percentile_cont(0.1) WITHIN GROUP(ORDER BY score) AS numeric)) AS integer) score FROM scored GROUP BY grandparent_rating_key)
