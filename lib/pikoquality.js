  ...
  try{
    ...
    const seasons=await sql`
      WITH scored AS (
        ...
        SELECT grandparent_rating_key,parent_index,count(*)::int total_count FROM plex_items WHERE active AND item_type='episode' GROUP BY grandparent_rating_key,parent_index
      ), agg AS (
        SELECT grandparent_rating_key,parent_index,count(*)::int analyzed_count,round((0.75*percentile_cont(0.5) WITHIN GROUP(ORDER BY score)+0.25*percentile_cont(0.1) WITHIN GROUP(ORDER BY score)))::numeric)::int score FROM scored GROUP BY grandparent_rating_key,parent_index
      )
    ...
    const shows=await sql`
      ...
      ), totals AS (SELECT grandparent_rating_key,count(*)::int total_count FROM plex_items WHERE active AND item_type='episode' GROUP BY grandparent_rating_key),
      agg AS (SELECT grandparent_rating_key,count(*)::int analyzed_count,round((0.75*percentile_cont(0.5) WITHIN GROUP(ORDER BY score)+0.25*percentile_cont(0.1) WITHIN GROUP(ORDER BY score)))::numeric)::int score FROM scored GROUP BY grandparent_rating_key)
...
export async function qualitySummary(){
  ...
  const distribution=await sql`SELECT band,count(*)::int count FROM piko_quality q JOIN plex_items p ON p.rating_key=q.rating_key WHERE p.active AND q.status='evaluated' AND q.formula_version=${QUALITY_VERSION} AND q.source_fingerprint=p.fingerprint GROUP BY band`;
