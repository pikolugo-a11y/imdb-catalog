import 'server-only';
import {db} from './db';
import {isGlobalSearchableTerm} from './global-search-rules';

const LIMIT=6;
const MAX_QUERY_LENGTH=100;
const PEOPLE_CANDIDATE_LIMIT=60;
const clean=s=>String(s||'').trim().slice(0,MAX_QUERY_LENGTH);

export async function globalSearch(rawQuery){
  const q=clean(rawQuery);
  if(!isGlobalSearchableTerm(q))return{titles:[],people:[],sagas:[]};
  ...
  const [titles,people,sagas]=await Promise.all([
    ...
    sql`WITH candidates AS (
          SELECT p.tmdb_person_id,p.name,p.profile_path,p.known_for_department,
                 CASE WHEN p.tmdb_person_id::text=${q} THEN 0 WHEN lower(p.name)=${exact} THEN 1 WHEN lower(p.name) LIKE lower(${starts}) THEN 2 ELSE 3 END match_rank
          FROM people p
          WHERE p.tmdb_person_id::text=${q} OR lower(p.name) LIKE lower(${like})
