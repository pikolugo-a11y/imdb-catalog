...
export function dataComplete(r){
  if(String(r?.manual_data_decision||'')==='accepted_incomplete')return true;
  return [
    present(r.title_es),
...
export function lifecycleAfterDataRefresh(r){
  ...
  return{state:'PIKOSCORE_PENDING',reason:String(r?.manual_data_decision||'')==='accepted_incomplete'?'Datos incompletos aceptados manualmente; ratings/PikoScore pendientes':'Datos estructurales completos; ratings/PikoScore pendientes'};
}
