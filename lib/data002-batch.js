...
export async function selectData002BatchEligibleIds(sql=db()){
 ...
 const rows=await sql.query(`WITH candidates AS (
   SELECT m.imdb_id,cl.lifecycle_state,m.final_rating,${fresh} AS fresh_days
   ...
   WHERE ex.imdb_id IS NULL
     ...
     AND (cl.lifecycle_state='PIKOSCORE_PENDING' OR (m.final_rating IS NOT NULL AND cl.lifecycle_state NOT IN('IDENTITY_PENDING','IDENTITY_VALIDATION','IDENTITY_REVIEW_REQUIRED','DATA_INCOMPLETE')))
 ), ratings AS (
   SELECT c.imdb_id,c.lifecycle_state,c.final_rating,
   ...
   GROUP BY c.imdb_id,c.lifecycle_state,c.final_rating
 ) SELECT imdb_id FROM ratings
   WHERE lifecycle_state='PIKOSCORE_PENDING' OR fresh_rating_count<2 OR rating_count=0 OR fresh_rating_count<>rating_count
   ORDER BY CASE WHEN lifecycle_state='PIKOSCORE_PENDING' THEN 0 ELSE 1 END,imdb_id`);
...
export async function startData002Batch({limit=null,concurrency=2,triggerSource='calidad_datos_batch'}={}){
