...
export async function getCatalogV3(filters={}){
  ...
  const rows=await sql`SELECT c.imdb_id,c.type,c.display_title,c.original_title,c.year,c.runtime,
    ...
    CASE WHEN mv.pikoscore_version='2.0.0' AND mv.pikoscore_calculated_at IS NOT NULL THEN c.final_rating ELSE NULL END final_rating,
    c.imdb_rating,c.imdb_votes,c.fa_rating,c.fa_votes,c.tmdb_rating,c.tmdb_votes,
    ...
    WHERE (${q}='' OR lower(c.display_title) LIKE ${'%'+q+'%'} OR lower(COALESCE(c.original_title,'')) LIKE ${'%'+q+'%'})
      ...
      AND (${yearFrom}::int IS NULL OR c.year>=${yearFrom})
      AND (${yearTo}::int IS NULL OR c.year<=${yearTo})
    ...
    ORDER BY
      CASE WHEN ${sort}='title' AND ${dir}='asc' THEN lower(c.display_title) END ASC NULLS LAST,
      CASE WHEN ${sort}='title' AND ${dir}='desc' THEN lower(c.display_title) END DESC NULLS LAST,
      CASE WHEN ${sort}='year' AND ${dir}='asc' THEN c.year END ASC NULLS LAST,
      CASE WHEN ${sort}='year' AND ${dir}='desc' THEN c.year END DESC NULLS LAST,
      CASE WHEN ${sort}='score' AND ${dir}='asc' THEN CASE WHEN mv.pikoscore_version='2.0.0' AND mv.pikoscore_calculated_at IS NOT NULL THEN c.final_rating END END ASC NULLS LAST,
      CASE WHEN ${sort}='score' AND ${dir}='desc' THEN CASE WHEN mv.pikoscore_version='2.0.0' AND mv.pikoscore_calculated_at IS NOT NULL THEN c.final_rating END END DESC NULLS LAST,
      CASE WHEN ${sort}='imdb' AND ${dir}='asc' THEN c.imdb_rating END ASC NULLS LAST,
      CASE WHEN ${sort}='imdb' AND ${dir}='desc' THEN c.imdb_rating END DESC NULLS LAST,
      CASE WHEN ${sort}='votes' AND ${dir}='asc' THEN c.imdb_votes END ASC NULLS LAST,
      CASE WHEN ${sort}='votes' AND ${dir}='desc' THEN c.imdb_votes END DESC NULLS LAST,
      CASE WHEN ${sort}='runtime' AND ${dir}='asc' THEN c.runtime END ASC NULLS LAST,
      CASE WHEN ${sort}='runtime' AND ${dir}='desc' THEN c.runtime END DESC NULLS LAST,
      CASE WHEN ${sort}='type' AND ${dir}='asc' THEN c.type END ASC NULLS LAST,
      CASE WHEN ${sort}='type' AND ${dir}='desc' THEN c.type END DESC NULLS LAST,
      CASE WHEN ${sort}='status' AND ${dir}='asc' THEN COALESCE(c.effective_status,'missing') END ASC NULLS LAST,
      CASE WHEN ${sort}='status' AND ${dir}='desc' THEN COALESCE(c.effective_status,'missing') END DESC NULLS LAST,
      CASE WHEN mv.pikoscore_version='2.0.0' AND mv.pikoscore_calculated_at IS NOT NULL THEN c.final_rating END DESC NULLS LAST,c.imdb_votes DESC NULLS LAST,c.imdb_id
...
export async function getCatalogStatsV3(filters={}){
  const sql=db();
  const {q,type,status,genres,genreMode,genreCount,yearFrom,yearTo}=params(filters);
  const [row]=await sql`SELECT count(*)::int total,
    count(*) FILTER(WHERE c.effective_status='in_plex')::int in_plex,
    count(*) FILTER(WHERE c.effective_status='acquiring')::int acquiring,
    count(*) FILTER(WHERE c.effective_status NOT IN ('in_plex','acquiring') OR c.effective_status IS NULL)::int missing
    FROM catalog_read_model c
    WHERE (${q}='' OR lower(c.display_title) LIKE ${'%'+q+'%'} OR lower(COALESCE(c.original_title,'')) LIKE ${'%'+q+'%'})
      AND (${type}='' OR (${type}='movie' AND c.type=${movieType}) OR (${type}='series' AND c.type=ANY(${seriesTypes})))
      AND (${yearFrom}::int IS NULL OR c.year>=${yearFrom})
...
export async function getCatalogFiltersV3(){
  const sql=db();
  const [genres,range]=await Promise.all([sql`SELECT name_es value FROM genres ORDER BY name_es`,sql`SELECT min(year)::int min_year,max(year)::int max_year FROM catalog_read_model WHERE year IS NOT NULL`]);
  return{genres:genres.map(x=>x.value),minYear:range[0]?.min_year||1900,maxYear:range[0]?.max_year||new Date().getFullYear()};
}
