...
function parse(filters={}){
  ...
  const rawStatus=String(one(filters.status)||'');
  const status=rawStatus==='acquiring'?'missing':rawStatus;
...
export async function getCatalogV3(filters={}){
  ...
  const rows=await sql`SELECT c.imdb_id,c.type,c.display_title,c.original_title,c.year,c.runtime,
    ...
    CASE WHEN c.effective_status='in_plex' THEN 'in_plex' ELSE 'missing' END effective_status,c.plex_status,c.resolution,c.collection_name,c.poster_path,c.tmdb_id,c.tmdb_collection_id
    ...
    WHERE (${q}='' OR lower(c.display_title) LIKE ${'%'+q+'%'} OR lower(COALESCE(c.original_title,'')) LIKE ${'%'+q+'%'})
      ...
      AND (${status}='' OR (${status}='in_plex' AND c.effective_status='in_plex') OR (${status}='missing' AND c.effective_status IS DISTINCT FROM 'in_plex'))
    ORDER BY
      ...
      CASE WHEN ${sort}='status' AND ${dir}='asc' THEN CASE WHEN c.effective_status='in_plex' THEN 'in_plex' ELSE 'missing' END END ASC NULLS LAST, CASE WHEN ${sort}='status' AND ${dir}='desc' THEN CASE WHEN c.effective_status='in_plex' THEN 'in_plex' ELSE 'missing' END END DESC NULLS LAST,
...
export async function getCatalogStatsV3(filters={}){
  ...
  const [row]=await sql`SELECT count(*)::int total,count(*) FILTER(WHERE c.effective_status='in_plex')::int in_plex,count(*) FILTER(WHERE c.effective_status IS DISTINCT FROM 'in_plex')::int missing FROM catalog_read_model c WHERE (${q}='' OR lower(c.display_title) LIKE ${'%'+q+'%'} OR lower(COALESCE(c.original_title,'')) LIKE ${'%'+q+'%'}) AND (${type}='' OR (${type}='movie' AND c.type=${movieType}) OR (${type}='series' AND c.type=ANY(${seriesTypes}))) AND (${yearFrom}::int IS NULL OR c.year>=${yearFrom}) AND (${yearTo}::int IS NULL OR c.year<=${yearTo}) AND (${genreCount}=0 OR (${genreMode}='any' AND EXISTS(SELECT 1 FROM movie_genres_canonical mgc JOIN genres g ON g.id=mgc.genre_id WHERE mgc.imdb_id=c.imdb_id AND g.name_es=ANY(${genres}))) OR (${genreMode}='all' AND (SELECT count(DISTINCT g.name_es) FROM movie_genres_canonical mgc JOIN genres g ON g.id=mgc.genre_id WHERE mgc.imdb_id=c.imdb_id AND g.name_es=ANY(${genres}))=${genreCount})) AND (${status}='' OR (${status}='in_plex' AND c.effective_status='in_plex') OR (${status}='missing' AND c.effective_status IS DISTINCT FROM 'in_plex'))`;return row;
}
