...
function parseItem(row,item){
  let streamCount=0,bestV=null,bestA=null;
  for(const m of (item.Media||[]))for(const p of (m.Part||[]))for(const s of (p.Stream||[])){
    streamCount++;
...
export async function enrichPending(limit=DEFAULT_B_BATCH,includeErrors=false){
  ...
  const sql=db(),batch=Math.max(1,Math.min(250,Number(limit)||DEFAULT_B_BATCH)),runId=await startRun({formulaVersion:QUALITY_VERSION,batch,includeErrors,lifecycleGate:true});
  try{
    ...
    const candidates=await sql`
      ...
      ORDER BY p.plex_updated_at DESC NULLS LAST,p.rating_key LIMIT ${batch*10}`;
    ...
    const counts={processed:rows.length,updated:success.length,skipped:stale.length+blocked,errors:failed.length};
    await finishRun(runId,'success',counts,{formulaVersion:QUALITY_VERSION,batch,includeErrors,lifecycleGate:true,blockedByLifecycle:blocked,enriched:success.length,stale:stale.length,errors:failed.length,remaining:left.n,streamsInspected:success.reduce((a,x)=>a+x.streamCount,0)});
    return{requested:rows.length,enriched:success.length,stale:stale.length,errors:failed.length,blocked,remaining:left.n};
  }catch(e){await finishRun(runId,'failed',{processed:0,updated:0,skipped:0,errors:1},{formulaVersion:QUALITY_VERSION,lifecycleGate:true,error:String(e?.message||e)});throw e;}
}
