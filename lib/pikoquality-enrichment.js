...
export async function enrichPending(limit=DEFAULT_B_BATCH,includeErrors=false){
  ...
  try{
    ...
    await finishRun(runId,'success',counts,{formulaVersion:QUALITY_VERSION,batch,includeErrors,enriched:success.length,stale:stale.length,errors:failed.length,remaining:left.n,streamsInspected:success.reduce((a,x)=>a+x.streamCount,0)});
