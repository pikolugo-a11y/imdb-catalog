...
export async function GET(request){
  ...
  try{
    const observed=await executeObservedProcess({processCode:'PROC-PLAN-002',runKind:'system',triggerSource:'activity_planner',executor:'vercel',entityType:'planning',entityId:'automatic',idempotencyKey:`PROC-PLAN-002:${hourKey()}`,context:{surface:'/actividad',operation:'reconcile_plan_dispatch',automatic:true}},async()=>{
      ...
      return{functionalResult:changed?'updated':'no_change',metrics:{created_plans:created,launched,deferred,failed,purged_plans:purgedPlans,purged_process_runs:purgedProcessRuns,demand:result.demand},message:changed?`Planificación revisada: ${created} bloques nuevos, ${launched} lanzados, ${deferred} aplazados${failed?`, ${failed} con incidencia`:''}${purgedPlans?`, ${purgedPlans} planes antiguos purgados`:''}${purgedProcessRuns?`, ${purgedProcessRuns} ejecuciones técnicas antiguas purgadas`:''}`:'Planificación revisada sin cambios'};
    });
