...
export async function POST(req){
 ...
 return Response.json({ok:true,results:out,stats:{wikidata_imdb:Object.keys(wi.map).length,wikidata_tmdb:Object.keys(wt.map).length,wikidata_retry:Object.keys(wr.map).length,direct_fa:directHits,not_found:notFound},wikidata:{imdb_error:wi.error,tmdb_error:wt.error,retry_error:wr.error,ms:Number(wi.ms||0)+Number(wt.ms||0)+Number(wr.ms||0)}});
}
