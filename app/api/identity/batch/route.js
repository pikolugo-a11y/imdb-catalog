...
export async function POST(req){
 ...
 let braveHits=0,braveBlocked=0,braveNoCandidate=0,notFound=0;
 for(const imdbId of ids){const [state]=await sql`SELECT status,summary FROM pipeline_runs WHERE id=${runId}`;if(!state||state.status!=='running'||state.summary?.cancel_requested)return Response.json({ok:true,cancelled:true,results:out});const row=by.get(imdbId);if(!row){out.push({imdbId,ok:false,error:'Título no encontrado'});continue}try{let tmdbId=null,faId=null,method=[],brave=null;
   ...
   if((mode==='fa'||mode==='fa_test')&&!row.fa_id){faId=wi.map[imdbId]||null;if(faId)method.push('wikidata_imdb');if(!faId&&row.tmdb_id){faId=wt.map[String(row.tmdb_id)]||null;if(faId)method.push('wikidata_tmdb')}if(!faId){faId=wr.map[imdbId]||null;if(faId)method.push('wikidata_retry')}if(!faId){brave=await searchFaBrave(row,{singleQuery:mode==='fa_test'});faId=brave.faId;if(faId){method.push('fa_brave');braveHits++}else if(brave.blocked){method.push('fa_brave_blocked');braveBlocked++}else{method.push('fa_brave_not_found');braveNoCandidate++;notFound++}}}
   if(tmdbId||faId)await saveIdentity(imdbId,{tmdbId,faId,method:method.join('+')});out.push({imdbId,ok:true,updated:Boolean(tmdbId||faId),tmdbId,faId,method:method.join('+')||'already_complete',brave:brave?{blocked:Boolean(brave.blocked),blocked_reason:brave.blockedReason||null,status:brave.status||null,error:brave.error||null,requests:Number(brave.requests||0),query:brave.query||null,elapsed_ms:Number(brave.elapsedMs||0),candidates:Number(brave.candidates||0),fa_id:brave.faId||null}:null});
 }catch(e){out.push({imdbId,ok:false,error:e?.message||String(e)})}}
 return Response.json({ok:true,results:out,stats:{wikidata_imdb:Object.keys(wi.map).length,wikidata_tmdb:Object.keys(wt.map).length,wikidata_retry:Object.keys(wr.map).length,brave_fa:braveHits,brave_blocked:braveBlocked,brave_no_candidate:braveNoCandidate,not_found:notFound},wikidata:{imdb_error:wi.error,tmdb_error:wt.error,retry_error:wr.error,ms:Number(wi.ms||0)+Number(wt.ms||0)+Number(wr.ms||0)}});
}
