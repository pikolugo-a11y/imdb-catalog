...
export async function refreshRatingsAction(_prev,formData){let imdbId='';try{imdbId=id(formData);const requestKey=`PROC-DATA-002:manual:${imdbId}:${Math.floor(Date.now()/5000)}`;const observed=await executeObservedProcess({processCode:'PROC-DATA-002',runKind:'individual',triggerSource:'calidad_datos_manual',executor:'vercel',entityType:'title',entityId:imdbId,correlationKey:requestKey,idempotencyKey:requestKey,context:{surface:'/calidad/datos',operation:'refresh_ratings'}},async trace=>{const before=(await import('@/lib/title-ratings')).getTitleRatings?await (await import('@/lib/title-ratings')).getTitleRatings(imdbId):[];const r=await refreshRatingsForTitle(imdbId,{trace});const lifecycle=(await recomputeLifecycleForIds([imdbId])).get(imdbId),warnings=(r.steps||[]).filter(x=>x.status==='warning'&&x.expected!==true);for(const w of warnings)await recordProcessError(trace.runId,{error:new Error(w.error||`Warning ${w.provider}`),step:`ratings_${w.provider}`,source:w.provider,retryable:true,detail:{non_blocking:true}});const available=(r.ratings||[]).filter(x=>x.status==='available'&&Number(x.normalized_rating)>0);return{...r,lifecycle,technicalStatus:warnings.length?'partial':'succeeded',functionalResult:r.verified?'updated':'pending',before:{available:before.filter(x=>x.status==='available'&&Number(x.normalized_rating)>0).length,sources:before.map(x=>x.source)},after:{available:available.length,sources:available.map(x=>x.source),verified:r.verified,lifecycle:lifecycle?.state||lifecycle?.label||null},metrics:{available_ratings:available.length,verified:r.verified,rescue_steps:(r.steps||[]).filter(x=>x.status!=='skipped').length,warnings:warnings.length,expected_provider_misses:(r.steps||[]).filter(x=>x.status==='warning'&&x.expected===true).length},message:r.verified?'Ratings suficientes actualizados':'Ratings insuficientes tras la cascada de rescate'}});if(observed.reused){refresh(imdbId);return{ok:true,status:'duplicate',runId:observed.runId,message:'Esta solicitud ya se está procesando o acaba de procesarse. No se ha lanzado una segunda ejecución.'}}const r=observed.result;refresh(imdbId);const available=(r.ratings||[]).filter(x=>x.status==='available'&&Number(x.normalized_rating)>0);if(!r.verified)return{ok:false,status:'pending',runId:observed.runId,message:`Tras MDBList y los rescates disponibles siguen faltando ratings suficientes (${available.length}/2). Queda pendiente de resolución manual.`};return{ok:true,status:r.technicalStatus==='partial'?'partial':'updated',runId:observed.runId,message:`Ratings actualizados · ${available.length} fuentes disponibles · cascada MDBList → rescates sólo si fueron necesarios · estado: ${r.lifecycle?.label||r.lifecycle?.state||'actualizado'}.`}}catch(e){refresh(imdbId);return{ok:false,runId:e?.runId||null,message:e?.message||'No se pudieron actualizar los ratings'}}}
export async function calculatePikoScoreV3Action(_prev,formData){
  ...
  try{
    ...
    const observed=await executeObservedProcess({processCode:'PROC-DATA-003',runKind:'individual',triggerSource:'calidad_datos_manual',executor:'vercel',entityType:'title',entityId:imdbId,correlationKey:requestKey,idempotencyKey:requestKey,context:{surface:'/calidad/datos',operation:'calculate_pikoscore_v3'}},async trace=>{
      const {evaluatePikoScoreV3ForTitle}=await import('@/lib/pikoscore-v3');
      ...
      const preview=await evaluatePikoScoreV3ForTitle(imdbId);
      const before={score:preview.previous?.score??null,version:preview.previous?.version??null,confidence:preview.previous?.confidence??null,ratings_refreshed_at:preview.ratingsRefreshedAt||null};
      await trace.event({eventType:'step_completed',step:'evaluate_pikoscore',message:'PikoScore evaluado',data:{version:preview.version,score:preview.score,confidence:preview.confidence,source_count:preview.sourceCount,family_count:preview.familyCount,market:preview.market}});
      await trace.event({eventType:'step_started',step:'persist_pikoscore',message:'Guardando PikoScore canónico'});
      const r=await calculateAndSavePikoScoreV3ForTitle(imdbId);
      const lifecycle=(await recomputeLifecycleForIds([imdbId])).get(imdbId);
      await trace.event({eventType:'step_completed',step:'persist_pikoscore',message:'PikoScore guardado y Lifecycle recalculado',data:{lifecycle:lifecycle?.state||lifecycle?.label||null}});
      const changed=before.score!==r.score||before.version!==r.version||before.confidence!==r.confidence;
      return{...r,lifecycle,technicalStatus:'succeeded',functionalResult:changed?'updated':'no_change',before,after:{score:r.score,version:r.version,confidence:r.confidence,source_count:r.sourceCount,family_count:r.familyCount,market:r.market,lifecycle:lifecycle?.state||lifecycle?.label||null},metrics:{score:r.score,confidence:r.confidence,source_count:r.sourceCount,family_count:r.familyCount,family_coverage:r.familyCoverage,market:r.market},message:changed?'PikoScore recalculado y guardado':'PikoScore recalculado sin cambios funcionales'};
    });
    if(observed.reused){refresh(imdbId);return{ok:true,status:'duplicate',runId:observed.runId,message:'Esta solicitud ya se está procesando o acaba de procesarse. No se ha lanzado una segunda ejecución.'}}
    const r=observed.result;refresh(imdbId);const familyText=(r.contributions||[]).map(x=>`${x.family} ${x.score.toFixed(2)}`).join(' · ');return{ok:true,status:r.functionalResult||'updated',runId:observed.runId,message:`PikoScore 3.0 ${r.score.toFixed(2)} · confianza ${r.confidence.toFixed(1)}% · ${r.sourceCount} fuentes · ${r.familyCount} familias${familyText?` · ${familyText}`:''} · mercado ${r.market} · guardado · estado: ${r.lifecycle?.label||r.lifecycle?.state||'actualizado'}.`};
  }catch(e){refresh(imdbId);return{ok:false,runId:e?.runId||null,message:e?.message||'No se pudo calcular PikoScore 3.0'}}
}
