...
export async function startSer002BatchAction(formData){try{const r=await startSeriesBatch('PROC-SER-002',{limit:limitOf(formData)});refresh();return{ok:true,message:r.empty?'No hay detalle Plex pendiente.':r.reused?'Ya existe SER-002 activo.':`SER-002 preparado · ${r.eligibleCount} serie(s).`}}catch(e){refresh();return{ok:false,message:e?.message||'No se pudo iniciar SER-002'}}}
