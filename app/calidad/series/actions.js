...
import {syncPlexSeriesFast} from '@/lib/series-plex-sync-safe';
...
export async function syncPlexSeriesFastAction(){try{const r=await runTracked('plex_fast',null,()=>syncPlexSeriesFast());const continuation=await startSeriesBatch('PROC-SER-002',{triggerSource:'plex_sync_continuation'}).catch(error=>({error:String(error?.message||error)}));revalidate();const seriesDelta=`${r.created||0} nuevas · ${r.changed||0} modificadas · ${r.missing||0} eliminadas`,episodeDelta=`${r.episodeCreated||0} nuevos · ${r.episodeChanged||0} corregidos · ${r.episodeMissing||0} eliminados`;return{ok:true,message:`Plex: ${r.examined} series (${seriesDelta}) · ${r.episodeExamined||0} capítulos comprobados (${episodeDelta})${r.partial?' · revisión parcial':''}${continuation?.error?' · continuidad automática pendiente de reintento':''}`}}catch(e){return{ok:false,message:e?.message||'No se pudo sincronizar Plex'}}}
