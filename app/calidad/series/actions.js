...
export async function reviewSeriesDoubleEpisodeAction(formData){
  const ratingKey=String(formData.get('ratingKey')||'').trim(),season=Number(formData.get('season')),episode=Number(formData.get('episode')),mode=String(formData.get('mode')||'mark').trim(),sourceEpisode=Number(formData.get('sourceEpisode')||episode-1),combinedSize=Number(formData.get('combinedSize')||2);
  if(!ratingKey||!Number.isInteger(season)||!Number.isInteger(episode)||season<1||episode<1)throw new Error('Episodio inválido');if(!['mark','reopen'].includes(mode))throw new Error('Acción inválida');if(mode==='mark'&&(!Number.isInteger(sourceEpisode)||sourceEpisode<1||sourceEpisode===episode))throw new Error('Capítulo origen inválido');if(mode==='mark'&&![2,3].includes(combinedSize))throw new Error('Tamaño de capítulo combinado inválido');
  const targetEpisodes=mode==='mark'?Array.from({length:combinedSize-1},(_,i)=>episode+i):[episode],sql=db(),entityId=`${ratingKey}:S${season}E${episode}`,requestKey=`PROC-SER-005:${entityId}:combined:${combinedSize}:${mode}:${Math.floor(Date.now()/3000)}`;
  const observed=await executeObservedProcess({processCode:'PROC-SER-005',runKind:'individual',triggerSource:'calidad_series_manual',executor:'vercel',entityType:'episode',entityId,correlationKey:requestKey,idempotencyKey:requestKey,context:{surface:`/calidad/series/${ratingKey}`,operation:'review_series_combined_episode',rating_key:ratingKey,season,episode,source_episode:sourceEpisode,combined_size:combinedSize,target_episodes:targetEpisodes,mode}},async trace=>{
    ...
    if(mode==='reopen'){
      ...
      if(targets.some(x=>x.status!=='missing'))throw Object.assign(new Error('Alguno de los episodios ya no figura como faltante. Actualiza la serie y revisa de nuevo.'),{processStep:'load_target'});
