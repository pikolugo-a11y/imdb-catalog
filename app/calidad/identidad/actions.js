...
export async function startBraveTestAction(){const r=await dispatchIdentityBraveTest('identity_brave_test');refresh();if(!r.ok)return r;if(r.alreadyRunning)return{ok:true,message:'Ya hay una ejecución de Identidad en curso.'};return{ok:true,message:`Prueba Brave lanzada para ${Number(r.total||0).toLocaleString('es-ES')} títulos, uno a uno con 5 s de pausa.`}}
...
export async function retryMissingIdentityAction(_prev,formData){try{const id=imdb(formData),sql=db(),[row]=await sql`SELECT imdb_id,tmdb_id,fa_id FROM movies WHERE imdb_id=${id}`;if(!row)throw new Error('Título no encontrado');if(!row.tmdb_id||!row.fa_id)return{ok:false,message:'La identidad aún no está completa. Usa “Reanalizar todo” para buscar los IDs que faltan.'};const r=await refreshKnownIdentity(id);refresh(id);return{ok:true,message:`Datos refrescados: ${r?.title||id}`}}catch(e){return{ok:false,message:e?.message||'No se pudieron refrescar los datos'}}}
