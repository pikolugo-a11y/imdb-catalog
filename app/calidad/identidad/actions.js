...
import {dispatchIdentityRefresh,dispatchIdentityBraveTest,dispatchIdentitySingle,cancelIdentityRefresh} from '@/lib/identity-run-control';
...
export async function obtainIdentityAction(_prev,formData){try{const id=imdb(formData),r=await dispatchIdentitySingle(id,'identity_page_single');if(!r.ok)return r;return{ok:true,runId:r.runId,imdbId:id,message:'Obteniendo identidad…'}}catch(e){return{ok:false,message:e?.message||'No se pudo iniciar la obtención de identidad'}}}
