...
export default async function Identidad({searchParams}){
  ...
  const statusOptions=[['','Todos'],['untried',`Sin intentar (${nf(catalog.statusCounts.untried)})`],['not_found',`Sin coincidencia (${nf(catalog.statusCounts.not_found)})`],['error',`Error al obtener identidad (${nf(catalog.statusCounts.error)})`],['review',`Requiere revisión (${nf(catalog.statusCounts.review)})`],['processing',`Procesando (${nf(catalog.statusCounts.processing)})`]];
  return <div className="identity-page"><IdentityAutoRefresh/>
    ...
    <header className="identity-head"><div><div className="eyebrow">Integridad maestra</div><h1>Identidad</h1><p>Títulos pendientes de completar su identidad. Obtén o corrige TMDb para continuar a Validación de identidad.</p></div></header>
    ...
    <section className="identity-table-wrap"><table className="identity-table"><thead><tr><th>Título</th><th>Identidad / colección</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{catalog.rows.map(r=>{
      const meta=statusMeta[r.identity_status]||statusMeta.untried,detailHref=`/catalogo/${r.imdb_id}?from=${encodeURIComponent(currentPath)}`,imdbUrl=providerUrl('imdb',r.imdb_id,r.type),tmdbUrl=providerUrl('tmdb',r.tmdb_id,r.type);
      ...
      return <tr key={r.imdb_id}>
        <td><div className="identity-title-cell"><div><strong>{r.display_title}</strong><small>{r.original_title&&r.original_title!==r.display_title?`${r.original_title} · `:''}{r.year||'—'} · {r.type||'—'}</small></div></div></td>
