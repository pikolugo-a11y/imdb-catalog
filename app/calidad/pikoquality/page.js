...
function priorityReason(r){if(r.status==='error'&&r.source_fingerprint===r.fingerprint)return'Error técnico';if(r.rating_key&&(r.status==='stale'||r.formula_version!==QUALITY_VERSION||r.source_fingerprint!==r.fingerprint))return'PikoQuality desactualizado';if(!r.rating_key)return'Sin archivo Plex vigente';if(String(r.resolution||'').toLowerCase().includes('4k')&&Number(r.bitrate||0)>0&&Number(r.bitrate)<5000)return'Bitrate bajo para 4K';return'Pendiente sin evaluar'}
...
export default async function Page({searchParams}){
  ...
  const allDone=s.pending_a===0&&s.stale===0&&s.errors===0;
  return <div className={styles.page}>
    ...
    <section className={styles.hero}>
      ...
      <div className={styles.infoBox}><h3>Fuente de verdad</h3><ul><li>El análisis individual usa el snapshot persistido, no una lectura en vivo de Plex.</li><li>Un cambio de archivo invalida el resultado anterior.</li><li>La versión de fórmula forma parte de la validez.</li><li>Individual y Batch deberán producir exactamente el mismo resultado.</li></ul></div>
    </section>

    <section className={styles.metrics}>
      <Metric label="Cobertura" value={`${s.progressA}%`} sub={`${nf(s.evaluated)} / ${nf(s.total)} vigentes`}/>
      <Metric label="Películas en cola" value={nf(queue.total)} sub="Total real con los filtros actuales" href={paramsHref(current,{page:1,status:''})}/>
      ...
      <Metric label="Incidencias" value={nf(s.errors)} sub="Errores que requieren atención" href={paramsHref(current,{page:1,status:'error'})}/>
    </section>

    {allDone&&!hasFilters?<section className={styles.completedState}><div className={styles.completedIcon}>✓</div><div><span className={styles.kicker}>SIN TRABAJO PENDIENTE</span><h2>PikoQuality al día</h2><p>{nf(s.evaluated)} / {nf(s.total)} elementos elegibles vigentes. No se requiere intervención.</p></div></section>:<section className={styles.queueSection}>
      <div className={styles.panel}>
        <div className={styles.sectionHead}><div><h2>Películas que requieren trabajo</h2><p className={styles.subtitle}>Busca, filtra y analiza únicamente la página necesaria. Los resultados ya evaluados se consultan en Catálogo.</p></div><div className={styles.range}>{rangeStart}–{rangeEnd} de {nf(queue.total)}</div></div>
      ...
      <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Película</th><th>Archivo técnico</th><th>Audio</th><th>Prioridad / estado</th><th>Acción</th></tr></thead><tbody>{queue.rows.length?queue.rows.map(r=>{const [status,statusClass]=queueStatus(r);return <tr key={r.imdb_id}><td><strong>{r.title_es||r.plex_title||r.imdb_id}</strong><br/><span className={styles.muted}>{r.year||r.plex_year||'—'} · {r.imdb_id}</span></td><td><div className={styles.techMain}>{r.resolution||'—'} · {String(r.video_codec||'—').toUpperCase()} {r.bit_depth?`· ${r.bit_depth} bit`:''}</div><span className={styles.techSub}>{mbps(r.bitrate)}</span></td><td><div className={styles.techMain}>{String(r.audio_codec||'—').toUpperCase()}</div><span className={styles.techSub}>{r.audio_channels?`${r.audio_channels} canales`:'—'}</span></td><td><span className={`${styles.statusChip} ${statusClass}`}>{status}</span><span className={styles.priorityReason}>{priorityReason(r)}</span></td><td><AnalyzeForm imdbId={r.imdb_id}/></td></tr>}):<tr><td colSpan="5"><div className={styles.emptyState}><b>No hay elementos con estos filtros.</b><span>Modifica los filtros o vuelve a la cola completa.</span></div></td></tr>}</tbody></table></div>
    ...
    <section className={styles.bodyGrid}><div className={styles.panel}><h2>Salud de la fórmula activa</h2><div className={styles.distribution}>{distribution.map(([key,cls,range])=><div className={`${styles.band} ${cls}`} key={key}><span>{labels[key]}</span><strong>{pct(s.distribution[key],s.evaluated)}%</strong><small>{nf(s.distribution[key])} · {range}</small></div>)}</div><p className={styles.formulaNote}>Resumen de salud, no una vista de consulta. Las películas ya evaluadas se explotan en Catálogo. C5 permanece congelada para revalidación cuando avance la captura.</p></div><div className={styles.panel}><h2>Estado del dominio</h2><div className={styles.detailGrid}><div><span>Total elegible</span><b>{nf(s.total)}</b></div><div><span>Películas</span><b>{nf(s.movies)}</b></div><div><span>Episodios</span><b>{nf(s.episodes)}</b></div><div><span>Bloqueados lifecycle</span><b>{nf(s.blocked_by_lifecycle)}</b></div></div><div className={styles.domainState}><b>{s.recommendation.label}</b><span>{s.recommendation.description}</span></div></div></section>

    <div className={styles.adminNote}>ⓘ Captura técnica y cálculo PikoQuality son procesos distintos. El frontal consulta BBDD paginada y nunca recorre Plex completo para construir esta pantalla.</div>
  </div>;
}
