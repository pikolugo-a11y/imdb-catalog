...
export default async function Page({searchParams}){
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.metrics}>
      ...
      <Metric label="Películas en cola" value={nf(queue.overallTotal)} sub="Total real de trabajo de películas" href="/calidad/pikoquality"/>
    ...
    {allDone&&!hasFilters?<section className={styles.completedState}><div className={styles.completedIcon}>✓</div><div><span className={styles.kicker}>SIN TRABAJO PENDIENTE</span><h2>PikoQuality al día</h2><p>{nf(s.evaluated)} / {nf(s.total)} elementos elegibles vigentes. No se requiere intervención.</p></div></section>:<section className={styles.queueSection}>
      <div className={styles.panel}>
        <div className={styles.sectionHead}><div><h2>Películas que requieren trabajo</h2><p className={styles.subtitle}>Busca, filtra y analiza únicamente la página necesaria. Los resultados ya evaluados se consultan en Catálogo.</p></div><div className={styles.range}>{rangeStart}–{rangeEnd} de {nf(queue.total)}{hasFilters?` · ${nf(queue.overallTotal)} totales`:''}</div></div>
      ...
      <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Película</th><th>Archivo técnico</th><th>Audio</th><th>Prioridad / estado</th><th>Acción</th></tr></thead><tbody>{queue.rows.length?queue.rows.map(r=>{const [status,statusClass]=queueStatus(r);return <tr key={r.imdb_id}><td><strong>{r.title_es||r.title||r.original_title||r.plex_title||r.imdb_id}</strong><br/><span className={styles.muted}>{r.year||r.plex_year||'—'} · {r.imdb_id}</span></td><td><div className={styles.techMain}>{r.resolution||'—'} · {String(r.video_codec||'—').toUpperCase()} {r.bit_depth?`· ${r.bit_depth} bit`:''}</div><span className={styles.techSub}>{mbps(r.bitrate)}</span></td><td><div className={styles.techMain}>{String(r.audio_codec||'—').toUpperCase()}</div><span className={styles.techSub}>{r.audio_channels?`${r.audio_channels} canales`:'—'}</span></td><td><span className={`${styles.statusChip} ${statusClass}`}>{status}</span><span className={styles.priorityReason}>{r.priority_reason||'Pendiente sin evaluar'}</span></td><td><AnalyzeForm imdbId={r.imdb_id}/></td></tr>}):<tr><td colSpan="5"><div className={styles.emptyState}><b>No hay elementos con estos filtros.</b><span>Modifica los filtros o vuelve a la cola completa.</span></div></td></tr>}</tbody></table></div>
