...
export default function ActivityRefresh({active=false}){
  const router=useRouter();
  useEffect(()=>{
    let timer=null;
    const refresh=()=>{if(document.visibilityState==='visible')router.refresh();};
    const arm=()=>{clearInterval(timer);if(active&&document.visibilityState==='visible')timer=setInterval(refresh,30000);};
