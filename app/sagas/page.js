...
function statusOf(r){
  if(Number(r.actionable_total)===0)return{label:'Sin títulos exigibles',tone:'empty'};
  if(Number(r.missing)===0)return{label:'Completa en Plex',tone:'complete'};
...
export default async function Sagas({searchParams}){
  ...
  return <main className="sv4">
    ...
    <section className="sv4-kpis">
      ...
      <Link href="/sagas?state=no_plex"><span>Sin Plex</span><strong>{nf(s.no_plex)}</strong><small>ningún título exigible en Plex</small></Link>
    ...
    <section className="sv4-table-wrap">
      ...
      {data.rows.length===0?<div className="sv4-empty">No hay sagas con estos filtros.</div>:<>
        <table className="sv4-table"><thead><tr><th>Saga</th><th>Estado</th><th>En Plex</th><th>Faltan</th><th>PikoScore</th><th>Disponibilidad</th><th>Fuera PikoFilm</th><th>Periodo</th><th></th></tr></thead><tbody>{data.rows.map(r=>{const st=statusOf(r),hasActionable=Number(r.actionable_total)>0;return <tr key={r.tmdb_collection_id}><td><Link className="sv4-title" href={`/sagas/${r.tmdb_collection_id}`}>{poster(r.poster_path)?<img src={poster(r.poster_path)} alt={`Cartel de ${r.name_clean}`}/>:<span className="sv4-poster-empty">S</span>}<span><b>{r.name_clean}</b><small>{r.catalog_total} en PikoFilm · {r.total} en TMDb</small></span></Link></td><td><span className={`sv4-state ${st.tone}`}>{st.label}</span></td><td><b>{hasActionable?`${r.owned}/${r.actionable_total}`:'—'}</b></td><td>{hasActionable?(Number(r.missing)||'—'):'—'}</td><td>{r.saga_score!=null?<b className="sv4-score">{Number(r.saga_score).toFixed(2)}</b>:'—'}</td><td><span className="sv4-muted">{Number(r.cinema_count)?`${r.cinema_count} en cines`:''}{Number(r.cinema_count)&&Number(r.upcoming_count)?' · ':''}{Number(r.upcoming_count)?`${r.upcoming_count} próximas`:''}{!Number(r.cinema_count)&&!Number(r.upcoming_count)?(hasActionable?'Todo exigible':'Nada exigible ahora'):' '}</span></td><td>{Number(r.outside_catalog)||'—'}</td><td>{r.first_year||'—'}{r.last_year&&r.last_year!==r.first_year?`–${r.last_year}`:''}</td><td><Link className="sv4-open" href={`/sagas/${r.tmdb_collection_id}`}>Abrir →</Link></td></tr>})}</tbody></table>
        <div className="sv4-mobile-list">{data.rows.map(r=>{const st=statusOf(r),hasActionable=Number(r.actionable_total)>0;return <Link href={`/sagas/${r.tmdb_collection_id}`} key={r.tmdb_collection_id}><div><b>{r.name_clean}</b><span className={`sv4-state ${st.tone}`}>{st.label}</span></div><p>{hasActionable?`${r.owned}/${r.actionable_total} en Plex · ${r.missing} faltan`:'Sin títulos exigibles ahora'} · PikoScore {r.saga_score!=null?Number(r.saga_score).toFixed(2):'—'}</p><small>{r.cinema_count} en cines · {r.upcoming_count} próximas · {r.outside_catalog} fuera de PikoFilm</small></Link>})}</div>
      </>}
    </section>
