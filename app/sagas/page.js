...
export default async function Sagas({searchParams}){
  ...
  const spotlight=oneRows[0]||incompleteRows[0]||null;
  ...
  return <main className="sagas-modern">
    ...
    {rows.length===0?<div className="saga-empty"><b>No hay sagas en esta vista</b><p>Prueba otro estado o actualiza las colecciones.</p></div>:<section className="saga-modern-grid">{rows.map(r=><SagaCard key={r.tmdb_collection_id} r={r}/>)}</section>}
  </main>;
}
