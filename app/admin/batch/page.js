...
export default async function BatchAdmin(){
  ...
  const activeSources=d.limits.filter(x=>Boolean(x.enabled)).length;
  return <>
    ...
    <div className="mini-kpis"><Kpi label="Motor" value={paused?'PAUSADO':'PREPARADO'}/><Kpi label="Fuentes activas" value={`${activeSources}/${d.limits.length}`}/><Kpi label="Worker" value="NO INSTALADO"/><Kpi label="Runs" value={d.summary.runs}/><Kpi label="Jobs" value={d.summary.jobs}/><Kpi label="En cola" value={d.summary.queued}/><Kpi label="Activos" value={d.summary.active}/></div>
