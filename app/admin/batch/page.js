...
const durationMs=(a,b)=>a&&b?Math.max(0,new Date(b)-new Date(a)):null;
const fmtMs=ms=>{if(!Number.isFinite(Number(ms))||Number(ms)<0)return'—';const s=Math.round(Number(ms)/1000);if(s<60)return`${s} s`;const m=Math.floor(s/60),r=s%60;if(m<60)return`${m} min ${r?s%60+' s':''}`.trim();const h=Math.floor(m/60),rm=m%60;return`${h} h ${rm?rm+' min':''}`.trim()};
...
export default async function BatchAdmin({searchParams}){
  ...
  const limit=positiveBatchLimit(sp.limit,25);
  const d=await getBatchControlOverview();
  const paused=Boolean(d.control?.paused),active=d.activeRun,hasLive=Boolean(active&&['queued','running'].includes(active.status));
  const [preview,timing]=await Promise.all([getFlexibleBatchPreview({stage,retryMode:retry,limit}),getBatchTiming({stage,activeRunId:active?.id||null})]);
  ...
  return <>
    ...
    {active&&<section className="section compact-section batch-active"><div className="section-head"><div><h2>Run activo #{active.id}</h2><p>{label(active.target_stage)} · {retryLabel(active.limits?.retry_mode)} · {dt(active.started_at||active.created_at)}</p></div><strong>{activeProgress}</strong></div>
      <div className="batch-outcomes compact-outcomes"><span className="status ok">Corregido {active.corrected}</span><span className="status warn">No encontrado {active.not_found}</span><span className="status warn">Incompleto {active.incomplete}</span><span>Sin cambios {active.no_change}</span><span className="status bad">Error {active.functional_error}</span><span className="status bad">Revisión {active.manual_review}</span></div>
      <div className="batch-time-strip"><span><b>Media actual:</b> {liveAvg?fmtMs(liveAvg):'sin muestra suficiente'}/elemento</span><span><b>ETA:</b> {liveEta?fmtMs(liveEta):'—'}</span><span><b>Restantes:</b> {timing.active?.remaining??'—'}</span></div>
      <div className="quick-actions">{['running','queued'].includes(active.status)&&<form action={pauseRunAction}><input type="hidden" name="runId" value={active.id}/><button>Pausar run</button></form>}{active.status==='paused'&&<form action={resumeRunAction}><input type="hidden" name="runId" value={active.id}/><button>Reanudar run</button></form>}<form action={cancelRunAction}><input type="hidden" name="runId" value={active.id}/><button className="ghost danger-soft">Cancelar</button></form></div></section>}

    <section className="section compact-section" id="launch"><div className="section-head"><div><h2>Lanzar lote</h2><p>Previsualiza antes de ejecutar. El anti-bucle decide qué entra.</p></div></div>
      <form action={previewBatchRunAction} className="batch-launch-form compact-launch">
        ...
        <label><span>Cantidad</span><input name="limit" type="number" min="1" step="1" defaultValue={limit} inputMode="numeric"/></label>
      ...
      <div className="batch-preview compact-preview"><div className="batch-preview-main"><span>Entrarán</span><strong>{selectedCount}</strong><small>{preview.eligible} elegibles · {preview.counts.total} en etapa</small></div><div className="batch-preview-grid"><span><b>{preview.counts.new}</b>Nuevos</span><span><b>{preview.counts.context_changed}</b>Cambio contexto</span><span><b>{preview.counts.technical_due}</b>Error retry</span><span><b>{preview.counts.unresolved}</b>No resueltos</span><span><b>{preview.counts.cooldown}</b>Cooldown</span><span><b>{preview.counts.manual_review}</b>Revisión</span></div></div>
      <div className="batch-time-strip estimate"><span><b>Media histórica de {label(stage)}:</b> {historicalAvg?fmtMs(historicalAvg):'sin muestra suficiente'}/elemento</span><span><b>Muestra:</b> {timing.historical.samples} jobs</span><span><b>Tiempo estimado para {selectedCount}:</b> {estimatedTotal?fmtMs(estimatedTotal):'—'}</span></div>
      <form action={createBatchRunAction} className="batch-create-run compact-create"><input type="hidden" name="stage" value={stage}/><input type="hidden" name="limit" value={limit}/><input type="hidden" name="retryMode" value={retry}/><button disabled={!preview.eligible}>Crear run de {selectedCount}</button><span>{retryLabel(retry)} · {paused?'quedará en espera':'se podrá ejecutar ya'}</span></form>
      <details className="batch-fold"><summary>Ver títulos preseleccionados ({preview.selected.length}{selectedCount>25?' de '+selectedCount:''})</summary><div className="table-wrap"><table><thead><tr><th>Título</th><th>IMDb</th><th>Último resultado</th><th>Intentos</th></tr></thead><tbody>{preview.selected.map(x=><tr key={x.imdb_id}><td><b>{x.title}</b><span className="sub">{x.year||'—'} · {x.type||'—'}</span></td><td>{x.imdb_id}</td><td>{x.last_outcome?<span className={`status ${outcomeClass(x.last_outcome)}`}>{x.last_outcome}</span>:'Nunca intentado'}</td><td>{x.attempt_count}</td></tr>)}</tbody></table></div></details>
    </section>

    <section className="section compact-section"><div className="section-head"><div><h2>Últimos jobs</h2><p>Una fila por título. Entra en “Detalle” para ver steps, respuestas e historial.</p></div></div><div className="table-wrap"><table><thead><tr><th>Job</th><th>Título</th><th>Etapa</th><th>Resultado</th><th>Lifecycle</th><th>Tiempo</th><th></th></tr></thead><tbody>{recentJobs.map(j=><tr key={j.id}><td>#{j.id}</td><td><b>{titleOf(j)}</b><span className="sub">{j.entity_id} · {j.year||'—'}</span></td><td>{label(j.stage)}</td><td>{j.functional_outcome?<span className={`status ${outcomeClass(j.functional_outcome)}`}>{j.functional_outcome}</span>:j.status}</td><td>{j.lifecycle_before||'—'} → {j.lifecycle_after||'—'}</td><td>{fmtMs(durationMs(j.started_at,j.finished_at))}</td><td><Link className="title" href={`/admin/batch/job/${j.id}`}>Detalle →</Link></td></tr>)}</tbody></table></div></section>
