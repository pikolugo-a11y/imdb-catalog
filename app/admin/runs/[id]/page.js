...
export default async function RunDetail({params}){
  ...
  return <div className="ops-shell">
    ...
    <header className="ops-detail-hero"><div><div className="ops-kicker">Ejecución</div><h1>{r.process_code}</h1><p>{r.entity_type||'Sin entidad'}{r.entity_id?` · ${r.entity_id}`:''}</p></div><div className="ops-detail-state"><span className={`ops-badge ${tone(r.technical_status)}`}>{labels[r.technical_status]||r.technical_status}</span>{r.functional_result&&<strong>{results[r.functional_result]||r.functional_result}</strong>}</div></header>

    <section className="ops-detail-grid">
      <article><span>Solicitado</span><strong>{dt(r.requested_at)}</strong><small>{r.trigger_source}</small></article>
      <article><span>Ejecutor</span><strong>{r.executor}</strong><small>{r.run_kind}</small></article>
    ...
    <div className="ops-detail-columns">
      ...
      <aside className="ops-detail-side">
        ...
        <section className="ops-panel"><div className="ops-panel-head"><div><span className="ops-label">Correlación</span><h2>Identificadores</h2></div></div><dl className="ops-dl"><div><dt>Run ID</dt><dd>{r.run_id}</dd></div><div><dt>Parent</dt><dd>{r.parent_run_id||'—'}</dd></div><div><dt>Correlation</dt><dd>{r.correlation_key||'—'}</dd></div><div><dt>Idempotency</dt><dd>{r.idempotency_key||'—'}</dd></div></dl></section>
      </aside>
    </div>

    {d.children.length>0&&<section className="ops-panel ops-children"><div className="ops-panel-head"><div><span className="ops-label">Hijos</span><h2>Ejecuciones relacionadas</h2></div></div><div className="ops-runs">{d.children.map(c=><Link className="ops-run" href={`/admin/runs/${c.run_id}`} key={c.run_id}><div className={`ops-status-line ${tone(c.technical_status)}`}></div><div className="ops-run-main"><strong>{c.process_code}</strong><p>{c.entity_type||'sin entidad'}{c.entity_id?` · ${c.entity_id}`:''}</p><small>{dt(c.requested_at)}</small></div><div className="ops-run-meta"><span className={`ops-badge ${tone(c.technical_status)}`}>{labels[c.technical_status]||c.technical_status}</span><b>{duration(c.duration_ms)}</b></div></Link>)}</div></section>}
