...
export default async function RunDetail({params}){
  ...
  return <div className="ops-shell">
    ...
    {d.batchItems?.length>0&&<section className="ops-panel"><div className="ops-panel-head"><div><span className="ops-label">Items Batch</span><h2>Resultado por elemento</h2></div></div><div className="ops-detail-errors">{d.batchItems.map(i=><article key={i.item_id}><div><strong>{entityDisplay(i.entity_type)} · {i.entity_id}</strong><span className={i.status==='succeeded'?'resolved':''}>{labels[i.status]||i.status}</span></div><p>{i.last_error||'Sin error registrado en el item'}</p><small>{i.attempt_count||0} intento(s) · {dt(i.finished_at||i.started_at)}</small></article>)}</div></section>}
