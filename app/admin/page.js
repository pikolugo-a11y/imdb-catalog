...
export default async function Operations({searchParams}){
  ...
  return <div className="ops-shell">
    ...
    <section className="ops-panel"><div className="ops-panel-head"><div><span className="ops-label">Atención actual</span><h2>Incidencias activas</h2></div><span className="ops-coming">30 días de historial</span></div>{d.incidents.length===0?<div className="ops-empty compact"><b>Sin incidencias activas</b><span>Los fallos ya resueltos o superados por una ejecución correcta no aparecen aquí.</span></div>:<div className="ops-incident-list">{d.incidents.map((e,i)=>{const proc=processDisplay(e.process_code);return <article key={`${e.process_code}-${e.error_key}-${e.step}-${i}`}><div className="ops-incident-main"><div><strong>{proc.name}</strong><span>{e.error_key}{e.source?` · ${e.source}`:''}</span></div><p>{e.message}</p><small>{e.occurrences} ocurrencia(s) · {e.affected_entities} alcance(s) · última {dt(e.last_seen)}</small></div><div className="ops-incident-actions"><Link href={`/admin/runs/${e.sample_run_id}`}>Diagnosticar</Link><form action={resolveIncidentAction}><input type="hidden" name="errorId" value={e.sample_error_id}/><button>Descartar</button></form></div></article>})}</div>}</section>
    ...
    <section id="control" className="ops-control"><div className="ops-control-head"><span className="ops-label">Centro de control</span><h2>Actúa sólo donde hace falta</h2><p>Cuatro dominios claros. Los detalles internos no dominan la pantalla y las acciones delicadas siguen protegidas.</p></div><div className="ops-control-nav"><a href="#sistema">Sistema / Batch</a><a href="#fuentes">Fuentes y límites</a><a href="#recuperacion">Recuperación</a><a href="#mantenimiento">Mantenimiento</a></div>
      ...
      <section id="fuentes" className="ops-panel ops-domain"><div className="ops-panel-head"><div><span className="ops-label">Fuentes y límites</span><h2>Configurado, límite duro y estado efectivo</h2></div></div><OperationsApiSources sources={apiSources}/></section>
      <section id="recuperacion" className="ops-domain"><OperationsResetTitle/></section>
