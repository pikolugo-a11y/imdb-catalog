...
export default async function Catalogo({searchParams}){
  ...
  return <div className="catalog-v4">
    ...
    <section className="cv4-summary" aria-label="Resumen de resultados"><article><span>Total</span><strong>{total.toLocaleString('es-ES')}</strong></article><article><span>En Plex</span><strong>{Number(result.summary.in_plex||0).toLocaleString('es-ES')}</strong></article><article><span>Sin Plex</span><strong>{Number(result.summary.without_plex||0).toLocaleString('es-ES')}</strong></article></section>
