...
import {CATALOG_V4_PAGE_SIZE,catalogV4Href,getCatalogV4,getCatalogV4Countries,getCatalogV4Genres,parseCatalogV4} from '@/lib/catalog-v4-queries';
...
export default async function Catalogo({searchParams}){
  ...
  const clearHref=catalogV4Href(s,{q:'',plex:'all',countries:[],countryMode:'any',genres:[],genreMode:'any',yearFrom:null,yearTo:null,page:1});
  const sagaHref=`/sagas?catalogReturn=${encodeURIComponent(returnTo)}`;
  const hasFilters=Boolean(s.q||s.plex!=='all'||s.countries.length||s.genres.length||s.yearFrom!=null||s.yearTo!=null);
  return <div className="catalog-v4">
    <header className="cv4-hero"><div><span className="cv4-eyebrow">Base audiovisual</span><h1>Catálogo</h1><p>Tu colección maestra, ordenada para encontrar, comparar y consultar.</p></div><Link className="cv4-excluded-link" href="/catalogo/excluidas"><span aria-hidden="true">⊘</span> Excluidas</Link></header>
    <nav className="cv4-tabs" aria-label="Secciones del catálogo"><Link className={s.scope==='all'?'active':''} href={tabHref('all')}>Todo</Link><Link className={s.scope==='movies'?'active':''} href={tabHref('movies')}>Películas</Link><Link className={s.scope==='series'?'active':''} href={tabHref('series')}>Series</Link><Link href={sagaHref}>Sagas <span aria-hidden="true">↗</span></Link></nav>
    <CatalogFiltersV4 genres={genres} countries={countries} invalidYearRange={s.invalidYearRange}/>
    <section className="cv4-summary" aria-label="Filtrar por presencia en Plex">
      <Link className={s.plex==='all'?'active':''} href={catalogV4Href(s,{plex:'all',page:1})}><span>Total</span><strong>{summaryTotal.toLocaleString('es-ES')}</strong></Link>
    ...
    {s.invalidYearRange?<section className="cv4-state invalid"><strong>Revisa el rango de años</strong><p>El año inicial no puede ser posterior al año final. No se ha ejecutado la búsqueda.</p></section>:result.rows.length===0?<section className="cv4-state"><strong>No hay títulos que coincidan con estos filtros</strong><p>La consulta es válida, pero no devuelve obras de PikoFilm.</p>{hasFilters&&<Link href={clearHref}>Limpiar filtros</Link>}</section>:s.view==='posters'?<div className="cv4-poster-grid">{result.rows.map(r=><Link className="cv4-poster-card" key={r.imdb_id} href={titleHref(r,returnTo)}><div className="cv4-poster-art"><Poster path={r.poster_path} title={r.display_title}/><span className={`cv4-plex-dot ${r.effective_status==='in_plex'?'in':'out'}`} title={r.effective_status==='in_plex'?'En Plex':'Sin Plex'}>{r.effective_status==='in_plex'?'✓':'—'}</span></div><div className="cv4-poster-copy"><strong>{r.display_title}</strong><span>{r.year||'—'}{s.scope==='all'?` · ${typeLabel(r.type)}`:''} · {countryPreview(r.countries)}{seriesSize(r)?` · ${seriesSize(r)}`:''}</span><div><b>{fmt(r.final_rating)}</b><small>PikoScore</small>{s.scope==='series'&&<><b className="cv4-poster-rel">{relevanceScore(r.pikorelevancia)}</b><small>PikoRelevancia</small></>}<em>{r.effective_status==='in_plex'?'En Plex':'Sin Plex'}</em></div></div></Link>)}</div>:<div className="cv4-table-shell"><table className="cv4-table"><thead><tr>
      ...
      <td>{r.year||'—'}</td>
      {s.scope==='all'&&<td>{typeLabel(r.type)}</td>}
      <td title={(r.countries||[]).join(', ')}>{countryPreview(r.countries)}</td>
      <td>{genrePreview(r.genres)}</td>
