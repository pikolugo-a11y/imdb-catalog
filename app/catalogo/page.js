...
export default async function Catalogo({searchParams}){
  ...
  return <div className="catalog-v3 catalog-v3-rebuild catalog-r4">
    ...
    <div className="catalog-resultbar catalog-resultbar-r4"><span>Mostrando <b>{first.toLocaleString('es-ES')}–{last.toLocaleString('es-ES')}</b> de <b>{total.toLocaleString('es-ES')}</b>{hasFilters?' filtrados':''}</span><CatalogSortV3 sort={sort} dir={dir}/></div>
    ...
    {rows.length===0?<div className="catalog-empty"><b>No hay títulos con estos criterios</b><span>Prueba a limpiar o cambiar los filtros.</span></div>:view==='grid'?
      ...
      <div className="catalog-table-wrap"><table className="catalog-data-table"><thead><tr>
        <th><Link href={sortHref('title')}>Título{arrow('title')}</Link></th><th><Link href={sortHref('year')}>Año{arrow('year')}</Link></th><th><Link href={sortHref('type')}>Tipo{arrow('type')}</Link></th><th>Géneros</th><th>País</th>
        <th className="num"><Link href={sortHref('score')}>PikoScore{arrow('score')}</Link></th><th className="num"><Link href={sortHref('imdb')}>IMDb{arrow('imdb')}</Link></th><th className="num"><Link href={sortHref('votes')}>Votos{arrow('votes')}</Link></th><th className="num"><Link href={sortHref('runtime')}>Duración{arrow('runtime')}</Link></th><th><Link href={sortHref('status')}>Estado{arrow('status')}</Link></th><th>Acciones</th>
      </tr></thead><tbody>{rows.map(r=>{const href=`/catalogo/${r.imdb_id}?from=${encodeURIComponent(returnTo)}`;return <tr key={r.imdb_id}>
        <td className="catalog-table-title"><Link href={href}>{r.display_title}</Link><small>{r.original_title&&r.original_title!==r.display_title?r.original_title:''}</small></td>
        <td>{r.year||'—'}</td><td>{r.type||'—'}</td><td title={(r.genres||[]).join(', ')}>{genresText(r.genres)}</td><td title={r.country||''}>{firstCountry(r.country)}</td>
