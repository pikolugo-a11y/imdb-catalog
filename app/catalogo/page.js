...
export default async function Catalogo({searchParams}){
  ...
  const pageSize=catalogPageSize(view);
  ...
  const sort=String(p.sort||'score');
  const dir=String(p.dir||(sort==='title'?'asc':'desc'));
  ...
  const sortHref=key=>{
    ...
    const nextDir=current?(dir==='asc'?'desc':'asc'):(key==='title'?'asc':'desc');
    return '/catalogo?'+qs(p,{sort:key,dir:nextDir,page:1,view:'list'});
  };
  ...
  return <div className="catalog-v3 catalog-v3-rebuild catalog-r4">
    ...
    <div className="catalog-filter-shell catalog-filter-shell-r4">
      <CatalogFiltersV3 genres={f.genres} minYear={f.minYear} maxYear={f.maxYear} initial={initial}/>
      <div className="catalog-r4-side">
        <Link className="catalog-excluded" href="/catalogo/excluidas">◉ Excluidas</Link>
        <div className="catalog-view-switch" aria-label="Vista"><span>Vista</span><div><Link className={view==='grid'?'active':''} href={'/catalogo?'+qs(p,{view:'grid',page:1})} title="Carátulas">▦</Link><Link className={view==='list'?'active':''} href={'/catalogo?'+qs(p,{view:'list',page:1})} title="Tabla">☷</Link></div></div>
      </div>
    </div>
    ...
    <div className="catalog-resultbar catalog-resultbar-r4">
      ...
      <CatalogSortV3 sort={sort} dir={dir}/>
    </div>
    ...
    {rows.length===0?<div className="catalog-empty"><b>No hay títulos con estos criterios</b><span>Prueba a limpiar o cambiar los filtros.</span></div>:view==='grid'?
      <div className="media-grid catalog-poster-grid catalog-poster-grid-r4">{rows.map(r=><article className="media-action catalog-poster-item" key={r.imdb_id}><div className="catalog-poster-wrap"><MediaCard item={r} href={`/catalogo/${r.imdb_id}?from=${encodeURIComponent(returnTo)}`}/><span className={`catalog-poster-status ${r.effective_status||'missing'}`} title={statusLabel(r.effective_status)}>{r.effective_status==='in_plex'?'✓':r.effective_status==='acquiring'?'⋯':'!'}</span></div>{r.effective_status!=='in_plex'&&<div className="quick-actions"><form action={markAcquiring}><input type="hidden" name="imdbId" value={r.imdb_id}/><button>+ En proceso</button></form><form action={excludeTitle}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={returnTo}/><button className="danger-soft">Excluir</button></form></div>}</article>)}</div>:
      <div className="catalog-table-wrap"><table className="catalog-data-table"><thead><tr>
        <th><Link href={sortHref('title')}>Título{arrow('title')}</Link></th>
        <th><Link href={sortHref('year')}>Año{arrow('year')}</Link></th>
        <th><Link href={sortHref('type')}>Tipo{arrow('type')}</Link></th>
        <th>Géneros</th><th>País</th>
        <th className="num"><Link href={sortHref('score')}>PikoScore{arrow('score')}</Link></th>
        <th className="num"><Link href={sortHref('imdb')}>IMDb{arrow('imdb')}</Link></th>
        <th className="num"><Link href={sortHref('votes')}>Votos{arrow('votes')}</Link></th>
        <th className="num"><Link href={sortHref('runtime')}>Duración{arrow('runtime')}</Link></th>
        <th><Link href={sortHref('status')}>Estado{arrow('status')}</Link></th><th>Acciones</th>
      </tr></thead><tbody>{rows.map(r=><tr key={r.imdb_id}>
        <td className="catalog-table-title"><Link href={`/catalogo/${r.imdb_id}?from=${encodeURIComponent(returnTo)}`}>{r.display_title}</Link><small>{r.original_title&&r.original_title!==r.display_title?r.original_title:''}</small></td>
        <td>{r.year||'—'}</td><td>{r.type||'—'}</td><td title={(r.genres||[]).join(', ')}>{genresText(r.genres)}</td><td>{r.country||'—'}</td>
        <td className="num score-cell">{fmt1(r.final_rating)}</td><td className="num">{fmt1(r.imdb_rating)}</td><td className="num">{Number(r.imdb_votes||0).toLocaleString('es-ES')}</td><td className="num">{runtime(r.runtime)}</td>
        <td><span className="catalog-list-state"><StateDot state={r.effective_status==='in_plex'?'plex':r.effective_status==='acquiring'?'acquiring':'missing'}/>{statusLabel(r.effective_status)}</span></td>
        <td className="catalog-table-actions">{r.effective_status!=='in_plex'?<><form action={markAcquiring}><input type="hidden" name="imdbId" value={r.imdb_id}/><button>Proceso</button></form><form action={excludeTitle}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={returnTo}/><button>Excluir</button></form></>:'—'}</td>
      </tr>)}</tbody></table></div>}
