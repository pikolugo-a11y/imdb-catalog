...
export default async function Excluidas({searchParams}){
  ...
  return <div className="excluded-v3">
    ...
    <section className="excluded-filter-shell" aria-label="Filtros de excluidas">
      <form method="get" className="excluded-filter-form">
        ...
        <div className="excluded-filter-row">
          <div className="excluded-type" aria-label="Tipo">
            <span>Tipo</span>
            <Link className={!p.type?'active':''} href={'/catalogo/excluidas?'+qs(p,{type:'',page:1})}>Todos</Link>
            <Link className={p.type==='movie'?'active':''} href={'/catalogo/excluidas?'+qs(p,{type:'movie',page:1})}>Películas</Link>
            <Link className={p.type==='series'?'active':''} href={'/catalogo/excluidas?'+qs(p,{type:'series',page:1})}>Series</Link>
          </div>
    ...
    <section className="excluded-stats" aria-label="Resumen">
      <article><strong>{total.toLocaleString('es-ES')}</strong><span>Total excluidas</span></article>
      <article><strong>{Number(stats.movies||0).toLocaleString('es-ES')}</strong><span>Películas</span></article>
      <article><strong>{Number(stats.series||0).toLocaleString('es-ES')}</strong><span>Series / miniseries</span></article>
      <article><strong>{Number(stats.last30||0).toLocaleString('es-ES')}</strong><span>Últimos 30 días</span></article>
    </section>

    <div className="excluded-resultbar">
      <span>Mostrando <b>{first.toLocaleString('es-ES')}–{last.toLocaleString('es-ES')}</b> de <b>{total.toLocaleString('es-ES')}</b>{hasFilters?' filtradas':''}</span>
      <div className="excluded-result-tools">
        <form method="get" className="excluded-sort-form">
          {Object.entries(p).filter(([k])=>!['sort','page'].includes(k)).map(([k,v])=><input key={k} type="hidden" name={k} value={Array.isArray(v)?v[0]:v}/>) }
          <label>Ordenar<select name="sort" defaultValue={p.sort||'newest'}><option value="newest">Más recientes</option><option value="oldest">Más antiguas</option><option value="title">Título A–Z</option><option value="year_desc">Año ↓</option><option value="year_asc">Año ↑</option></select></label>
          <button>Aplicar</button>
        </form>
        <div className="excluded-view-switch"><Link className={view==='grid'?'active':''} href={'/catalogo/excluidas?'+qs(p,{view:'grid',page:1})} title="Carátulas">▦</Link><Link className={view==='list'?'active':''} href={'/catalogo/excluidas?'+qs(p,{view:'list',page:1})} title="Tabla">☷</Link></div>
      </div>
    </div>

    {rows.length===0?<div className="excluded-empty"><b>No hay títulos excluidos con estos criterios</b><span>{hasFilters?'Prueba a limpiar o cambiar los filtros.':'Cuando excluyas un título aparecerá aquí para poder recuperarlo.'}</span></div>:view==='grid'?
      <div className="excluded-grid">{rows.map(r=><article className="excluded-card" key={r.imdb_id}>
        <div className="excluded-poster"><Poster path={r.poster_path} title={r.display_title}/><span className="excluded-x">×</span></div>
        <div className="excluded-card-body"><b title={r.display_title}>{r.display_title}</b><span>{r.year||'—'} · {typeLabel(r.type)}</span>{r.final_rating!=null&&<small>★ {Number(r.final_rating).toFixed(1)}</small>}<div className="excluded-reason" title={r.reason||'Sin motivo'}>{r.reason||'Sin motivo indicado'}</div><time>{date(r.excluded_at)}</time></div>
        <form action={restoreTitle}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={restoreReturn}/><button>↻ Restaurar al catálogo</button></form>
      </article>)}</div>:
      <div className="excluded-table-wrap"><table className="excluded-table"><thead><tr><th>Título</th><th>Año</th><th>Tipo</th><th>PikoScore</th><th>IMDb</th><th>Motivo</th><th>Fecha exclusión</th><th>Acción</th></tr></thead><tbody>{rows.map(r=><tr key={r.imdb_id}><td><b>{r.display_title}</b><small>{r.original_title&&r.original_title!==r.display_title?r.original_title:r.imdb_id}</small></td><td>{r.year||'—'}</td><td>{typeLabel(r.type)}</td><td>{r.final_rating!=null?Number(r.final_rating).toFixed(1):'—'}</td><td>{r.imdb_rating!=null?Number(r.imdb_rating).toFixed(1):'—'}</td><td className="reason-cell" title={r.reason||''}>{r.reason||'Sin motivo'}</td><td>{date(r.excluded_at)}</td><td><form action={restoreTitle}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={restoreReturn}/><button>Restaurar</button></form></td></tr>)}</tbody></table></div>}
