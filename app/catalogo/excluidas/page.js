...
export default async function Excluidas({searchParams}){
  ...
  return <div className="excluded-v4">
    ...
    <div className="ev4-table-shell"><table className="ev4-table"><thead><tr><th>Título</th><th>Año</th><th>Tipo</th><th>Fecha de exclusión</th><th/></tr></thead><tbody>{result.rows.map(r=>{const href=`/catalogo/${r.imdb_id}?from=${encodeURIComponent(returnTo)}`;return <tr key={r.imdb_id}><td><Link href={href}>{r.display_title}</Link></td><td>{r.year||'—'}</td><td>{typeLabel(r.type)}</td><td>{date(r.excluded_at)}</td><td><form action={restoreExclusionAction}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={returnTo}/><PendingSubmitButton pendingLabel="Restaurando…">↻ Restaurar</PendingSubmitButton></form></td></tr>})}</tbody></table><div className="ev4-mobile-list">{result.rows.map(r=>{const href=`/catalogo/${r.imdb_id}?from=${encodeURIComponent(returnTo)}`;return <article key={r.imdb_id}><Link href={href}><strong>{r.display_title}</strong><span>{r.year||'—'} · {typeLabel(r.type)} · Excluida {date(r.excluded_at)}</span></Link><form action={restoreExclusionAction}><input type="hidden" name="imdbId" value={r.imdb_id}/><input type="hidden" name="returnTo" value={returnTo}/><PendingSubmitButton pendingLabel="Restaurando…">↻ Restaurar</PendingSubmitButton></form></article>})}</div></div>}

    {result.total>EXCLUDED_V4_PAGE_SIZE&&<nav className="ev4-pagination" aria-label="Paginación">{s.page>1?<Link href={pageHref(s.page-1)}>‹ Anterior</Link>:<span className="disabled" aria-disabled="true">‹ Anterior</span>}<span>Página <b>{s.page}</b> de {result.pageCount}</span>{s.page<result.pageCount?<Link href={pageHref(s.page+1)}>Siguiente ›</Link>:<span className="disabled" aria-disabled="true">Siguiente ›</span>}</nav>}
  </div>;
}
