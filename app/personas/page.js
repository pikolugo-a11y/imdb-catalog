  ...
  return <div className="personas-v4">
    ...
    <section className="pv4-discovery">
      <form className="pv4-tools">
        ...
        <label><span>Ordenar por</span><select name="sort" defaultValue={d.sort}><option value="relevance">Destacados · Relevancia</option><option value="score">Mejor PikoScore</option><option value="titles">Filmografía relevante</option><option value="plex">Más en Plex</option></select></label>
        <input type="hidden" name="role" value={d.role}/><button>Aplicar</button>
      </form>
      <div className="pv4-ranking-note"><span aria-hidden="true">✦</span><p><strong>Destacados</strong> favorece calidad e importancia cinematográfica con rendimientos decrecientes: una filmografía enorme de créditos marginales no gana por volumen.</p></div>
    </section>

    <div className="pv4-toolbar"><div><strong>{first.toLocaleString('es-ES')}–{last.toLocaleString('es-ES')}</strong><span> de {Number(d.total).toLocaleString('es-ES')} personas relevantes</span></div><span>{d.sort==='relevance'?'Orden principal: relevancia':d.sort==='score'?'Orden: PikoScore medio':d.sort==='titles'?'Orden: filmografía relevante':'Orden: presencia en Plex'}</span></div>
