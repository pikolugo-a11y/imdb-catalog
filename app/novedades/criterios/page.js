...
import {SPANISH_SERIES_MARKET_RESCUE} from '@/lib/news-discovery-settings.mjs';
...
export default async function Criterios({searchParams}){
  ...
  const rescue=`${SPANISH_SERIES_MARKET_RESCUE.minRating.toLocaleString('es-ES')} IMDb y ${SPANISH_SERIES_MARKET_RESCUE.minVotes.toLocaleString('es-ES')} votos`;
  return <div className="criteria-v4">
    ...
    <form action={saveNewsSettingsAction} className="criteria-form">
      <div className="criteria-grid">
        ...
        <RuleCard title="Series españolas" description={`Regla específica para series y miniseries españolas. Además, PikoFilm aplica un rescate de mercado desde ${rescue} para evitar que el menor volumen de voto español las deje fuera por escala.`} ratingName="seriesSpainRating" ratingValue={s.series.spain.minRating} votesName="seriesSpainVotes" votesValue={s.series.spain.minVotes}/>
