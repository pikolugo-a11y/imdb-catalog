...
export async function enrichNewsCandidateAction(formData){
  ...
  const snap=candidate.source_snapshot||{},isManual=snap.manual===true||snap.manual==='true',type=candidate.candidate_type==='movie'?'Película':'Serie',title=snap.title||snap.originalTitle||imdbId;const hasMinimumIdentity=title!==imdbId&&Boolean(candidate.candidate_type)&&['movie','tvSeries','tvMiniSeries'].includes(candidate.candidate_type);const run=await startRun('single_title','news',{stage:'adapter',imdb_id:imdbId});
  try{await sql`INSERT INTO movies(imdb_id,type,title,title_es,year,origin,source_status,synced_at,inclusion_origin) VALUES(${imdbId},${type},${title},${title},${candidate.year||null},'imdb_discovery','{"staging":true}'::jsonb,now(),${isManual?'imdb_manual':'imdb_discovery'})`;const result=await enrichTitle(imdbId);await sql`UPDATE movies SET inclusion_origin=${isManual?'imdb_manual':'imdb_discovery'},origin='imdb_discovery',source_status=COALESCE(source_status,'{}'::jsonb)-'staging' WHERE imdb_id=${imdbId}`;await sql`UPDATE catalog_candidates SET eligibility_status='catalogued',processed_at=now(),updated_at=now() WHERE imdb_id=${imdbId}`;await finishRun(run.id,'success',{processed:1,added:1,summary:{stage:'done',imdb_id:imdbId,title:result.title,origin:isManual?'manual':'discovery',enrichment:'complete'}});await audit('news','candidate',imdbId,'catalogue',{origin:isManual?'manual':'discovery',enrichment:'complete'})}
  catch(e){const err=errorInfo(e);if(hasMinimumIdentity){const partial={partial:true,enrichment_status:'pending',last_enrichment_error:err?.message||String(e),last_enrichment_at:new Date().toISOString()};await sql`UPDATE movies SET inclusion_origin=${isManual?'imdb_manual':'imdb_discovery'},origin='imdb_discovery',source_status=(COALESCE(source_status,'{}'::jsonb)-'staging')||${JSON.stringify(partial)}::jsonb WHERE imdb_id=${imdbId}`;await sql`UPDATE catalog_candidates SET eligibility_status='catalogued',processed_at=now(),updated_at=now(),source_snapshot=COALESCE(source_snapshot,'{}'::jsonb)||${JSON.stringify({lastEnrichmentError:partial.last_enrichment_at,enrichmentPartial:true})}::jsonb WHERE imdb_id=${imdbId}`;await finishRun(run.id,'success',{processed:1,added:1,errors:1,summary:{stage:'partial',imdb_id:imdbId,title,origin:isManual?'manual':'discovery',enrichment:'partial',pending_error:err}});await audit('news','candidate',imdbId,'catalogue_partial',{origin:isManual?'manual':'discovery',error:err?.message||String(e)});refreshNews();redirect(`/catalogo/${imdbId}?notice=news_added_partial`)}await sql`DELETE FROM movies WHERE imdb_id=${imdbId} AND source_status->>'staging'='true'`;await sql`UPDATE catalog_candidates SET eligibility_status='eligible',updated_at=now(),source_snapshot=COALESCE(source_snapshot,'{}'::jsonb)||${JSON.stringify({lastEnrichmentError:new Date().toISOString()})}::jsonb WHERE imdb_id=${imdbId}`;await finishRun(run.id,'failed',{processed:1,errors:1,summary:{stage:'failed_identity',imdb_id:imdbId,error:err}});refreshNews();redirect(`/novedades?notice=enrich_error&imdb=${encodeURIComponent(imdbId)}`)}
