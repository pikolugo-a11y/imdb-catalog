...
async function finish(job,result){
  ...
  await sql`UPDATE batch_jobs SET status='done',result=${JSON.stringify(result)}::jsonb,finished_at=now(),leased_until=NULL,updated_at=now() WHERE id=${job.id} AND worker_id=${WORKER_ID}`;
