...
async function main(){
  ...
  const wikidataCountryIds=await wikidataCountries();const tmdb=await tmdbResolve(basics.candidate_type);const countries=tmdb?.countries?.length?tmdb.countries:wikidataCountryIds;
  ...
  const snap={...prior,title:basics.title||prior.title||imdbId,originalTitle:basics.originalTitle||prior.originalTitle||basics.title||imdbId,isAdult:basics.isAdult,manual:true,manualActive:true,matchedRule:'manual',discoveryVersion:'novedades-v1',countries,countryStatus:countries.length?'resolved':'pending',wikidataCountryIds,datasetRatings:RATINGS,datasetBasics:BASICS,discoveredAt:prior.discoveredAt||nowIso(),manualAuthoritativeResolvedAt:nowIso(),manualResolver:'imdb-datasets+wikidata+tmdb',authoritativeStatus:'complete',tmdbId:tmdb?.tmdbId||prior.tmdbId||null,posterPath:tmdb?.posterPath||prior.posterPath||null,overview:tmdb?.overview||prior.overview||null};
  ...
  await adminEvent('manual_authoritative_resolved',{rating:rating.rating,votes:rating.votes,type:basics.candidate_type,year:basics.year,countries,wikidataCountryIds});
  console.log(JSON.stringify({imdbId,...basics,...rating,countries,wikidataCountryIds},null,2));
}
main().catch(async e=>{await adminEvent('manual_authoritative_failed',{error:e?.message||String(e)});console.error(e);process.exit(1)});
