import {claimBatchItem,executeClaimedItem,reconcileExpiredLeases,heartbeatPool,batchSql,refreshParent} from '../lib/batch-worker-runtime.mjs';
import {executeId001Canonical} from '../lib/id001-canonical.mjs';
import {refreshIdentityEvidenceCanonical,validateIdentityCanonical} from '../lib/identity-validation-canonical.mjs';
import {executeData001Canonical} from '../lib/data001-canonical.mjs';
import {refreshRatingsCanonical} from '../lib/ratings-refresh-core.mjs';
import {calculateAndSavePikoScoreV3WithSql} from '../lib/pikoscore-v3-batch.mjs';
import {refreshSeriesUnitaryCanonical} from '../lib/series-unitary-core.mjs';
import {confirmSeriesEsAvailabilityCanonical} from '../lib/series-es-availability-core.mjs';
import {rebuildSeriesQualityReadModelForRatingKey} from '../lib/series-quality-read-model-core.mjs';
import {refreshCatalogSeriesProfileCanonical} from '../lib/catalog-series-profile.mjs';
import {recomputeLifecycleWithSql} from '../lib/lifecycle-recompute-core.mjs';
import {refreshPersonFilmographyCanonical} from '../lib/people-refresh-core.mjs';
import {refreshSagaCollectionCanonical} from '../lib/saga-refresh-core.mjs';
import {createApiGate} from '../lib/batch-api-governance.mjs';
import {executeNov008} from '../lib/plex-news-worker.mjs';
import {computePikoRelevanceCanonical} from '../lib/pikorelevance-core.mjs';
import {startWakeServer} from '../lib/worker-wake-server.mjs';
const POOL='api',CAPACITY=Math.max(1,Math.min(Number(process.env.BATCH_API_CAPACITY)||3,16)),HEARTBEAT_MS=Math.max(5000,Math.min(Number(process.env.BATCH_HEARTBEAT_MS)||15000,60000)),workerId=`batch-api:${process.env.RAILWAY_REPLICA_ID||process.pid}`;
async function recomputeIdentityLifecycle(id,sql){const[row]=await sql`SELECT cl.lifecycle_state,m.tmdb_id FROM catalog_lifecycle cl JOIN movies m USING(imdb_id) WHERE cl.imdb_id=${id}`;if(!row)return null;const next=row.tmdb_id?'IDENTITY_VALIDATION':'IDENTITY_PENDING';await sql`UPDATE catalog_lifecycle SET previous_state=CASE WHEN lifecycle_state<>${next} THEN lifecycle_state ELSE previous_state END,lifecycle_state=${next},blocking_reason=CASE WHEN ${next}='IDENTITY_PENDING' THEN 'Falta identidad: TMDb' ELSE 'Identidad pendiente de validación' END,state_changed_at=CASE WHEN lifecycle_state<>${next} THEN now() ELSE state_changed_at END,computed_at=now() WHERE imdb_id=${id}`;return{state:next}}
async function executeIv001(sql,id,{trace,item}){const[cl]=await sql`SELECT lifecycle_state FROM catalog_lifecycle WHERE imdb_id=${id}`;if(!cl||cl.lifecycle_state!=='IDENTITY_VALIDATION')throw Object.assign(new Error('El título ya no está pendiente de evidencia en Validación'),{permanent:true});const r=await refreshIdentityEvidenceCanonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})});const lifecycle=await recomputeLifecycleWithSql(sql,[id]),next=lifecycle.get(id)?.label||null;return{...r,technicalStatus:'succeeded',functionalResult:r.complete?'updated':'pending',after:{...r.after,lifecycle:next},metrics:{evidence_complete:r.complete,duration_ms:r.durationMs},message:r.complete?'Evidencia preparada para validar':'Evidencia actualizada; sigue incompleta'}}
async function executeData001(sql,id,{trace,item}){const[cl]=await sql`SELECT lifecycle_state FROM catalog_lifecycle WHERE imdb_id=${id}`;if(!cl||cl.lifecycle_state!=='DATA_INCOMPLETE')throw Object.assign(new Error('El título ya no está pendiente de datos estructurales'),{permanent:true});return executeData001Canonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})})}
async function ratingFingerprint(sql,id){const rows=await sql`SELECT source,status,normalized_rating,votes FROM title_ratings WHERE imdb_id=${id} ORDER BY source`;return JSON.stringify(rows.map(r=>[String(r.source||''),String(r.status||''),r.normalized_rating==null?null:Number(r.normalized_rating),r.votes==null?null:Number(r.votes)]))}
async function executeData002(sql,id,{trace,item}){const[cl]=await sql`SELECT cl.lifecycle_state,m.final_rating FROM catalog_lifecycle cl JOIN movies m USING(imdb_id) WHERE cl.imdb_id=${id}`;if(!cl||(cl.lifecycle_state!=='PIKOSCORE_PENDING'&&cl.final_rating==null))throw Object.assign(new Error('El título ya no necesita ratings ni mantenimiento de PikoScore'),{permanent:true});const beforeFingerprint=await ratingFingerprint(sql,id),r=await refreshRatingsCanonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})}),afterFingerprint=await ratingFingerprint(sql,id),ratingsChanged=beforeFingerprint!==afterFingerprint,warnings=(r.steps||[]).filter(x=>x.status==='warning'&&!x.expected);if(!r.verified){const lifecycle=await recomputeLifecycleWithSql(sql,[id]),next=lifecycle.get(id)?.label||null;return{technicalStatus:warnings.length?'partial':'succeeded',functionalResult:'pending',before:r.before,after:{...r.after,lifecycle:next,ratings_changed:ratingsChanged},metrics:{available_ratings:r.available,verified:false,warnings:warnings.length,duration_ms:r.durationMs,ratings_changed:ratingsChanged,pikoscore_recalculated:false},message:'Ratings insuficientes tras la cascada de rescate'}}const shouldRecalculate=cl.lifecycle_state==='PIKOSCORE_PENDING'||cl.final_rating==null||ratingsChanged,score=shouldRecalculate?await calculateAndSavePikoScoreV3WithSql(sql,id,{trace}):null,lifecycle=await recomputeLifecycleWithSql(sql,[id]),next=lifecycle.get(id)?.state||null;return{technicalStatus:warnings.length?'partial':'succeeded',functionalResult:ratingsChanged||score?'updated':'no_change',before:{...r.before,...(score?{pikoscore:score.previous}:{})},after:{...r.after,...(score?{pikoscore:{score:score.score,confidence:score.confidence,version:score.version,source_count:score.sourceCount,family_count:score.familyCount}}:{}),lifecycle:next,ratings_changed:ratingsChanged},metrics:{available_ratings:r.available,verified:true,warnings:warnings.length,duration_ms:r.durationMs,ratings_changed:ratingsChanged,pikoscore_recalculated:Boolean(score),...(score?{pikoscore:score.score,pikoscore_confidence:score.confidence,pikoscore_sources:score.sourceCount}:{})},message:score?`Ratings actualizados y PikoScore ${score.score.toFixed(2)} calculado`:ratingsChanged?'Ratings actualizados':'Ratings comprobados sin cambios; PikoScore conservado'}}
async function executeSer003(sql,id,{trace,item}){const r=await refreshSeriesUnitaryCanonical(sql,{ratingKey:id,trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})});await rebuildSeriesQualityReadModelForRatingKey(sql,r.ratingKey);await trace?.event?.({eventType:'step_completed',step:'series_read_model',message:'Read model de Series actualizado',data:{rating_key:r.ratingKey}});return{...r,technicalStatus:'succeeded',functionalResult:'updated',after:{rating_key:r.ratingKey,tmdb_status:r.tmdbStatus,next_check_at:r.nextCheckAt,lifecycle:r.lifecycle?.label||null},metrics:{seasons:r.seasons,episodes:r.episodes},message:'Referencia TMDb de serie actualizada'}}
async function executeSer004(sql,id,{trace,item}){const r=await confirmSeriesEsAvailabilityCanonical(sql,{ratingKey:id,trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})});await rebuildSeriesQualityReadModelForRatingKey(sql,r.ratingKey);return{...r,technicalStatus:r.partial?'partial':'succeeded',functionalResult:r.changed?'updated':'pending',after:{rating_key:r.ratingKey,partial:r.partial,lifecycle:r.lifecycle?.label||null},message:r.partial?'Disponibilidad España comprobada parcialmente':'Disponibilidad España comprobada'}}
async function executeSer007(sql,id,{trace,item}){return refreshCatalogSeriesProfileCanonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})})}
async function executePer001(sql,id,{trace,item}){return refreshPersonFilmographyCanonical(sql,id,{trace,apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})})}
async function executeSaga001(sql,id,{trace,item}){return refreshSagaCollectionCanonical(sql,id,{trace,apiGate:createApiGate(sql,{batchRunId:item.batch_run_id})})}
async function executeRel001(sql,id,{trace}){return computePikoRelevanceCanonical(sql,id,{trace})}
async function executeLifecycleContinuation(sql,id,{trace,item}){
 const steps=[];let previousState=null;
 for(let guard=0;guard<8;guard++){
  const map=await recomputeLifecycleWithSql(sql,[id]),state=map.get(id)?.state||null;
  await trace?.event?.({eventType:'continuation_state',step:'lifecycle',message:`Lifecycle: ${state||'sin estado'}`,data:{state,iteration:guard}});
  if(!state)return{technicalStatus:'partial',functionalResult:'pending',after:{state:null,steps},message:'Lifecycle no disponible'};
  if(previousState===state){await trace?.event?.({eventType:'continuation_stalled',step:'lifecycle',message:`Lifecycle sin progreso en ${state}`,data:{state,iteration:guard,steps}});return{technicalStatus:'partial',functionalResult:'pending',after:{state,steps,stalled:true,blocked_in:state==='PIKOSCORE_PENDING'?'/calidad/datos':'/calidad'},message:`Lifecycle detenido: ${state} no cambió tras ejecutar su proceso canónico`};}
  previousState=state;
  if(state==='IDENTITY_PENDING'){const r=await executeId001Canonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id}),recomputeLifecycle:recomputeIdentityLifecycle});steps.push({state,process:'PROC-ID-001',result:r?.functionalResult||null});continue;}
  if(state==='IDENTITY_VALIDATION'){
   const r=await executeIv001(sql,id,{trace,item});steps.push({state,process:'PROC-IV-001',result:r?.functionalResult||null});
   if(!r.complete)return{technicalStatus:r.technicalStatus||'succeeded',functionalResult:'pending',after:{state:'IDENTITY_VALIDATION',steps,blocked_in:'/calidad/validacion-identidad',human_decision:true},message:'Evidencia de identidad incompleta; decisión visible en Calidad'};
   const validation=await validateIdentityCanonical(sql,id,{trace});steps.push({state,process:'PROC-IV-002',result:validation.status,score:validation.score??null});
   await recomputeLifecycleWithSql(sql,[id]);
   if(validation.status==='valid'){await trace?.event?.({eventType:'continuation_auto_approved',step:'identity_validation',message:'Identidad validada automáticamente por evidencia fuerte',data:{status:validation.status,score:validation.score??null}});continue;}
   return{technicalStatus:'succeeded',functionalResult:'pending',after:{state:'IDENTITY_VALIDATION',steps,blocked_in:'/calidad/validacion-identidad',validation_status:validation.status,validation_score:validation.score??null,human_decision:true},message:`Identidad ${validation.status}; decisión visible en Calidad`};
  }
  if(state==='DATA_INCOMPLETE'){const r=await executeData001(sql,id,{trace,item});steps.push({state,process:'PROC-DATA-001',result:r?.functionalResult||null});if(r?.functionalResult==='pending')return{technicalStatus:r.technicalStatus||'partial',functionalResult:'pending',after:{state,steps,blocked_in:'/calidad/datos'},message:'Datos incompletos; bloqueo visible en Calidad'};continue;}
  if(state==='PIKOSCORE_PENDING'){const r=await executeData002(sql,id,{trace,item});steps.push({state,process:'PROC-DATA-002',result:r?.functionalResult||null,pikoscore:r?.after?.pikoscore?.score??null});if(r?.functionalResult==='pending')return{technicalStatus:r.technicalStatus||'partial',functionalResult:'pending',after:{state,steps,blocked_in:'/calidad/datos'},message:'Ratings/PikoScore pendientes; visible en Calidad'};continue;}
  if(state==='SERIES_SYNC_PENDING'){const[row]=await sql`SELECT pcs.rating_key FROM plex_catalog_status pcs WHERE pcs.imdb_id=${id} AND pcs.status='in_plex' LIMIT 1`;if(!row?.rating_key)return{technicalStatus:'partial',functionalResult:'pending',after:{state,steps,blocked_in:'/calidad/series'},message:'Serie sin ratingKey Plex para continuar'};const r=await executeSer003(sql,row.rating_key,{trace,item});steps.push({state,process:'PROC-SER-003',result:r?.functionalResult||null});continue;}
  const human=['IDENTITY_REVIEW_REQUIRED','MOVIE_FILE_REVIEW','SERIES_REVIEW'].includes(state),blocked=state==='COMPLETE'?null:state.startsWith('MOVIE')?'/calidad/peliculas':state.startsWith('SERIES')?'/calidad/series':state==='TECH_PENDING'?'/calidad/pikoquality':'/calidad';
  return{technicalStatus:'succeeded',functionalResult:state==='COMPLETE'?'updated':'pending',after:{state,steps,blocked_in:blocked,human_decision:human},message:state==='COMPLETE'?'Lifecycle completado automáticamente':`Continuación detenida de forma trazable en ${state}`};
 }
 return{technicalStatus:'partial',functionalResult:'pending',after:{steps,guard_exhausted:true},message:'Continuación detenida por límite de seguridad'};
}
const adapters={'PROC-NOV-008':executeNov008,'PROC-ID-001':async(sql,id,{trace,item})=>executeId001Canonical(sql,id,{trace,lane:'batch',apiGate:createApiGate(sql,{batchRunId:item.batch_run_id}),recomputeLifecycle:recomputeIdentityLifecycle}),'PROC-IV-001':executeIv001,'PROC-DATA-001':executeData001,'PROC-DATA-002':executeData002,'PROC-SER-003':executeSer003,'PROC-SER-004':executeSer004,'PROC-SER-007':executeSer007,'PROC-PER-001':executePer001,'PROC-SAGA-001':executeSaga001,'PROC-REL-001':executeRel001,'PROC-LC-001':executeLifecycleContinuation};
async function recoverLifecycleAdapterMismatch(){const rows=await batchSql`UPDATE batch_run_items bi SET status='queued',lease_owner=NULL,lease_until=NULL,finished_at=NULL,updated_at=now() FROM batch_run_control c,process_runs pr WHERE bi.batch_run_id=c.run_id AND pr.run_id=c.run_id AND c.worker_pool='api' AND c.process_code='PROC-LC-001' AND c.closed_at IS NULL AND pr.technical_status='running' AND bi.status='failed' AND bi.last_error='Adapter API no registrado' AND bi.attempt_count<3 RETURNING bi.item_id,bi.batch_run_id`;return rows.length}
const active=new Set();let stopping=false;
const nap=ms=>new Promise(r=>setTimeout(r,ms));
async function launch(item){
  const execute=adapters[item.process_code];
  if(!execute){
    await batchSql`UPDATE batch_run_items SET status='failed',lease_owner=NULL,lease_until=NULL,last_error='Adapter API no registrado',finished_at=now(),updated_at=now() WHERE item_id=${item.item_id}`;
    await refreshParent(item.batch_run_id);
    return null;
  }
  const task=executeClaimedItem(item,{workerId,executor:'railway_batch_api',errorSource:'batch_api_worker',execute:(sql,id,ctx)=>execute(sql,id,{...ctx,item})})
    .catch(error=>console.error(JSON.stringify({type:'batch_item_unhandled',pool:POOL,item_id:item.item_id,error:String(error?.message||error)})))
    .finally(()=>active.delete(task));
  active.add(task);
  return task;
}
async function fill(){
  let claimed=0;
  while(!stopping&&active.size<CAPACITY){
    const item=await claimBatchItem({pool:POOL,workerId});
    if(!item)break;
    claimed++;
    await launch(item);
  }
  return claimed;
}
async function maintenance(){
  const recovered=await recoverLifecycleAdapterMismatch();
  if(recovered)console.log(JSON.stringify({type:'lifecycle_adapter_recovered',count:recovered}));
  const reconciled=await reconcileExpiredLeases({pool:POOL});
  if(reconciled.checked)console.log(JSON.stringify({type:'batch_reconcile',pool:POOL,...reconciled}));
  await heartbeatPool(POOL);
}
async function drain(){
  let lastMaintenance=0;
  while(!stopping){
    const now=Date.now();
    if(now-lastMaintenance>=HEARTBEAT_MS){
      try{await maintenance()}catch(error){console.error(JSON.stringify({type:'batch_api_maintenance_error',error:String(error?.message||error)}))}
      lastMaintenance=Date.now();
    }
    const claimed=await fill();
    if(active.size===0){
      if(claimed===0)break;
      continue;
    }
    await Promise.race([nap(HEARTBEAT_MS),...active]);
  }
  await Promise.allSettled([...active]);
}
const wake=startWakeServer({pool:POOL,onWake:drain});
process.on('SIGTERM',()=>{stopping=true;wake.close()});
process.on('SIGINT',()=>{stopping=true;wake.close()});
console.log(JSON.stringify({type:'batch_worker_started',mode:'wake_driven',pool:POOL,worker_id:workerId,capacity:CAPACITY,adapters:Object.keys(adapters)}));
