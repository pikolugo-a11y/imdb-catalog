...
import {ratingsTmdb,ratingsOmdb,ratingsImdbFallback,ratingsFilmAffinity,finalizeRatings,calculatePikoScore} from './lifecycle-pikoscore-executor.mjs';
...
async function executeData(job){const recipe=recipeFor(job.stage),begin=await beginLifecycleJob(sql,job,{retryMode:job.retryMode});if(!begin.eligible){await markLifecycleJobSkipped(sql,job,begin);return}
 ...
 const omdb=await guardedStep(job,byKey.data_omdb,()=>dataOmdb(sql,job.entity_id));steps.push(omdb);
 steps.push(await skipLifecycleStep(sql,job,byKey.data_imdb_fallback,omdb.ok?'OMDb ya cubre IMDb en el proceso unitario; fallback no necesario':'Fallback IMDb pertenece al proceso de ratings; datos conservan el valor existente'));
...
async function executePikoScore(job){const recipe=recipeFor(job.stage),begin=await beginLifecycleJob(sql,job,{retryMode:job.retryMode});if(!begin.eligible){await markLifecycleJobSkipped(sql,job,begin);return}
 const steps=[];const byKey=Object.fromEntries(recipe.steps.map((s,i)=>[s.key,{...s,order:i+1}]));
 const tmdb=await guardedStep(job,byKey.ratings_tmdb,()=>ratingsTmdb(sql,job.entity_id));steps.push(tmdb);
 const omdb=await guardedStep(job,byKey.ratings_omdb,()=>ratingsOmdb(sql,job.entity_id));steps.push(omdb);
 let imdbFallback;if(omdb.ok)imdbFallback=await skipLifecycleStep(sql,job,byKey.ratings_imdb_fallback,'OMDb devolvió IMDb nota/votos válidos');else imdbFallback=await guardedStep(job,byKey.ratings_imdb_fallback,()=>ratingsImdbFallback(sql,job.entity_id));steps.push(imdbFallback);
 const fa=await guardedStep(job,byKey.ratings_filmaffinity,()=>ratingsFilmAffinity(sql,job.entity_id));steps.push(fa);
 const imdbOk=omdb.ok||imdbFallback.ok,verified=Boolean(tmdb.ok&&imdbOk&&fa.ok);
 const finalized=await guardedStep(job,byKey.finalize_ratings,()=>finalizeRatings(sql,job.entity_id,verified));steps.push(finalized);
 if(verified&&finalized.ok&&finalized.result?.pikoscore_enabled)steps.push(await guardedStep(job,byKey.calculate_pikoscore,()=>calculatePikoScore(sql,job.entity_id)));else steps.push(await skipLifecycleStep(sql,job,byKey.calculate_pikoscore,'No se calcula hasta verificar TMDb + IMDb + FilmAffinity'));
 const criticalFailed=!verified||!finalized.ok||steps.some(s=>s.key==='calculate_pikoscore'&&!s.ok&&!s.skipped);
 const result=await finishLifecycleJob(sql,job,{before:begin.before,steps,complete:!criticalFailed,extra:{recipe:'PIKOSCORE_PENDING',unitary_reference:'refreshRatingsForTitle + calculatePikoScoreForTitle',sources_verified:verified}});
 await finishPhysical(job,result.outcome==='ERROR'?'failed':result.outcome==='REVISION_MANUAL'?'review':'done')}

async function processJob(job){try{if(job.stage==='DATA_INCOMPLETE')return await executeData(job);if(job.stage==='PIKOSCORE_PENDING')return await executePikoScore(job);throw Object.assign(new Error(`Etapa aún no conectada al worker Lifecycle: ${job.stage}`),{permanent:true})}catch(e){await markLifecycleJobError(sql,job,{error:e});await sql`UPDATE batch_jobs SET status=${e?.paused||e?.sourceDisabled||e?.sourceBudget?'retry_wait':'failed'},available_at=CASE WHEN ${Boolean(e?.paused||e?.sourceDisabled||e?.sourceBudget)} THEN now()+interval '10 minutes' ELSE available_at END,error_class=${e?.permanent?'permanent':'technical'},error_message=${String(e?.message||e).slice(0,1000)},finished_at=CASE WHEN ${Boolean(e?.paused||e?.sourceDisabled||e?.sourceBudget)} THEN NULL ELSE now() END,leased_until=NULL,updated_at=now() WHERE id=${job.id}`}}

process.on('SIGTERM',()=>{stopping=true});process.on('SIGINT',()=>{stopping=true});
console.log(`[lifecycle-worker] ${WORKER_ID} listo; etapas=DATA_INCOMPLETE,PIKOSCORE_PENDING`);
