...
async function claimRequest(){const [r]=await sql`WITH pick AS (SELECT id FROM admin_job_requests WHERE job_type='imdb_discovery' AND status='pending' ORDER BY requested_at LIMIT 1 FOR UPDATE SKIP LOCKED) UPDATE admin_job_requests q SET status='running',dispatched_at=now() FROM pick WHERE q.id=pick.id RETURNING q.*`;return r||null}
async function startRun(source){const [r]=await sql`INSERT INTO pipeline_runs(job_type,source,status,started_at,summary,created_at,updated_at) VALUES('imdb_discovery',${source},'running',now(),'{}'::jsonb,now(),now()) RETURNING id`;return r.id}
...
async function loadKnown(){
  const [catalog,excluded,candidates]=await Promise.all([
    sql`SELECT imdb_id FROM movies`,sql`SELECT imdb_id FROM catalog_exclusions`,sql`SELECT imdb_id,eligibility_status,source_snapshot FROM catalog_candidates`
  ]);
...
async function readRatings(settings){
  ...
  const out=new Map();let scanned=0;
  const rl=createInterface({input:await stream(RATINGS),crlfDelay:Infinity});let head=true;
...
async function resolveCountries(candidates,known){
  const map=new Map(),need=[];
  for(const c of candidates){const cached=known.candidates.get(c.imdb_id)?.source_snapshot?.countries;if(Array.isArray(cached)&&cached.length)map.set(c.imdb_id,new Set(cached));else need.push(c.imdb_id)}
  const wiki=await wikidataCountries(need);for(const [k,v] of wiki)map.set(k,v);
  const unresolved=candidates.filter(c=>!map.has(c.imdb_id)||map.get(c.imdb_id).size===0);
  await pool(unresolved.slice(0,800),8,async c=>{const codes=await tmdbCountry(c.imdb_id,c.candidate_type);if(codes.length)map.set(c.imdb_id,new Set(codes))});
  return map;
}
...
async function main(){
  const queued=process.argv.includes('--queued'),request=queued?await claimRequest():null;if(queued&&!request){console.log('Sin solicitudes pendientes');return}
  const runId=await startRun(queued?'queue':'schedule'),started=Date.now();
  try{
    ...
    for(const c of basicsPhase.candidates){const countries=[...(countryMap.get(c.imdb_id)||[])],isIndia=countries.some(x=>excluded.has(x)),isSpain=countries.some(x=>x==='ES'||x==='Q29');let eligibility='not_eligible',matchedRule=null,countryStatus=countries.length?'resolved':'unknown';if(isIndia){eligibility='rejected';rejectedCountry++}else if(c.general){if(countries.length||!process.env.TMDB_API_TOKEN){eligibility='eligible';matchedRule='general';general++}else{eligibility='not_eligible';countryStatus='pending';pendingCountry++}}else if(c.spainZone&&isSpain){eligibility='eligible';matchedRule='spain';spain++}else if(c.spainZone&&!countries.length){eligibility='not_eligible';countryStatus='pending';pendingCountry++}
    ...
    await sql`UPDATE catalog_candidates SET eligibility_status='not_eligible',last_evaluated_at=now(),updated_at=now() WHERE source_snapshot->>'discoveryVersion'='novedades-v1' AND COALESCE(source_snapshot->>'manual','false')<>'true' AND eligibility_status IN('eligible','processing') AND last_seen_at < to_timestamp(${Date.now()/1000})`;
    const summary={settingsVersion:settings.version||1,elapsedSeconds:Math.round((Date.now()-started)/1000),ratingsScanned:ratingsPhase.scanned,ratingsPreselected:ratingsPhase.ratings.size,basicsScanned:basicsPhase.scanned,potentialCandidates:basicsPhase.matched,generalEligible:general,spanishRescues:spain,rejectedCountry,pendingCountry,rowsUpserted:rows.length};
    await finishRun(runId,'success',summary,{processed:basicsPhase.matched,added:general+spain,skipped:rejectedCountry+pendingCountry});if(request)await sql`UPDATE admin_job_requests SET status='success',finished_at=now() WHERE id=${request.id}`;console.log(JSON.stringify(summary,null,2));
  }catch(e){await finishRun(runId,'failed',{error:e?.message||String(e)},{errors:1});if(request)await sql`UPDATE admin_job_requests SET status='failed',finished_at=now(),error=${String(e?.message||e).slice(0,1000)} WHERE id=${request.id}`;throw e}
}
