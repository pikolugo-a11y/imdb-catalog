...
def run(payload):
    ...
    if not titles: return {'ok': False, 'status': 'error', 'error': 'missing_title'}
    ...
    try:
        ...
        if not ranked:
            return {'ok': True, 'status': 'not_found', 'fa_id': None, 'confidence': 0, 'queries': queries, 'candidates': [], 'elapsed_s': round(time.perf_counter() - started, 3)}

        best = ranked[0]; second = ranked[1]['score'] if len(ranked) > 1 else 0; margin = best['score'] - second
        if best['score'] < MIN_ACCEPT_SCORE:
            return {'ok': True, 'status': 'ambiguous', 'fa_id': None, 'best_fa_id': best['id'], 'confidence': best['score'], 'margin': margin, 'candidates': ranked[:5], 'queries': queries, 'elapsed_s': round(time.perf_counter() - started, 3)}
        ...
        return {'ok': True, 'status': status, 'fa_id': best['id'], 'confidence': vscore, 'margin': margin, 'candidates': ranked[:5], 'queries': queries, 'verified': {'id': best['id'], 'title': verified.get('title'), 'original_title': verified.get('original_title'), 'year': vy}, 'elapsed_s': round(time.perf_counter() - started, 3)}
    except Exception as exc:
        message = str(exc); low = message.lower()
        blocked = any(x in low for x in ['429', '403', 'too many requests', 'captcha', 'blocked'])
        return {'ok': False, 'status': 'blocked' if blocked else 'error', 'fa_id': None, 'error': message[:300], 'elapsed_s': round(time.perf_counter() - started, 3)}
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'ok': False, 'status': 'error', 'error': str(exc)[:300]}, ensure_ascii=False))
        sys.exit(1)
