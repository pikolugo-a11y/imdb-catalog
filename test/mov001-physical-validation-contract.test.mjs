...
test('MOV-001 Batch selecciona sólo la fase física canónica y conserva ejecución manual',()=>{assert.match(batch,/lifecycle_state='MOVIE_FILE_PENDING'/);assert.match(batch,/triggerSource='calidad_peliculas_batch'/);assert.match(batch,/worker_pool:'fast'/);assert.match(batch,/startMov001Batch/)});
test('el sync Plex manual invalida cambios físicos y encadena MOV-001 automáticamente después',()=>{assert.match(plex,/recomputePhysicalMovieChanges/);assert.match(plex,/mfv\.source_fingerprint IS DISTINCT FROM movie_physical_fingerprint/);assert.match(plex,/recomputeLifecycleForIds\(ids\)/);assert.match(plex,/startMov001Batch\(\{triggerSource:'plex_sync_continuation'\}\)/);assert.match(plex,/functionalContinuation/);assert.doesNotMatch(plex,/validateMovieFile/)});