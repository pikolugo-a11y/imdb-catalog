...
test('DATA-001 UI is ordered before ratings and supports progressive sizes',()=>{assert.ok(panels.indexOf('Data001BatchPanel')<panels.indexOf('Data002BatchPanel'));assert.match(actions,/startData001Batch/);const panel=await readFile(new URL('../components/Data001BatchPanel.js',import.meta.url),'utf8');assert.match(panel,/1 título/);assert.match(panel,/5 títulos/);assert.match(panel,/25 títulos/);assert.match(panel,/Todos/)});
