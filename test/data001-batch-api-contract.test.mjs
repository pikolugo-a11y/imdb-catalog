...
test('DATA-001 uses shared fail-closed API governance in manual and Batch lanes',()=>{assert.match(unit,/createApiGate/);assert.match(unit,/lane:'manual'/);assert.match(worker,/lane:'batch'/);assert.match(canonical,/requireApiGate\(apiGate,source\)/);assert.match(canonical,/gate\.acquire\(source/);assert.match(canonical,/gate\.release\(permit\)/);assert.match(canonical,/retry-after/);assert.match(canonical,/apiGateReason='rate_limited'/)});
