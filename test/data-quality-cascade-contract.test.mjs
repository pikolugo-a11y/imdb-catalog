...
const unit=fs.readFileSync(new URL('../lib/data-quality-unitary.js',import.meta.url),'utf8');
const source=fs.readFileSync(new URL('../lib/data001-canonical.mjs',import.meta.url),'utf8');
...
test('cada proveedor recibe IMDb ID y solo campos pendientes y soportados',()=>{assert.match(source,/function wanted\(row,source\)/);assert.match(source,/row\.missing\.filter\(k=>supported\.has\(k\)\)/);assert.match(source,/new Set\(attempted\)/);assert.match(source,/sourceStep\(sql,imdbId,source,fn,row,ctx\)/)});
test('Completar datos respeta el tipo validado y no reidentifica ni cambia de medio',()=>{assert.match(source,/mediaFromType\(before\.type\)/);assert.doesNotMatch(source,/\/find\//);assert.doesNotMatch(source,/ensureTmdbIdentity/);assert.doesNotMatch(source,/recoverMediaTypeIfNeeded/);assert.match(unit,/executeData001Canonical/)});
test('TMDb profundiza en episodios, créditos e imágenes cuando el tipo validado es serie',()=>{assert.match(source,/seriesEpisodeFacts/);assert.match(source,/\/episode\/\$\{ref\.episode\}\/credits/);assert.match(source,/\/images\?include_image_language=es,null,en/);assert.match(source,/tmdb_series_season/)});
...
test('duración de serie cuenta para cobertura pero no bloquea el avance',()=>{assert.match(dataQuality,/effectiveSeverity=\(k,r\)=>k==='runtime'&&isSeries\(r\)\?'optional'/);assert.match(source,/k==='runtime'&&isSeries\(row\.type\)/)});
