CREATE TABLE IF NOT EXISTS series_quality_overrides (
  show_rating_key text PRIMARY KEY REFERENCES series_reference(show_rating_key) ON DELETE CASCADE,
  decision text NOT NULL CHECK (decision IN ('manual_complete')),
  note text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

COMMENT ON TABLE series_quality_overrides IS 'Decisiones manuales a nivel de serie. No alteran la evidencia física Plex ni series_diagnostics.';
COMMENT ON COLUMN series_quality_overrides.decision IS 'manual_complete: Calidad trata todos los episodios oficiales como cubiertos hasta que el usuario retire la decisión.';

CREATE OR REPLACE VIEW series_episode_effective_status AS
WITH base AS (
  SELECT d.show_rating_key,
         d.season_number,
         d.episode_number,
         ...
         COALESCE(qo.decision='manual_complete',false) AS manual_complete
  ...
  LEFT JOIN series_quality_overrides qo
    ON qo.show_rating_key=d.show_rating_key
   AND qo.decision='manual_complete'
), resolved AS (
  SELECT b.*,
         COALESCE(
           b.episode_status,
...
SELECT show_rating_key,
       ...
       CASE
         WHEN manual_complete THEN 'present'
